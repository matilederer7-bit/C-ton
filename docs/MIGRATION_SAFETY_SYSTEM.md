...
- Hosted hosts need `--allow-hosted` for both tools; neither has ever been run against a hosted database from a repository session. Staging and production are never repaired automatically: the doctor (read-only) is run first, and any repair is an explicit owner action with `--yes`.

## Numbering rule for parallel branches

Ids `062`-`064` were reserved by parallel branches and never landed; master took `065`/`066`; the R9C financial rails appended `067`/`068` after them (PR #9, on master and on staging at ledger positions 60/61). Whichever branch lands second appends AFTER the other so ledger positions stay contiguous (`scripts/migration_manifest.cjs` comment). Historical migrations are never rewritten; a checksum mismatch on a persistent developer database is an environment finding (CRLF-era ledger, `migrations:doctor` classifies it) and never a product-correctness finding. The preflight's "upgrade from origin/master" scenario is the check that a branch did not edit an already-applied file and that the merged manifest still produces the fresh-install schema.
