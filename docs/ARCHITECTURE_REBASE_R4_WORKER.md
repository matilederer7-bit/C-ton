...
## R4I — Deployment readiness (NOT provisioned)

Intended Render Background Worker definition — deliberately NOT added to
`render.yaml`, because Blueprint autosync would immediately provision a PAID
instance; the architecture gate continues to assert the blueprint contains
no Worker service:

```yaml
# FUTURE r4 worker service (do not add before the cost gate is approved):
#  - type: worker
#    name: siton-staging-worker
#    runtime: docker
#    region: frankfurt          # same region as the Web service and close to eu-central-1
#    plan: starter              # 0.5 CPU / 512 MB — cheapest valid worker plan
#    branch: master
...
#             siton_worker_login secret — NEVER the Web login),
