...
> `claude/shelf-heavy-closeout-20260917`, source `codex/closed-pilot-war-game`). What was carried:
> the settled-money admin-overview fix (`settled_units` / `settled_delivery_cost` in
> `/api/admin/overview`) with a focused regression test
> (`tests/admin_overview_settlement_truth_validation.ts`), and the rehearsal harness
> `scripts/closed_pilot_war_game.cjs` + `scripts/closed_pilot_scenario.cjs` (`npm run qa:pilot-war-game`,
> `-- --scenario-only`, `-- --only=<suite>`). The harness applies whatever the current migration
> manifest contains (63 ledger rows at port time, not the 58 below) and every allowlisted suite still
> exists on master. Figures below are the original 2026-09-09 run on base `d4c7877`; the port's own
> run evidence is recorded in `PROJECT_STATUS.md`.
