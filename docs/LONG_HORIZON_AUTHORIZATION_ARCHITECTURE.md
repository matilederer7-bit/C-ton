...
## 3. What is stored (migration 069)
...
|---|---|---|
| two workers renew concurrently | one `reauthorize` identity per participant (069 guard + `beginProviderAttempt`), owner lease fencing, same-identity re-send only | R5 |
