...
Grounding baseline: worktree `claude/release-readiness-night` on top of `origin/master` `82c91d6`. Render services: `siton-staging-web` (web) and `siton-staging-worker` (worker), both built from the same `Dockerfile`, both `autoDeploy: true` from `branch: master` (`render.yaml:15-23`, `render.yaml:71-79`).
...
2. Run the canonical preflight. Profiles (`config/release-preflight-gates.json`): `static` (no DB/Docker), `standard` (static + local DB gates + Docker lab when available), `full` (standard + payment and security test groups).
   ...
   REAL_MONEY: BLOCKED
     ...
     - FINANCIAL_BRANCH_NOT_INTEGRATED
   RELEASE_PREFLIGHT_RESULT overall=PASS report=.release-artifacts/release-preflight.md logs=.release-artifacts/preflight/
   RELEASE_PREFLIGHT_PASS
   ```
   `SKIPPED_ENVIRONMENT` is never PASS; `WARNING` needs a human decision; exit code is non-zero only on FAIL.
...
- Manifest: `scripts/migration_manifest.cjs` is an append-only ordered list; position = array index + 1 (`:67`). 063/064 are reserved by the financial branch; master holds 065/066; the financial branch appends 067/068 after them (its manifest comment, `git show claude/review-r9c-financial:scripts/migration_manifest.cjs`).
