...
| J5 window | "J5 transactions have a timeframe of up to 7 days" — a property of the AUTHORIZATION INSTRUMENT, not a bound on Siton deals (LONG_HORIZON_DEALS, 2026-09-16: deal lifetime is independent of authorization lifetime; the window is recorded as the binding's `expires_at` and a lapsed hold is renewed at the charging boundary once a Grow stored-instrument re-authorization is proven — `docs/LONG_HORIZON_AUTHORIZATION_ARCHITECTURE.md` §7). "If no J4 transaction is made, J5 will automatically release after 10 days." Manual cancel = "J4 transaction and an immediate refund" (moves real money — NOT automated by Siton). |
