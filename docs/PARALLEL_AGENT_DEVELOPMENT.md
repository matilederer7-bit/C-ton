...
|---|---|---|
| `PROJECT_STATUS.md` | 6,200 lines; every branch prepends its status at the top → the top of the file conflicts on every cross-merge (probe: `git merge-tree` of `claude/review-r9c-financial` × `claude/ux-product-polish-round2` conflicts **only** on this file) | Each branch writes under its **own `## <branch> — <date>` heading**; never edit another branch's section; never reflow older text. Resolve by keeping both sections in date order. |
| `scripts/migration_manifest.cjs` + `src/migrations/NNN_*.sql` | one ordered array; position = array index + 1 (`:67`); the runner and `/readiness` refuse gaps, renames, or a changed file (`scripts/run_migrations.cjs:79-85`) | **Append-only.** Reserve an id range in your status section before writing SQL. Current map: 060/061 used by master; 063/064 reserved by the financial candidate; 065/066 landed on master; the financial branch renumbered its two files to 067/068 because master landed first (`git show claude/review-r9c-financial:scripts/migration_manifest.cjs` comment). **Renumber rule:** whichever branch merges second takes the next free ids AFTER everything already on master, keeps its files unchanged otherwise, and re-runs `npm run migrations:preflight` (upgrade-from-`origin/master` + schema-drift scenarios). Never renumber a file that any ledger (staging included) already holds. |
...
Order is fixed by risk and by who renumbers whom:

1. **Financial branch first** (`claude/review-r9c-financial`, migrations 067/068, `src/app.ts` + `src/frontend_runtime.ts` + `scripts/migration_manifest.cjs`). It carries the money invariants (settlement horizon, dispatch lifecycle) and the migration ids; everything else must rebase onto its manifest. Merge gate: financial reviewer verdict + `backend-quality-gates` + `web-runtime-depth` green; `config/real-money-release-policy.json` stays BLOCKED (`FINANCIAL_BRANCH_NOT_INTEGRATED` is cleared by this merge, the other three reasons are not).
2. **UX branches second** (`claude/ux-product-polish-round2` = `codex/ux-premerge-hardening-night` at `9ce6df9`, then any later UX work). No migrations; touches `src/frontend_runtime.ts`, `web/`, `package.json`. Rebase onto the merged financial master; resolve `PROJECT_STATUS.md` by section.
3. **Release engineering last** (`claude/release-readiness-night`): tooling, workflows, docs, `config/release-preflight-gates.json`. Rebase onto the result so `release:preflight`, `migrations:preflight` and the Docker lab run against the real merged manifest (this is the last OPEN item in `scripts/release_checklist.cjs` `OPEN_ITEMS`).
4. Merge-probe before each step (read-only, no checkout):
   ...
   ```
   Measured on 2026-09-14: master × each of the three branches merges clean; financial × UX conflicts on `PROJECT_STATUS.md` only; financial × release-night on `.gitignore` only; UX × release-night clean.
...
| 062 | — | codex/amazon-benchmark-upgrade | reserved (`PROJECT_STATUS.md:183,260`) | no file exists on any branch; OPEN whether the reservation still stands — ask before reuse |
| 063-064 | — | financial candidate (superseded by 067/068) | released | never applied anywhere (`scripts/migration_manifest.cjs:62-64`) |
...
| 067 | 067_payment_operation_lifecycle.sql | claude/review-r9c-financial | on branch | applied only on disposable DBs |
| 068 | 068_payment_settlement_horizon.sql | claude/review-r9c-financial | on branch | same |
| 069+ | — | next free after the financial merge | free | the UX plan branch proposes 069 for hero-video content (`claude/backend-sensitive-ux-plan`) |

Keep this table current in the reserving agent's status section; the manifest comment (`scripts/migration_manifest.cjs:62-64`) is the second copy and must agree.
...
5. Every cross-branch merge probe conflicts on `PROJECT_STATUS.md` and nothing else (or one line of `.gitignore`) — the "own section, append-only" rule.
