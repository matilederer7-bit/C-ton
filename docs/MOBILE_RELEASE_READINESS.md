...
> readiness tests (`npm run test:mobile-readiness`), the mobile CI workflow and these inventories.
> Port evidence on Linux (Node 22): `mobile:build` → `cap sync` (android + ios) → normalize →
> `MOBILE_GATE_PASS` → `MOBILE_TESTS_PASS negative_controls=12`, legacy `mobile_readiness_validation`
> PASS. Everything under "EXTERNAL" below is still an external blocker: signing keys, store
> accounts, Apple/Google credentials, domain association hosting and device proof were NOT
> supplied and the `.invalid` placeholders are deliberately rejected by `--release`.
