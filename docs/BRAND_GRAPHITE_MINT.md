...
| `--brand-mint` | `#2dd4bf` | the logo dash, highlights, selected interactions, positive progress in motion, live dots, section-marker bars, focus ring |
| `--brand-mint-ink` | `#115e59` | mint as **text** (mint itself is not readable on white): links, prices, active tabs, "collecting" pills |
...
| `--ink` / `--ink-soft` / `--ink-faint` | `#0f172a` / `#334155` / `#475569` | text ramp (all ≥ 7:1 on every ground) |
| red | `#c8261a` (`--pomegranate`) | real errors and danger only |
| blue | `#1d5fd0` (`--sky`) | informational / charging |

Rules: components reference tokens, never a brand hex. Do not add brand colours. Semantic colours (green / amber / red / blue) are never decoration.
...
`C-ton`: the C and "ton" in graphite; between them a **short horizontal mint dash** (never a dot), a little thinner than the letter stroke, with a very soft mint glow (a blurred copy of the dash at low opacity — not neon).
...
- Buttons: primary = graphite fill, white text; hover graphite-hi; active graphite-deep; focus = 3 px mint ring; disabled = `--bg-deep` fill with `--ink-faint` text, no lift; loading = the disabled state while the request is in flight.
