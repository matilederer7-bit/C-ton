...
| `migrations:repair` | yes (`--yes` to apply) | `--allow-hosted --yes`; never run against staging or production from an agent session — owner console action only, after `migrations:doctor --allow-hosted` (read-only) shows exactly the rows to touch (`docs/MIGRATION_SAFETY_SYSTEM.md`) |
...
| `extra_in_database` | a ledger row the repository does not know | BLOCKING; usually a not-yet-merged branch migration applied to the wrong database (the way an unmerged `067`/`068` would have looked on a master database before PR #9 landed them). Do not delete; align the deploy branch |
...
STOP CONDITIONS: (a) doctor `BLOCKED`; (b) `audit_log` newer than `outbox_events` (partial restore across tables); (c) the restore was taken from a database whose ledger is ahead of the deployed code's manifest (a branch migration applied before its code merged), or vice versa.
