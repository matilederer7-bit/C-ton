...
- OPEN: **The money fix is not verified by a hosted HTTP request.** It is verified against a running server locally (failing before, passing after), by CI, and the deployed SHA matches master — but neither hosted path could exercise it: staging runs `internal-runtime` and needs a seller session that was not provisioned, and `siton-demo-preview` **has been failing to boot independently of this work** (its own production guard refuses `APP_DEPLOYMENT_MODE=demo-preview` on a hosted deployment, and its database `dpg-d77p6boule4c73denbjg-a` no longer resolves). Owner action. The plural singulars are likewise not observable on staging: there are no live deals and no single-unit participants, so no count-bearing copy renders. `min_units: -5` still clamps silently to 1 — long-standing behaviour the create defaults rely on, so changing it is a product decision. Sections **not** attacked in this pass, claiming nothing: workers/outbox beyond existing coverage, migrations, Render, Supabase, general security. Carried over: no approved English legal text (owner action), `OTP_HASH_SALT` absent from both Render services, the unpublished `deal_page` CMS draft, Sentry holding zero projects, the external link-viewer journey unproven end to end.
- PERCENTAGE: Bilingual layer 100%, deployed and verified hosted. Red team: money-input boundary 100% (found, fixed, merged, deployed — hosted request outstanding); database runtime-role boundary 100% (measured, no product defect, gate added); cross-role escalation 100% (probed, clean); bilingual attack 100%; concurrency reviewed and sound. **§2.9–§2.14 were not reached in this pass and are listed openly rather than assumed clean — call it roughly half the declared attack surface covered.**
- NEXT STEP: Owner to decide on the two blocked hosted verifications (repair or retire `siton-demo-preview`; or provision a synthetic staging seller so money-input guards can be exercised over HTTP). Then §2.9–§2.14: workers/outbox, migrations, Render, Supabase and general security.
