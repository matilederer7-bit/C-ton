## AUTHENTICATED UI ACCEPTANCE HARNESS — READY, BLOCKED ONLY ON CREDENTIALS (2026-09-14) — branch `claude/authenticated-ui-acceptance-harness` from exact master `a5e60cf`, NOT merged

- **COMPLETED:** A reusable authenticated browser acceptance harness for staging: `scripts/authenticated_ui_acceptance.cjs` (headless Edge/Chrome over CDP, no third-party driver) drives the deployed React app through the real auth panels — credentials are typed into `AuthPanel` / `AdminStepUp` and go only to the canonical Supabase password grant; no user is created, Supabase is untouched, nothing is stored in Git, no token is invented. Credentials come ONLY from `SITON_ACCEPTANCE_SELLER_EMAIL/PASSWORD` and `SITON_ACCEPTANCE_ADMIN_EMAIL/PASSWORD` (no `.env` loading, no file reads; values never printed). **Seller flow:** S0 unauthenticated boundary (auth panel only, `/api/seller/deals` 401, RTL + no overflow at 1440/390) → S1 login → S2 dashboard (+ seller API 200) → S3 profile forms → S4–S6 edit the public "about" field, save, reload, verify persistence, **restore the original and re-verify** → S7 another seller's deal refused at the API (`/api/seller/deals/:id` + `/fulfillment` 401/403/404) and in the UI (refusal state, no deal controls; fixture from env or public-catalog discovery) → S8 other viewport → S9 console/network hygiene. **Admin/CMS flow:** A0 unauthenticated denial (step-up gate, admin API 401, forged unlock marker without a session yields no shell) → A1 seller identity refused at the step-up and by the admin API → A2 admin login → A3 CMS loads → A4–A6 edit `footer.text`, save, reload, verify in the CMS form AND public `/api/site-content` → A7 revision conflict (concurrent write with the form's revision — which also restores the original — then the stale form save is refused 409 with the Hebrew conflict message; restoration public-verified) → A8 restore/revert **NOT_SUPPORTED** on master (server keeps `previous_value_jsonb`, no route/UI) — reported as such, never PASS → A9 other viewport → A10 hygiene. Every step captures uncaught exceptions, `console.error/warn`, failed loads and unexpected ≥400 responses (`diagnostics.jsonl`), screenshots per step, `report.json`; the throwaway browser profile is deleted and storage cleared on exit. `--viewport=desktop|mobile` selects the primary width (1440 / 390) and the other width is re-checked. **Boundary:** `--credentials-check` / `--plan` (no browser, no network) and the full run all report `CREDENTIALS_MISSING` (exit 3) with the missing variables named and every authenticated step as `SKIP … CREDENTIALS_MISSING(surface)`; `verdict=PASS` only when every step ran and passed. **Local dry-fit:** `--local-dryfit` (refuses any non-loopback host or any server with Supabase configured) drives every post-login step against a local demo-preview server with the same non-secret local demo session the repo's other local proofs seed (`scripts/authenticated_ui_dryfit_seed.ts` mints the foreign-seller draft + a throwaway local admin cookie session); login steps report `DRYFIT`, verdict `DRYFIT_PASS/FAIL`, never acceptance PASS. Docs: `docs/AUTHENTICATED_UI_ACCEPTANCE.md`; npm: `acceptance:ui`, `acceptance:ui:check`, `acceptance:ui:plan`, `acceptance:ui:dryfit-seed`, `test:authenticated-ui-harness`. No product, payment, Grow, financial, migration or Supabase change; R9C and Codex branches untouched.
- **TESTED:** (1) Hosted staging (`a5e60cf` live) without credentials, real browser: **S0 PASS, A0 PASS, S9/A10 hygiene PASS** (29 + 18 requests, 0 unexpected failures; the two 401s are the asserted denials), 16 authenticated steps **SKIP CREDENTIALS_MISSING**, A8 NOT_SUPPORTED, `verdict=CREDENTIALS_MISSING`, exit 3 — no PASS claimed for any authenticated step. (2) Credential-boundary self-test `tests/authenticated_ui_acceptance_harness_validation.ts` **6/6** (missing → exit 3 + all four variables named; plan dispositions; seller-only → admin still SKIP; credential values never echoed; non-loopback dry-fit refused; no dotenv/file reads, profile deletion + storage clearing + exception/network capture pinned). (3) Local dry-fit against master's own bundle (local `web/` build reproduces the live hash `index-DwQhN0Dn.js`) on a fresh migrated DB: **DRYFIT_PASS 17/17** at desktop-primary and **17/17** at mobile-primary — S2–S9 and A3–A10 all executed for real (save/reload persistence, foreign-deal 404 + UI refusal, 409 conflict with restoration, 390/1440 no overflow, 0 unexpected console/network failures); DB checked afterwards: no run marker left in the profile or footer. The dry-fit caught and fixed one harness race (S2 asserted before the dashboard loader finished at 390). Dry-fit refusal of the staging host verified. `tsc -p tsconfig.test.json` PASS.
- **OPEN:** the two staging Supabase logins (one approved seller, one admin) — the only blocker; S1–S8 and A1–A9 on staging are unproven until they exist. A8 restore/revert has no product support (documented, not a harness gap). A1 needs the seller login in addition to the admin login. The public-catalog foreign-deal discovery needs at least one published deal by another seller on staging (else set `SITON_ACCEPTANCE_FOREIGN_DEAL_ID`).
- **PERCENTAGE:** AUTHENTICATED UI ACCEPTANCE HARNESS 100 % (ready, self-validated locally, fails safely at the boundary); AUTHENTICATED STAGING UI ACCEPTANCE 0 % proven (blocked on credentials; STAGING ACCEPTANCE overall unchanged ≈ 85 %); REAL MONEY READINESS 0 % / NOT COMPLETE — unchanged, untouched.
- **NEXT STEP:** owner sets the four `SITON_ACCEPTANCE_*` variables in a shell (never in Git) and runs `npm run acceptance:ui` (then `-- --viewport=mobile`); attach `report.json` + `diagnostics.jsonl` + screenshots to the acceptance record; any FAIL is a real product or harness defect to triage.
