...
1. **Guest / pilot screen** — the closed-pilot disclosure keeps its text and position; the decorative glyph beside it is gone and **nothing replaced it** (the block keeps its own spacing, so the section still reads balanced). The hero image stays: it is the CMS-editable primary medium item 7B is about extending.
2. **Required-field validation** — new `web/src/fieldAttention.ts`: one rule, no global hack. A failed continue marks **the exact control that failed** with a restrained two-second border breath (three cycles, then rest — never a strobe), `aria-invalid="true"`, and scroll-to-centre + focus; `prefers-reduced-motion` swaps the animation for a static ring so nothing rides on motion alone. `settleErrors()` only ever **removes** errors while the user works, so a text field clears on the first valid character, a select / number / option group clears the moment its value becomes valid, and typing in one field never lights up another. Wired into the create wizard (22 fields + the images block + the delivery notice + the receipt fieldset) and the draft edit form, whose validation was extracted into one `validateEdit()` so live and submit rules cannot drift.
3. **Decorative glyphs off headings** — 37 removed across the seller flow, deal page, tracking, pickup, inquiries and admin (every `panel-title` / `section-title` / `h1..h3` / `legend`), including the four the owner named and the buyer's delivery-type glyphs. Functional icons on **actions** (buttons, share glyphs, status pills, empty states) deliberately kept.
4. **Siton selection controls** — new `ChoiceCard` + `.choice-card`: neutral unselected; selected = orange border, orange tint and an indicator whose **inside fills with the canonical Siton orange**. The indicator **shape carries the arity** — round when exactly one option may be active, square check when several may be — and the native input stays in the accessibility tree, only visually replaced. Applied to "איך הקונה יקבל את מה ששילם עליו", the buyer's delivery choice and the buyer's public-name opt-in (the real multi-select case).
5. **Share card promoted + social buttons standardised** — "מכירים מישהו שזה יעניין אותו?" now renders directly under the decision block and **above "איך זה עובד"** (proved by DOM order *and* measured page position at every width). All five social buttons are one identical **white disc** with the network glyph on top (the previous X implementation generalised); the per-network coloured discs are gone and size, radius, glyph colour, hover and focus are one rule.
6. **Virality label** — "צמיחה וויראליות" → "ויראליות" in both user-facing places; historical documentation untouched.

**EXISTING FEATURES EXPOSED / FIXED (not new models).**
- **Seller public profile (item 5).** The data was already canonical — migration 066 added `seller_accounts.public_profile_id` + `profile_image_id`, and `publicSeller()` already returns name / about / image / stats / deals. Two things were missing and are now fixed: (a) `buildPublicDealPayload` (`src/frontend_runtime.ts`) did not project the profile id or the logo, so the deal page could not render them — the SELECT and the `seller` block now also carry `profile_id` and `image` (**additive read projection from the same row; no second model, no new storage, no write path, no state machine**); (b) the frontend did not render them — the deal page's seller panel now shows the logo, display name, approval badge, About text and a link to the full public profile, and `SellerIdentity` / `PublicSellerPage` were restyled into the Siton design language. Still never exposed: e-mail, phone, address, bank, payment identifiers, internal `seller_id`, raw verification status, buyer information; contact remains the internal inquiry rail only.
- **Legal pages (item 9).** Master already serves the canonical `src/legal_pages.ts` content through the CMS at `#/content/legal_*`, so the old branch's parallel `#/legal/*` renderer was deliberately **not** ported. The *shell* was brought up to standard instead: `.content-doc` (orange section markers, measured line length, RTL-safe wrapping) plus a chip strip between the legal documents.

**OLD UX WORK PORTED SEMANTICALLY** from `claude/launch-ux-cleanup` (`682b93c`, read as reference only — not merged, not cherry-picked): typed quantities (`web/src/quantityInput.ts` + `QtyInput` — digits only, numeric keyboard, **no `+/−` stepper and no browser spinner**, replacing `QtyStepper` on the buyer join); true back/forward scroll restoration **per history entry** (`web/src/scrollRestoration.ts`, replacing the unconditional scroll-to-top; a new entry still starts at the top); deletion of the orphaned buyer viral-tree canvas `web/src/vtree.tsx` (281 lines — nothing on master imported it; the buyer's tracking page already shows share identity + impact only). Deliberately **not** ported: the old legal route (superseded, above), `navIcons`/pickup navigation, buyer-search intent and growth windows (out of scope or already solved differently), and the old branch's `.impact-stats` CSS removal (master's tracking page still uses those classes).

**BACKEND-SENSITIVE ITEMS DELIBERATELY LEFT OPEN** — documented, not faked, each with safe frontend groundwork so the UI needs no rework when the backend lands (full detail in `docs/UX_PRODUCT_POLISH_ROUND_2.md`):
- **Multiple receipt / fulfillment methods per deal.** `siton.deals.receipt_config` holds exactly ONE `method` and `validateReceiptConfig()` rejects anything else, so the UI keeps single-select radio semantics and the **round** indicator — a frontend-only multi-select would show the seller choices the database cannot store. `ChoiceCard` already supports `mode="many"` (square, checkbox semantics, identical orange fill) and is browser-proven on the real multi-select control; wiring it is one line once the contract exists. Closing it needs a migration to an ordered method collection, collection validation, a per-method entitlement/redemption path and a backfill.
- **CMS hero media as IMAGE or VIDEO.** Blocked twice: `content_assets.mime_type` has `CHECK (mime_type IN ('image/png','image/jpeg','image/webp'))`, so a video upload is rejected **at the database**; and `validateContent()` has no notion of a medium *kind*, so "which of the two is active" cannot be stored. **Real defect fixed on the frontend meanwhile:** a configured background video used to render *behind* the configured hero image — `web/src/heroMedium.ts` is now the single decision point and returns exactly ONE medium, so both can never appear. Closing it needs a migration adding video MIME types (plus size/duration limits), a `kind` on the hero field, `validateContent` support and an admin picker.
- **CMS-managed FAQ.** `siton.site_content` stores one JSONB object per key and `validateContent()` accepts **only** a flat `Record<string, string>` of declared fields, so an ordered array of `{q, a}` pairs is rejected before it reaches the database. `web/src/faqContent.ts` is now the ONE place the landing reads its FAQ from and already accepts the eventual ordered shape, a bare array, and flat numbered pairs, falling back to the canonical Hebrew list so a malformed payload can never blank the section. Closing it needs a list-valued content contract (validation, caps, ordering, revisioning) and an add / edit / remove / reorder editor.
- **Virality date filters.** Needed new backend modules, `viral_graph` changes and a `/api/admin/growth` that accepts `?days=` / `?from=&to=` / `?range=all`; master's route takes no parameters, so the filter is **not** frontend-compatible. Left open rather than faking a client-side filter over data the server did not window.
- **Scope note:** the buyer-facing deal page carries the same pilot glyph pattern (`pilot-line`, `pay-pilot-note`), asserted verbatim by `tests/frontend_foundation_buyer_polish_validation.ts`. That is not the guest screen, so it was left alone rather than silently widening item 1 — a one-line follow-up if the owner wants it everywhere.

**TESTED.** New `scripts/ux_polish_round2_browser_proof.cjs` (`npm run proof:ux-round2`) — **85/85 PASS at 390 / 430 / 1280 / 1440**, driving the *real* React components in headless Edge over CDP against deterministic fixtures: the pilot text without its glyph and with nothing replacing it; exactly one hero medium; the marked control after a failed continue (pulsing border + `aria-invalid` + scrolled into view + focused) and its clearing on the first valid character **with the other missing controls still marked**; zero decorative glyphs on headings; the orange-filled indicator on selection (round single / square multi) with neutral unselected and the fill following the choice; the share block above "איך זה עובד"; five identical white social discs (equal size, radius, glyph colour); the seller logo / name / About / stats with **no contact leak**; the Siton legal shell; typed quantities with no stepper or spinner; RTL; **zero horizontal overflow; zero console errors**. Regression, one runner at a time on fresh local databases: unit **15/15**, api **44/44**, security **39/39**, e2e **13/13**, integration **31/31** (the single first-pass failure was the fresh-worktree absence of `.mobile_dist`, an environment artefact — `npm run mobile:verify` built it and the file re-ran green), db **8/8**, workers **13/13**, payments **29/29**, concurrency **7/7**, failure **9/9**. TypeScript clean on all three projects (web, backend, tests); `mobile:verify` PASS. One pre-existing test was **strengthened, not weakened**: `buyer_feedback_support_operations_validation` assertion 8 keeps its strict key allow-list (a future key must be added deliberately) and gained explicit negative proofs — the projected id is the public profile UUID and **not** the internal `seller_id`, and the block carries no contact/bank detail and no raw status. Real money 0; Grow calls 0; migrations added 0; financial files changed 0.

**PERCENTAGE:** requested UX round-2 scope **100 % of the frontend-implementable work** implemented and browser-proven; the four backend-sensitive items above are **0 %** by instruction (documented and left OPEN for after the financial merge). CLOSED WEB PILOT readiness unchanged by this branch (UX only). REAL MONEY readiness **0 %** — untouched and separate.

**NEXT STEP:** independent review of this branch, then merge **after** the frozen financial review resolves; the four backend-sensitive items are then a single follow-up sprint (receipt-method collection, hero media kind + video MIME, list-valued FAQ content, windowed virality) — each already has its frontend groundwork in place.
