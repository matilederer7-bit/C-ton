## R9C ROUND 7 — OBSERVER INTEGRITY HARDENING (ORACLE / LAB ONLY) — READY FOR CODEX RE-REVIEW (2026-09-14)

Branch `claude/review-r9c-financial`, NOT merged, NOT deployed. Master `82c91d6` unmoved (verified by `git ls-remote` at the start and at the end). Frozen review SHA at start `9df33bf`; new review SHA = the branch tip carrying this entry. Report: `docs/R9C_ROUND7_OBSERVER_INTEGRITY.md`. Codex's round-6 re-review (`.tmp_r9c_round6_independent_report.md`, read only) confirmed the round-6 transport bug CLOSED, the orphan blocker CLOSED and F12 PASS, and found three P1 defects in the PROOF OBSERVER (`tests/lab/siton_observer.ts`) — not in the oracle rule, not in production: (A) query ids restarted with the process (`WORKER_ID:q1` twice → a held answer borrowed an older receipt), (B) false `verdict_recorded` (0-row UPDATE, COMMIT answering ROLLBACK, note parameter as identity), (C) `status_received` stamped after `text()` before `JSON.parse`.

COMPLETED — REPRODUCTION FIRST. Codex's own scripts (copied with absolute requires into `.tmp_r7/`, worktrees untouched) run against the unchanged 9df33bf: independent controls 12 PASS / 6 FAIL (`RESTART_IDS worker-restart:q1 × 2`, `BEFORE_JSON_PARSE [status_received]`, `ZERO_ROW rowCount 0 + verdict`, `ABORTED_COMMIT command ROLLBACK + verdict`, `WRONG_IDENTITY [VICTIM, OTHER]`, synthetic reused-id accepted) and the real restart + held-transport counterexample exit 1 (`received=false`, both queries `stable-worker-id:q1`, evidence `received_seq=7` for an answer created at 9, `violations=[]`). All five reproduction flags YES.

COMPLETED — OBSERVER CONTRACT + FIXES (lab only, `git diff 9df33bf -- src/` empty). QUERY_ID = `<label>:<instance-uuid>:q<n>` (UUID minted per observer incarnation, monotonic counter; unique across lifetime/restart/worker/concurrency; the simulator echoes it as `x-siton-lab-query-id`). STATUS_RECEIVED only inside the app's own `JSON.parse` of the exact `text()` string the observer handed over, after echo match + 2xx + provider-shape validation; parse failure / invalid shape / mismatch / unparsed body / dead incarnation record nothing; one body → one receipt. VERDICT_RECORDED only for a recognised terminal write (`UPDATE siton.payment_attempts` assigning `result_class` success/permanent_fail, identity = the `correlation_id=$n` WHERE binding or RETURNING rows — exact SQL shape, never parameter guessing), rowCount exactly 1, STAGED in the transaction (savepoint-aware) and PUBLISHED only when the COMMIT answered command tag `COMMIT`; ROLLBACK / ROLLBACK TO / rejected COMMIT / aborted COMMIT discard; the CASE-computed class (`settleProviderDispatch`) is READ BACK from the locked row in the same transaction. Emissions of one incarnation are serialised (HTTP worker seam keeps order). Oracle defence in depth (`dispatch_legality.ts`): a receipt binds only when its query id names exactly one provider request, the answer was written, and the receipt follows the query's arrival. Round-6 T1 hop now forwards the echo. Observer handle exposes `instance`, `stats()`, `drain()`.

TESTED — targeted, one runner at a time on fresh databases (`.tmp_r7/`): new `review_observer_integrity_validation.ts` O1–O17 17/17 (real child processes for restart / simultaneous workers, real hops, real PostgreSQL with the production statement texts read verbatim from `src/payment_attempt_helpers.ts`, Codex's chronology with REAL committed verdicts → REJECT and waited twin → ACCEPT, static conformance of all 6 production statements); observer mutants OM-A..OM-K 11/11 killed by that suite alone (0 compilation-invalid); Codex's independent controls 18/18 and restart-transport script exit 0 on the hardened tree; round-6 oracle mutants OM-1b…OM-10 10/10 (OM-4/OM-5 re-anchored on the hardened receipt lookup); causal 14/14, observation 17/17, temporal 18/18; seed 209752203 300/300 (trace: 61 legal repeats, 32 status evidences all response-bound, 29 exact declines, 0 unbound, 0 illegal) and fresh seed 480387699 300/300; F12 12/12 + 4/4; orphan-authorization 1/1; two-worker concurrency matrix 32/32 with the hardened worker preload. Corrective reruns recorded in the report: O1/O2 child processes first hit the Windows libuv teardown abort (700 ms drain added); the first oracle receipt-position rule (`received > delivered_seq`) rejected Codex's synthetic out-of-order fixture (stale builder `delivered_seq`) and was relaxed to `received > seq` + "written at all"; one unintended partial PRODUCTION-mutant run (RM-1/RM-2 killed) was stopped during RM-3 and `src/app.ts` restored with `git checkout` (src diff verified empty). Full 246-file suite deliberately NOT rerun (task §10).

PRODUCTION AUDIT (task §11): ORACLE_OBSERVER_ONLY = YES. Production query ids are outbox-UUID based (`x-request-id` = correlation id / `reconcile:<event>` / `recovery_preflight:<event>:<pid>:<n>`), every terminal write runs inside `withTx` with `rowCount` checked by the helpers, and `parseJsonSafely` parses only after `text()` and returns `raw_body` → state unknown on malformed answers. No production source changed; no new production P0/P1.

OPEN / OUT OF SCOPE (untouched by instruction): F-13 provider-contract issue — REAL_MONEY_BLOCKER = YES; KYC policy mismatch; unpaid pickup-card UX; mobile CTA placement; stale P0 copy assertions; scanner defects Codex classified; Grow.

PERCENTAGE: R9C round-7 scope 100 % (all three Codex observer findings reproduced, root-caused, fixed under an explicit observer contract, proven by an independent integrity suite that kills its own mutants, and validated on the real rails). Real money 0. Grow never called. No migration. No deploy. Not merged.

NEXT STEP: CODEX INDEPENDENT RE-REVIEW OF THE OBSERVER + ORACLE, THEN PR CI INCLUDING THE DOCKER GATES.

## R9C ROUND 6 — STATUS-RESPONSE CAUSAL BINDING (TARGETED ORACLE FIX) — READY FOR CODEX RE-REVIEW (2026-09-13)

Branch `claude/review-r9c-financial`, NOT merged, NOT deployed. Master `82c91d6` unmoved (verified by `git ls-remote` at the start and at the end). Frozen review SHA at start `43e4dfc`; new review SHA = the branch tip carrying this entry. Report: `docs/R9C_ROUND6_STATUS_RESPONSE_CAUSAL_BINDING.md`. Codex's final merge gate (`.tmp_r9c_final_merge_gate_20260913.md`, read only) closed the orphan-authorization blocker and left one: the round-5 oracle could still legalise a retry on a status answer that had not arrived at Siton.

COMPLETED — REPRODUCTION FIRST. Codex's own `independent_transport_window_control.cjs` run read-only against the unchanged 43e4dfc: exit 1, `FULL_ORACLE_VIOLATIONS []` while `status_received_at_second_dispatch=false` (`.tmp_r6/r6-codex-transport-vs-43e4dfc.log`). Chronology: pending capture → pre-horizon status read received (Siton recorded permanent_fail — the "old resolved_at") → horizon elapses → post-horizon status answer WRITTEN by the provider but HELD by a transport hop → second capture sent inside the hold (money moved) → answer delivered. Root cause, two coupled defects: observation was identified with the provider's write (`delivered_seq`, blind to a transport hold), and the attempt-level `resolved_at <= dispatched_at` clause was satisfied by the verdict of an earlier, unrelated read — provider answer existence + some prior identity-level resolution timestamp were taken as proof that Siton observed THIS response.

COMPLETED — PRODUCTION AUDIT: ORACLE_ONLY_DEFECT = YES. `payment_attempts.resolved_at` is never read by any production payment path; every rail awaits its status answers before deciding, records the verdict before minting the next identity, and the recovery rail takes two awaited, uniquely-identified pre-flight reads before minting/arming/sending. Production never authorises a retry from a row timestamp or from a response it has not received. NO production source changed (`git diff 43e4dfc -- src/` is empty).

COMPLETED — CAUSAL MODEL (lab only). `tests/lab/siton_observer.ts` (new) records what SITON did at its own process boundary, positioned on the simulator's single sequencer: a unique `query_id` stamped on every outgoing status request and echoed by the provider; `status_received` when the app parsed the answer; `dispatch_sent` before a money request leaves; `dispatch_received`; `verdict_recorded` at the COMMIT of a terminal `payment_attempts` write (pg client wrapper), with process and outbox job. Installed by the lab runtime before the app loads and by `siton_observer_preload.ts` inside real worker processes (positions over the simulator's `/lab/observe` seam). `tests/lab/dispatch_legality.ts`: a status answer authorises a repeat only if it was RECEIVED by Siton before the dispatch was SENT, together with a durable verdict COMMITTED before the send and causally SOURCED from a received answer (same process/job, received before committed) — the evidence names its query_id, receipt, verdict and source; exact declines need receipt + committed verdict before the send; callbacks/operator verdicts keep their DB order against the arm. `resolved_at` removed from the oracle (option B). Wall clocks play no part.

TESTED — targeted, one runner at a time on fresh databases (`.tmp_r6/`): Codex's exact script on the fixed oracle → `AUTOMATIC_REPEAT_WHILE_UNKNOWN` (exit 0); new `review_oracle_causal_binding_validation.ts` 14/14 (T1 = real simulator + real observer + transport hold; N1–N9 reject incl. Codex's N2; P1–P4 accept incl. "earlier fully-observed Q1 stays valid while Q2 is pending"); round-5 observation controls 17/17 and round-4 temporal controls 18/18 (re-expressed with observations); oracle mutants OM-1b…OM-10 10/10 killed (M1 = OM-6, M2 = OM-4/OM-5, M3 = OM-8, M4 = OM-9/OM-10; Codex's transport-hold control kills M1/M2/M4); exact seeds 209752203 / 203965851 / 273739535 and fresh 474592646 all 300/300 — in the traced seed all 61 legal repeats carry response-bound evidence (32 status reads naming query/receipt/verdict/source, 29 exact declines), 0 illegal, 0 unbound; F12 targeted 12/12 + 4/4; orphan-authorization regression 1/1; lifecycle crash/race matrix 10/10; F-14/15/16 8/8; two-worker concurrency matrix 32/32 with the observer preload in the workers. Corrective reruns recorded in the report: three new mutants first failed to compile (INVALID, never a kill) and two round-5 controls needed their verdict stated; one mutant run aborted on a moved anchor with a partial mutant in the tree (restored by hand, harness now restores before exiting); one unintended full mutation run (all production mutants still killed). Full 246-file suite deliberately NOT rerun: only oracle/simulator/lab/test code changed (task §10).

OPEN / OUT OF SCOPE (untouched by instruction): F-13 provider-contract issue — REAL_MONEY_BLOCKER = YES; KYC policy mismatch; unpaid pickup-card UX; mobile CTA placement; stale P0 copy assertions; scanner defects Codex classified; Grow.

PERCENTAGE: R9C round-6 scope 100 % (the last Codex blocker reproduced, root-caused, fixed in the lab with response-bound evidence, mutated, and validated on the real rails). Real money 0. Grow never called. No migration. No deploy. Not merged.

NEXT STEP: CODEX INDEPENDENT RE-REVIEW OF THE ORACLE, THEN PR CI INCLUDING THE DOCKER GATES.

## R9C ROUND 5 — OBSERVED-EVIDENCE ORACLE + REAL-HANDLER AUTHORIZATION RACE — READY FOR CODEX INDEPENDENT ROUND-5 REVIEW (2026-09-11)

Branch `claude/review-r9c-financial`, NOT merged, NOT deployed. Master `82c91d6` unmoved (verified by `git ls-remote` at the start and at the end). Frozen review SHA at start `88b6d82`; new review SHA = the branch tip carrying this entry. Report: `docs/R9C_ROUND5_OBSERVED_EVIDENCE_AND_ARM_RACE.md`. Pre-flight recorded (`.tmp_r5/preflight.md`): no stale node/worker processes, zero backends on any disposable database, worktree clean; Codex's worktrees read only, never modified.

COMPLETED — BLOCKER 1, ORACLE OBSERVABILITY. Codex's round-4 finding reproduced in this tree first: the round-4 legality oracle positioned a provider status answer by the provider's arrival log position, so an answer the provider had generated but was still holding was treated as Siton's knowledge and a second capture dispatched inside the hold was judged legal (real-simulator control P1 + synthetic A/C/F/R: 5 of 13 controls FAIL against the round-4 oracle, `.tmp_r5/r5-oracle-observation-vs-shipped.log`; Codex's own `independent_oracle_delivery_control.cjs` now exits 0 against the fixed oracle). Rule fixed as PROVIDER KNOWLEDGE != SITON KNOWLEDGE: the simulator records `delivered_seq`/`delivered_at` from the same monotonic counter at the instant an answer is written back (null when it never leaves), and `tests/lab/dispatch_legality.ts` admits a provider-channel fact only when it was delivered before the dispatch arrived AND, when the ledger can say, Siton recorded the verdict before arming the dispatch (`resolved_at <= dispatched_at`; rows consulted for timing only). Controls: 17/17 observation (incl. lying-ledger layer isolation, non-final reads, same-identity replay) + round-4 temporal 18/18. Oracle mutants OM-4..OM-7 (M1 provider-generated == observed, M2 usable before delivery, M3 later DB state, M4 UNKNOWN as failed) added, OM-3 re-anchored on a layer-isolating control, OM-1a retired (subsumed by the delivery rule): 7/7 killed. Both exact seeds 300/300 under the new oracle (and 300/300 on the round-4 production code, i.e. the real rails never dispatch on undelivered knowledge).

COMPLETED — BLOCKER 2, ORPHAN AUTHORIZATION (PRODUCTION). Reproduced with the REAL handlers (`tests/review_arm_race_orphan_hold_validation.ts`, forced interleaving, `.tmp_r5/r5-arm-race-prefix.log`): finalize fails p2 between the charge handler's state read and its identity mint; the mint lands on a DealFailed participant, the arm refuses, and the never-dispatched `unknown/recorded` identity (0 provider requests) blocks the F-16 release (`payment-operation-blocked` case, job consumed); the reconcile it triggers status-reads an identity the provider never saw and records `permanent_fail/status_inference/responded` with no dispatch instant — an infinite 068 fence; the hold stays `ChargeAttempt` for ever (A, B, C, D of the brief all proven). Exact cause: the mint never re-read the participant's state under the lock (pre-existing 063 semantics; F-8 had patched the recovery instance locally), and `beginProviderAttempt` + the 067 INSERT guard treated any capture-side `unknown` row as unresolved regardless of `dispatch_state`; F-16 exposed it by making the release reachable; F-14/F-15 neither caused nor masked it, nothing of round 4 is reverted. Fix inside 067/068, NO migration: `beginProviderAttempt({ admitted })` re-reads the participant under the participant/deal lock and mints nothing when the rail's admitted state is gone (every rail passes its arm states); never-dispatched identities of a conflicting operation are retired in place as ABANDONED_BEFORE_DISPATCH (`temporary_fail` + `recorded` + `dispatched_at NULL` + `never_dispatched:<reason>` — the row stays, never claims a response, and can never be armed: the arm CAS requires `unknown`, DB-enforced) atomically before the superseding identity is minted; reconcile never status-reads a never-dispatched identity (live job owns it / operator case naming the job to requeue / retire and queue the release); the F-15 guard keeps its round-4 contract (a minted identity still defers) but no longer defers on a retired one. Regression 1/1 (round-4 code 0/1); crash/race matrix `tests/review_attempt_lifecycle_crash_race_matrix_validation.ts` 10/10 on real handlers — crash before dispatch (same identity dispatched once), job gone + phase over (retired, released), finalize before dispatch (no orphan, late job sends nothing), real handler frozen between mint and arm (capture lands, buyer completed), release vs created-not-sent capture (retired, released, stale arm refused by the DB), capture vs release 6 races (exactly one effect), lost response (UNKNOWN, no release), never-dispatched via the real sweeper window (no status read, no fence), worker dies after arm (reclaim, one effect), triple claim (one dispatch); round-4 code 5/9 of the original nine. Production mutants RM-10..RM-13 killed.

TESTED — clean sequential runs, one runner at a time on fresh local databases, logs under `.tmp_r5/` (not committed): F-14/15/16 8/8; F-12 durable escalation 12/12 (case-write failure, identity-read failure, same-event retry, exactly-one escalation); F-12 dual detect 4/4; P0 foreign reference PASS; identity rails 10/10; arm-race 1/1; matrix 10/10; oracle observation 17/17; temporal 18/18; exact seeds 209752203 and 203965851 300/300 each; fresh seed 273739535 300/300; groups payments 61/61, concurrency 8/8 (two-worker matrix included), workers 13/13 (two-process fencing included), security 39/39; DB fencing proof 21/21; migration proofs 49/49 + current-master upgrade 59->61 applying only 067/068 with the master ledger unchanged; typecheck, backend enforcement scan, payment compliance scan, runtime-DDL scan, architecture gate PASS; full mutation proof 19/19 (one corrective rerun recorded: RM-10's first edit did not compile and was counted INVALID, never as a kill; rewritten and re-run, then the whole proof re-run clean).

OPEN: F-13 unchanged (refund identity rotation trusts a final status read with no settlement horizon — provider-contract dependent; the observed-evidence rule makes the oracle stricter about such reads, the honest simulator still cannot manifest a lying provider); REAL_MONEY_BLOCKER = YES until the provider guarantee is researched separately. F-2, F-5, F-6, F-10, F-11 untouched by instruction.

PERCENTAGE: R9C financial integration on this branch 100 % of the round-5 scope (both Codex round-4 blockers reproduced, fixed, mutated and A/B-proven; every prior round green). Real money 0. Grow untouched, never called. No migration. No hosted database. No deploy. Not merged.

NEXT STEP: CODEX INDEPENDENT ROUND-5 REVIEW + FINAL MERGE GATE.

## R9C ROUND 4 — ORACLE SOUNDNESS + CONCURRENCY FINAL-STATE FAILURE — READY FOR CODEX INDEPENDENT ROUND-4 REVIEW (2026-09-11)

Branch `claude/review-r9c-financial`, NOT merged, NOT deployed. Master `82c91d6` unmoved (verified by `git ls-remote` at the end). Frozen review SHA at start `ec9cf24`; new review SHA = the branch tip carrying this entry (reported with the push). Report: `docs/R9C_ROUND4_ORACLE_AND_CONCURRENCY.md`. The F12 production fix Codex passed in round 3 is untouched.

COMPLETED — ORACLE SOUNDNESS. Codex's rejection reproduced in this tree with an own reviewer-style control (`tests/review_oracle_temporal_negative_validation.ts`): against the round-3 oracle, a retry dispatched while its predecessor was UNKNOWN is accepted with zero violations when a `late_money_effect` note appears later (Codex's exact history), when a decline appears later, when a reconciliation verdict appears later, and cross-operation for a recovery — 10 of 18 controls failed. Root cause: the repeat rule read the FINAL `result_class` and exempted a note written after the dispatch. The invariant is now written down and enforced by construction in `tests/lab/dispatch_legality.ts`: the legality of a money dispatch is decided ONLY from evidence positioned before it — provider sequence numbers for provider answers and status reads, the DB arm instant (`dispatched_at`, written before I/O) for callbacks and operator verdicts, never the two clocks against each other; evidence classes E1 exact-operation decline, E2 final non-executed status read (capture-side identities at/after the settlement horizon measured from the provider's last non-final answer), E3 operator verdict, E4 provider callback; ambiguous answers (pending, 5xx, 408/425/429, hang/reset/lost/timeout, malformed, non-final or mis-referenced reads) are never failed. Capture-side (capture+recover), refund and release families are judged, plus release-vs-capture and refund-needs-executed-capture. The simulator records what every status read DECLARED (state, finality, delivered, reference and amount checks) so nothing is inferred from application rows; every judgement names the evidence it relied on (`report.dispatch_judgements`), and the fuzz asserts each repeat is legal WITH named pre-dispatch evidence — its note-based assertion is gone. A deliberate scope decision, recorded not hidden: the first temporal rule applied the settlement horizon to refunds and rejected seed index 59 (a refund identity rotated 24 ms after a `captured/final` status read following a 503); the production contract scopes the horizon to capture-side identities (`app.ts` arms refund/release identities with `settlement_horizon_ms` null; migration 068's owner decision names recovery, release and the terminal decision), so the oracle applies the horizon to capture-side targets and the residual is recorded as finding F-13 for the owner.

COMPLETED — CONCURRENCY FINAL-STATE FAILURE. Root cause established from the production source and reproduced deterministically (`tests/review_finalize_participant_race_terminal_state_validation.ts`, 8/8 in three consecutive runs): `finalize_deal` commits `deal → Completed` in its own transaction and then transitions each participant in a SEPARATE transaction from the state it read; the F-2/R-11 gate defers on any unknown or executed-but-unapplied identity, so the only participant it can read as non-final and still proceed is one with no capture identity yet (never attempted — a `charge_deal` re-run after a lost lease has not reached it; threshold 1 is met by the other participant); when that capture lands between the read and the CAS, the CAS conflicts (`State mismatch participant … expected ChargingAttempt`), the job fails and is deferred by backoff, and the two-worker test audited after stopping the very workers that would run the retry. Codex's signature is reproduced exactly (deal Completed, participant `ChargedSuccess/ChargedSuccess`, finalize `pending#1 deferred=true`, oracle reporting exactly `COMPLETED_DEAL_PARTICIPANT_NOT_FINAL` with 2 captures / 2 canonical / 0 unknown), the deferred retry heals it with zero provider interaction and unchanged effects, and the terminal-state invariant is asserted explicitly (effects ↔ identities ↔ money_state ↔ buyer_state ↔ deal state; no failed-while-captured, no success-without-capture, no non-terminal residue; audit chain `ChargingAttempt→ChargedSuccess→DealCompleted`). Classification: B (audit timing) + E (wrong expected terminal state at that instant) on a benign, self-healing conflict — and, from the same analysis, THREE real adjacent defects proven failing-first and fixed in `src/app.ts`: F-15 the other order of the race (a `DealFailed` committed between a capture's arm and its ingest left a charged buyer marked failed with no refund path — pre-fix evidence `finalize=sent ingest=409 p2=DealFailed/ChargeAttempt` with one capture effect) is closed by taking the rails' participant:deal advisory lock inside the fail-participant transition and deferring while any capture-side identity is not `permanent_fail` (a plain read without the lock was tried first and proven insufficient); F-14 the R-11 retry path completed paid participants only, stranding unpaid siblings non-terminal on a Completed deal with holds never released — the whole completed-deal outcome is now one idempotent routine `applyCompletedDealOutcome` used by the fresh path and every retry (idempotency of each step verified in source); F-16 a never-attempted `ChargeAttempt` hold was never released on a decided deal — `ChargeAttempt` admitted by the release scheduler and rail entry/arm, with the `applyAuthorizationRelease` belt and the `beginProviderAttempt` conflict rule unchanged. The two-worker matrix now drains the terminal decision in-process after the workers stop, proves the drain captures nothing new, and only then runs the strict deal-level oracle; it also explains any transient (`TRANSIENT_NON_FINAL` dump of the finalize job, the audit chronology and the worker tails) before asserting. Natural recurrence on an idle host: 0 in 20 isolated runs (each 80/80 converged, 0 duplicates, 0 deadlocks); Codex's two observations came from loaded full-group runs, which the deterministic reconstruction covers.

TESTED — clean runs, one runner at a time on fresh local databases, each log kept under `.tmp_r4/gates/` (not committed): oracle temporal controls 18/18 (against the shipped round-3 oracle the same file was 8/18 — the reproduction); exact Codex seed 209752203 300/300 (474 participants, 541 provider effects; 539 dispatches judged, 60 repeats all legal by named evidence — 28 exact decline, 27 post-horizon status, 5 refund/release status — 0 illegal); fresh seed 259070122 300/300 (435 participants, 504 effects); finalize race reconstruction 8/8 (three consecutive runs); F12 durable escalation + fail-closed identity read 12/12; F12 escalation 4/4; P0 foreign-reference A/B PASS (0 failures); identity rails 10/10; DB fencing 21/21; independent migration proof 49/49; isolated migration proof PASS (61 migrations fresh + repeat, checksum ledger, drift 0, production changes 0); groups payments 61/61, concurrency 8/8, workers 13/13, security 39/39; lint, payment-compliance, runtime-DDL, architecture and route-authorization (static 1 + behavioural 4) gates PASS; TypeScript clean for src and tests; two-worker matrix flake probe 20/20 with 0 transient non-final observations. Anti-vacuity: 4/4 oracle mutants killed, each by the control built for it; production mutants RM-1…RM-9 8/8 killed (RM-1, RM-2, RM-3, RM-4, RM-6, RM-7, RM-8, RM-9; RM-5 retired — its predicate was replaced in round 2 and RM-6 was re-anchored on the round-3 structure), 12/12 with the oracle mutants in one run, tree restored byte-exactly. Seed 209752203 replay of index 152: n2 judged legal on `exact_decline@seq1 (402)`; the late callback plays no part. A slip the gate itself caught, recorded rather than hidden: the first round-4 commit `44b89f4` still carried the first edit of oracle mutant OM-1b because the reviewer harness saved a file's original once per EDIT (a two-edit mutant restored a half-mutated file); control I5 went red in the final gate exactly as it exists to, the guard is restored and the harness now records each original once and verifies the restore byte-exactly (`7ad339f`); every gate above ran on the corrected tree.

OPEN: F-13 (refund identity rotation trusts a final status read with no settlement horizon — contract-consistent, owner decision, the honest simulator cannot manifest it); F-2, F-5, F-6, F-10, F-11 untouched by instruction. Real money 0. Grow untouched, never called. No hosted DB. No deploy. Migrations 066/067/068 unchanged.

PERCENTAGE: R9C financial integration on this branch 100 % of the round-4 scope (oracle soundness proven with negative controls and killed mutants; concurrency failure explained, reproduced, three adjacent defects fixed with failing-first evidence); merge readiness pending Codex's independent round-4 review. REAL MONEY readiness 0 % — unchanged and separate.

NEXT STEP: CODEX INDEPENDENT ROUND-4 REVIEW + FINAL MERGE GATE.

## R9C F-12 ROUND 3 — FAIL-CLOSED IDENTITY READ — READY FOR CODEX RE-REVIEW (2026-09-10)

Branch `claude/review-r9c-financial`, NOT merged, NOT deployed. Master `82c91d6` contained and unmoved. Report: `docs/R9C_FINANCIAL_INDEPENDENT_REVIEW.md`.

COMPLETED: the remaining merge blocker from Codex's re-review of frozen SHA 812dd56 is fixed, and a regression this review had itself introduced is fixed with it. The blocker: the dual-capture identity read was listAttemptLifecycle(...).catch(() => []), so a database error became an empty evidence set, which is spelled exactly like "no other capture succeeded". That collapsed to "not a dual capture", the handler returned early, the provider event was marked ignored (a terminal status for claimEvent) and the delivery was acknowledged with HTTP 200. A read failure therefore became a silent verdict of safety, and the dedupe made it permanent because the same event id could never be processed again. Reproduced deterministically on the frozen SHA before any source change. The read now has three explicit outcomes, confirmed_single, confirmed_dual and unreadable, with unreadable as a VALUE so an evidence-read failure can no longer be spelled the same way as one identity and the caller must handle it; on unreadable it raises PaymentDualCaptureEvidenceUnavailableError BEFORE any write, so financial truth is untouched, the delivery is not acknowledged, and the event is left failed for the canonical retry path. The round-2 atomicity of money evidence and durable escalation is preserved unchanged. No migration was needed; 066 receipt content, 067 payment operation lifecycle and 068 settlement horizon are untouched. F-2, F-5, F-6, F-10 and F-11 were not implemented. Grow, the 8 percent fee policy and the absence of any distributor commission are unchanged.

A FALSE START, RECORDED RATHER THAN HIDDEN: the same round reproduced the payments-group failure Codex reported using the exact seed 209752203 index 152. It was a real regression from this review's own round-1 change and did NOT exist on the candidate (passes on 356574f, fails deterministically 3 of 3 on e2f9966 and 812dd56). The first remedy was to stop a late claim from overwriting the provider's answer to the exact request; running the payments group proved that WRONG, because FR-3 in payment_review_findings_reconstruction failed and FR-3 encodes real safety: a recovery the provider DECLINED whose effect turns out real must converge to success, since that convergence is what BLOCKS the pending release of money that actually moved (migration 067 refuses a release behind an executed capture). That product rule was therefore reverted. The actual defect was in the test instruments: the oracle and the fuzz suite both evaluated "a repeat is legal only after the previous identity is provider-declared failed" against the FINAL result_class, while the invariant is about the state AT DISPATCH TIME, so an identity that was permanent_fail when the repeat was dispatched and only later converged to success looked like an illegal repeat even though nothing unsafe happened at any decision point and no money moved twice. Both copies are now order-aware, keyed on the single path that produces the shape (a success settled with a late_money_effect note, which always leaves an operator case), so every other repeat over a non-permanent_fail identity still violates.

WHY A DURABLY MISSING DOUBLE-CHARGE ALERT PLUS HTTP 200 IS NOW IMPOSSIBLE: three independent reasons, one per failure point. An identity-read failure cannot return "not dual", it returns unreadable and the only handling is to raise before any write. An escalation-write failure cannot leave the case missing while the evidence commits, because both are one transaction. And acknowledgement is downstream of both, since the route returns 200 only after the handler returns normally, which requires the commit that contains the case; any failure is caught, marks the provider event failed and rethrows. The state is always recoverable because claimEvent re-processes an event left in failed (same event id) and detection counts identities rather than testing for a replay (any fresh id). Removing any one of these turns the regression red: mutants RM-6, RM-7 and RM-8.

TESTED, all as CLEAN group runs with no corrective reruns and no environment workarounds: payments 61 of 61, concurrency 8 of 8, workers 13 of 13, security 39 of 39. Codex's full fuzz seed 209752203 now passes 300 of 300 scenarios (474 participants, 541 provider effects); index 152 passes with provider_effects 3, exactly matching the candidate. New identity-read regression 12 of 12 through the real signed webhook seam: read unavailable gives HTTP 500 with the event left failed, zero cases and charge_start identical before and after (no fabricated success or failure merely to permit a retry); the SAME event id then repairs it with exactly one case; a fresh delivery id also repairs it with still exactly one case; the same holds at the in-process read seam; and the case-write failure matrix, the single-capture duplicate control, the concurrent-retry control and the exact-request-decline scenarios all hold. Existing F-12 suite 4 of 4 including the new DS-4. FR-3 13 of 13. P0 foreign-reference proof 0 failures. Identity rails 10 of 10. Raw-SQL dispatch and lease fencing 21 of 21. Independent migration proof 49 of 49 and the candidate migration proof PASS with a fresh install of 61 migrations and a 59-to-61 upgrade applying only 067 and 068. Mutants RM-8 and RM-9 both KILLED. All static gates PASS: TypeScript source and tests, backend enforcement, payment compliance, runtime-DDL, architecture truth, route authorization and whole-delta whitespace. Zero deletions and zero renames against master.

DISPOSITIONS OF THE TWO GROUP FAILURES CODEX REPORTED: payments was a REAL defect, mine, now fixed and proved by the exact seed passing 300 of 300 and by the order-aware instruments. Concurrency was an ENVIRONMENT failure, not a financial defect: the failing scenario is payment_lab_concurrency_matrix TWO_WORKERS and their own log shows Error: spawn EPERM inside spawnWorker, so the test could not start its worker child processes at all; their retry outside the sandbox passed 32 of 32, and the "State mismatch deal ... expected" lines that look alarming are ordinary compare-and-swap retry logs which appear in their PASSING run too. On this host the concurrency group passes 8 of 8 as a clean run, three times across this task. No participant terminal-state defect exists.

OPEN: the five documented follow-up findings F-2, F-5, F-6, F-10 and F-11, none of them merge blockers and none implemented. Two operational prerequisites stay open: after migration 068 every pre-existing capture-side permanent_fail row is permanently fenced and needs an operator to record exact evidence before automatic recovery or release can run, and this branch must be MERGED into master rather than fast-forwarded, reset or force-pushed over it. One residual is recorded honestly: the 34 best-effort observability case writers still suppress their errors, which is correct for a report about an already-persisted decision but means those particular cases would still be lost under a wholly broken operational_cases table; the financial-critical F-12 path no longer is.

PERCENTAGE: F-12 remediation 100 percent across all three rounds. Financial code readiness high for the reviewed scope. Grow provider proof readiness 0 percent. Real-money readiness 0 percent and blocked.

NEXT STEP: CODEX INDEPENDENT RE-REVIEW OF F12 FIX + FINAL MERGE GATE.

UNRESOLVED MERGE BLOCKERS: none. SAFE_FOR_CODEX_RE_REVIEW: YES. MERGED: NO. REAL_MONEY: 0. GROW_CALLED: NO. REAL_EMAIL_SENT: NO. REAL_SMS_SENT: NO. HOSTED_DB_CHANGED: NO. DEPLOYED: NO.

## R9C F-12 DURABLE ESCALATION REMEDIATION — READY FOR CODEX RE-REVIEW (2026-09-10)

Branch `claude/review-r9c-financial`, NOT merged, NOT deployed. Master `82c91d6` contained. Report: `docs/R9C_FINANCIAL_INDEPENDENT_REVIEW.md`.

COMPLETED: the one merge blocker from Codex's independent review of frozen SHA e2f9966 is fixed. It had two compounding root causes. First, the F-12 escalation was a suppressed best-effort side effect: the money evidence committed in one transaction and openPaymentOperationalCase wrote the operator case in another while swallowing every error, so a failing operational_cases INSERT produced HTTP 200, a delivery counted as handled, and no case existed. Second, the detection predicate was self-concealing: it asked whether EVERY executed capture-side identity differed from the reported one, which is a replay test rather than a dual-capture test, so the first delivery's own committed success row made the predicate false for ever and neither a same-id retry nor a fresh-id redelivery could detect the condition again. Detection now COUNTS distinct executed capture-side identities including the effect being reported, so two or more means dual capture and the answer is stable across deliveries; and the money evidence and the operator escalation are written in ONE transaction through a new in-transaction case writer whose errors propagate, so either both exist or neither does and a failure reaches the caller, which marks the provider event failed and rethrows. Because claimEvent re-processes an event left in failed, the same event id repairs it on retry, and any fresh delivery id repairs it too because detection is identity-based. The escalation key is derived from the obligation plus the sorted distinct executed capture-side identities, so it is identical on every delivery and independent of which event type reported it, and idempotency comes from the existing partial unique index ux_operational_cases_open_auto_key. openPaymentOperationalCase keeps its best-effort semantics for its 34 other observability call sites. No migration was required and migrations 066 receipt content, 067 payment operation lifecycle and 068 settlement horizon are untouched. F-2, F-5, F-6, F-10 and F-11 remain documented follow-ups and were not implemented. Grow, the 8 percent fee policy and the absence of any distributor commission are unchanged.

WHY THE LOST-ESCALATION STATE IS NOW IMPOSSIBLE: acknowledgement is downstream of the commit that contains the case, so no ordering exists in which a delivery is accepted while the case is missing. If the case cannot be written the transaction aborts, the evidence does not commit either, the event is left failed and is re-processed. Even if a capture success row were already committed by any other path, detection counts identities instead of testing for a replay, so every later delivery still sees two distinct executed capture-side operations and escalates. Rolling the evidence back loses nothing because the observation stays durable in siton.webhook_events with its payload; provider truth is never altered to avoid an escalation.

TESTED: new regression tests/review_payment_dual_capture_durable_escalation_validation.ts 7 of 7 through the real signed webhook seam with a local simulated provider, injecting the failure both at the database (a trigger failing every operational_cases INSERT) and at a new in-transaction fault seam. Measured: escalation unavailable gives HTTP 500 with the webhook row left failed, zero cases and charge_start still permanent_fail so nothing half-committed; the same event id retried after restore gives HTTP 200, charge_start success and exactly one case keyed dual-capture; a fresh delivery id keeps exactly one case with an identical key; the in-transaction seam aborts both halves and the retry commits both; one capture with three duplicate deliveries stays at zero cases with one provider capture; five concurrent fresh deliveries yield exactly one case; and a dual capture whose evidence is already recorded with no case is still escalated by a fresh delivery, which is the state the old predicate could never escape. Non-vacuity: mutants RM-6 (revert the predicate to the replay test) and RM-7 (move the escalation back onto its own suppressed transaction) are both KILLED. Existing F-12 suite 3 of 3, P0 foreign-reference proof 0 failures, exact-operation identity rails 10 of 10, raw-SQL dispatch and lease fencing 21 of 21, independent migration proof 49 of 49, candidate migration proof PASS with a fresh install of 61 migrations and a 59-to-61 upgrade applying only 067 and 068. Groups: payments 61 of 61, workers 13 of 13, concurrency 8 of 8, security 39 of 39. TypeScript source and tests compile clean.

OPEN: the five documented follow-up findings F-2, F-5, F-6, F-10 and F-11, none of them merge blockers and none implemented in this task. Two operational prerequisites also stay open: after migration 068 every pre-existing capture-side permanent_fail row is permanently fenced and needs an operator to record exact evidence before automatic recovery or release can run, and this branch must be MERGED into master rather than fast-forwarded, reset or force-pushed over it, or master's own fixes are lost. One residual is recorded honestly: the 34 best-effort observability case writers still suppress their errors, which is correct for a report about an already-persisted decision but means that under a wholly broken operational_cases table those particular observability cases would still be lost; the financial-critical F-12 path no longer is.

PERCENTAGE: F-12 remediation 100 percent. Pre-merge closure 100 percent. Financial code readiness high for the reviewed scope. Grow provider proof readiness 0 percent. Real-money readiness 0 percent and blocked.

NEXT STEP: CODEX INDEPENDENT RE-REVIEW OF F12 FIX + FINAL MERGE GATE.

UNRESOLVED MERGE BLOCKERS: none. SAFE_FOR_CODEX_RE_REVIEW: YES. MERGED: NO. REAL_MONEY: 0. GROW_CALLED: NO. REAL_EMAIL_SENT: NO. REAL_SMS_SENT: NO. HOSTED_DB_CHANGED: NO. DEPLOYED: NO.

## R9C PRE-MERGE CLOSURE — FROZEN AND READY FOR INDEPENDENT REVIEW (2026-09-10)

Branch `claude/review-r9c-financial`, NOT merged, NOT deployed. Reviewed candidate `codex/r9c-master-integration-candidate` @ `356574f` (untouched). Report: `docs/R9C_FINANCIAL_INDEPENDENT_REVIEW.md`.

COMPLETED: the branch is frozen at a clean pre-merge state. Master moved TWICE during this work (d4c7877 to c1ce4e4 to 82c91d6) and is fully contained by merge, never by rebase or force, so nothing of master's is lost: receipt methods, seller profile and trust, the CMS and content work, migration 066_receipt_trust_content, the newly added staging receipt-content grants and the acceptance record are all byte-identical to master, and the redemption money gate master added in c1ce4e4 survives. The complete diff against master was audited file by file: 74 files, zero deletions, zero renames, nothing outside the payment rails and their proofs, and no change to web, mobile, notification, UX, Grow runtime or hosted configuration. The two review fixes stand: the migration-id collision is resolved as 066 receipt trust at ledger position 59, 067 payment operation lifecycle at 60 and 068 settlement horizon at 61 with the SQL bytes unchanged; and F-12, a late original capture arriving after a successful recovery, is escalated instead of being absorbed as an idempotent replay. The remaining worker-fencing uncertainty is CLOSED at its root cause.

WORKER FENCING RESOLUTION: not a runtime defect, not a concurrency race, not a timeout that wanted raising. It is a test-harness defect. The blocking device, ACCESS EXCLUSIVE on siton.deals, is what makes the deadline_check handler's first read block, but that mode also blocks every other plain read of siton.deals, and runWorkerMaintenance runs each worker cycle and calls rescheduleStalledFinalizations, which scans FROM siton.deals. A worker inside maintenance when the lock lands is stalled there for the whole phase, never reaches its next claim, and the jobs it was meant to claim stay pending forever. A captured timeline of a failing run shows one worker claiming its three jobs at 271 ms and the other claiming nothing for the remaining 19.6 seconds while three rows stayed pending and both workers kept heartbeating. A longer timeout cannot help, because the stalled worker cannot claim until the lock is released at the end of the phase, and the lock mode cannot be weakened, because the handler's own blocking access is a plain SELECT. One hypothesis was tried and discarded honestly: making the phase's jobs visible in a single INSERT did not change the rate, so it was reverted rather than shipped. The fix retries the ARRANGEMENT only, bounded to two attempts inside the file's 180-second budget, releasing the lock and draining between attempts. Every assertion and every run label is byte-identical and the new readiness predicate is stricter than the count it replaces, because it also requires the three-plus-three ownership split up front. A second defect in the same file was fixed: it killed its worker children only on the success path, so a failed assertion leaked live workers that kept polling a database the harness was about to drop, and orphans accumulated across runs and degraded every later suite on the host.

TESTED: worker fencing 12 of 12 isolated runs pass after the fix against 6 of 12 before it, measured with the same probe on an idle host; the underlying stall still occurs at the same rate and two passing runs took about 39 seconds instead of 18, which is the retry absorbing it. Earlier measurements of this file were distorted by another test runner active on this host, which turned out to be the work that became master 82c91d6. Groups after the fix: workers 13 of 13, payments 60 of 60, concurrency 8 of 8, security 39 of 39. Proofs re-run on the closure tree: independent migration proof 49 of 49, the candidate's own migration proof PASS with a fresh install of 61 migrations and a true 59-to-61 upgrade applying only 067 and 068 with the master ledger unchanged and a tampered checksum rejected, the P0 foreign-reference A/B PASS with zero failures, the F-12 dual-capture proof 3 of 3, the exact-operation identity rails 10 of 10, and the raw-SQL dispatch and lease fencing proof 21 of 21. Static gates all PASS: TypeScript source and tests, backend enforcement, payment compliance, runtime-DDL, architecture truth, route authorization and whole-delta whitespace. The migration manifest reads 61 entries ending 066 at 59, 067 at 60, 068 at 61.

OPEN: five follow-up findings are documented and deliberately NOT implemented in this task, exactly as instructed. F-2, the port's F-6 correction creates the new pair buyer_state Dropped with money_state ChargeFailedRecovery, which seller analytics counts in both the pending and the failed bucket; display only. F-5, payment operational cases carry no participant_id, so they are invisible to participant-filtered admin views. F-6, an un-targeted ON CONFLICT DO NOTHING with a discarded return value could commit a capture with no fee row if a provider ever reused an event id; under-collection, never a double fee. F-10, Grow status selection prefers captured over refunded within one payment process, contained today only because Grow's ambiguity policy is fail-closed. F-11, PAYMENT_NEGATIVE_STATUS_AUTHORITATIVE defaults to the permissive value with no real-money assertion. Two operational prerequisites also stay open: after 068 every pre-existing capture-side permanent_fail row is permanently fenced and needs an operator to record exact evidence before automatic recovery or release can run, and the candidate must be MERGED into master rather than fast-forwarded, reset or force-pushed over it, or master's redemption money gate is lost.

PERCENTAGE: pre-merge closure of the requested scope 100 percent. Financial code readiness high for the reviewed scope, with the five follow-ups above open and non-blocking. Grow provider proof readiness 0 percent. Real-money readiness 0 percent and blocked.

NEXT STEP: Independent Codex final review and merge gate.

UNRESOLVED BLOCKERS: none. SAFE_FOR_INDEPENDENT_REVIEW: YES. MERGED: NO. REAL_MONEY: 0. GROW_CALLED: NO. REAL_EMAIL_SENT: NO. REAL_SMS_SENT: NO. HOSTED_DB_CHANGED: NO. DEPLOYED: NO.
...
COMPLETED: source verified (candidate really is based on d4c7877; merge-base is d4c7877 itself; all 12 financial runtime files byte-identical to reviewed source 3809b32; the app.ts and frontend_runtime.ts deltas are line-for-line identical both ways; migrations 001-065 untouched, 065 unchanged). Money lifecycle reconstructed from code. P0 foreign-reference false success REPRODUCED on canonical master and PROVED fixed on the candidate. Exact-operation identity attacked on every remaining rail (recovery pre-flight, refund lookup, release lookup, identity reuse, late callbacks, sibling participant). Dispatch and lease fencing proved in raw SQL at the database, not only in the process. Migrations reviewed strictly and proved by a second, independent proof script. Canonical 8% fee, VAT exclusion, delivery inclusion and zero distributor commission re-derived from source.

MERGE BLOCKER FOUND AND FIXED ON THE REVIEW BRANCH: origin/master advanced during the review to c1ce4e4 and LANDED migration id 066 (066_receipt_trust_content.sql) at ledger position 59, colliding with the candidate's 066_payment_operation_lifecycle.sql at the same id and position. The runner hashes exact bytes and rejects a changed id/position/checksum, so the two cannot coexist. Resolved per migration 065's own rule (the branch landing second appends after): the financial SQL is now 067_payment_operation_lifecycle.sql (position 60) and 068_payment_settlement_horizon.sql (position 61), SQL bytes unchanged, every landed migration keeping its id, filename, checksum and position. Neither file was ever applied outside disposable local databases, so this is an id assignment and not a rewrite of migration history.

TESTED: definitive full suite with the F-12 fix in place — 240 files, 239 pass, 9 of 10 groups green (unit 15/15, integration 31/31, db 8/8, api 44/44, workers 12/13, payments 60/60, security 39/39, concurrency 8/8, failure 9/9, e2e 13/13), fresh isolated database per file. The one failing file is worker_two_process_fencing_validation.ts at its 20-second "p2 all six claimed and blocked" poll — a genuine pre-existing flake, and that conclusion was TESTED rather than assumed: it failed once in isolation, so app.ts was reverted to the pre-fix commit for a controlled A/B, and because a single pre-fix pass is not evidence, more samples were taken; with the fix in place it passes 3 of 3 isolated (4 isolated samples: 3 pass, 1 fail), and no causal mechanism exists because deadline_check never ingests a payment event. Two earlier first-pass group failures were explained and resolved rather than reinterpreted: payments failed on exactly one file, this review's own deliberately-red F-12 counterexample (the failing-test-first evidence, now green), and integration hit the all-runner's 30-minute per-group cap under load while passing 31/31 in 72-84 seconds standalone and in the second full run. Independent migration proof 49/49 (fresh install 61, true 59-to-61 upgrade applying only 067/068 with production-shaped legacy payment rows, all 59 historical ledger rows byte-identical, fresh-vs-upgraded schema equivalent across 997 columns / 1073 constraints / 254 indexes / 19 triggers / 24 functions, idempotent re-run, direct SQL re-execution, tampered checksum rejected, 068 proved a strict superset of master's 053 money-transition table). Candidate's own migration proof re-pinned to the landed master ledger: PASS. DB-level fencing proof 21/21 in raw SQL. P0 A/B: canonical master FAILS 21 assertions (every one of seven foreign-reference shapes, including another participant's authorization, becomes ChargedSuccess with ZERO capture requests dispatched); review branch PASSES 29/29 with a visible reference-mismatch case each time. Reviewer adversarial identity suite 10/10 across all rails. Reviewer mutation proof 5/5 killed, 0 survived (RM-4 initially survived and exposed a real gap in the reviewer's own coverage, which is why RI-9 exists; RM-5 guards the F-12 fix). All 10 pre-existing test files the candidate modifies were read: none weakens an assertion. Static gates all PASS: TypeScript source and tests, backend enforcement, payment compliance, runtime-DDL, architecture truth, route authorization, whole-delta whitespace.

FOUND AND FIXED ON THE REVIEW BRANCH: F-12 — a dual capture was absorbed as an idempotent replay. recordLateMoneyEffectException judged "contradiction" from the money state alone, so a late ORIGINAL capture landing after a successful recovery left the state at RecoveredCharge, read as a harmless replay, and was dropped: measured capture=1 recover=1 at the provider with cases=[]. Two real captures, canonical state accounting for one, nobody told. Fixed by detecting it through identity rather than state, conservatively (only when a DIFFERENT capture-side identity is positively recorded as executed), with two controls proving a duplicate delivery stays silent and the pre-existing escalation is preserved. PREEXISTING in the reviewed lineage, not introduced by this candidate; canonical master is worse and has no late-effect mechanism at all. The rails do not cause it (the lab oracle already fails any scenario with more than one capture), so it is a production observability gap rather than a source of double money.

OPEN: F-2 seller read models double-count a dropped buyer whose money state is still ChargeFailedRecovery (display only, INTRODUCED BY THE PORT, not fixed here because it changes surfaces this task may not modify). F-5 payment operational cases carry no participant_id so they are invisible to participant-filtered admin views. F-6 an un-targeted ON CONFLICT DO NOTHING plus a discarded return value could commit a capture with no fee row if a provider reused an event id (under-collection, never a double fee). F-7 migration-day task: after 068 every pre-existing capture-side permanent_fail row is PERMANENTLY fenced because legacy rows have no dispatched_at and the 24h backfill matches none of them, so automatic recovery and release for those participants need an operator to record failure_evidence='operator'. F-10 Grow status selection prefers captured over refunded within a payment process (contained today only because Grow's ambiguity policy is fail-closed). F-11 PAYMENT_NEGATIVE_STATUS_AUTHORITATIVE defaults to the permissive value with no real-money assertion. F-3 voucher redemption without a money gate was a real P1 at d4c7877 and master's own newest commit already fixed it, so the candidate must be MERGED into master and never fast-forwarded, reset or force-pushed over it.

PERCENTAGE: review of the requested scope 100%. FINANCIAL_CODE_READINESS high for the reviewed scope. GROW_PROVIDER_PROOF_READINESS 0% (no sandbox credentials, no provider-side evidence of exact-operation references, finality, settlement horizon, settle/refund idempotency or authenticated callbacks). REAL_MONEY_READINESS 0% and blocked.

SAFE_TO_MERGE_CODE: YES_WITH_REQUIRED_FIXES_ON_REVIEW_BRANCH (the 067/068 renumbering and its reference updates, already applied here, plus merging rather than fast-forwarding). SAFE_FOR_REAL_MONEY: NO. REAL_MONEY_EXECUTED: 0. GROW_CALLED: NO. REAL_EMAIL_SENT: NO. REAL_SMS_SENT: NO. HOSTED_DB_CHANGED: NO. MERGED: NO.

NEXT STEP: owner decision on merging claude/review-r9c-financial into master, then the three non-blocking follow-ups (F-2, F-5, F-6). Before any Grow work: resolve F-10 and F-11 and plan F-7's operator annotation for existing failed captures.

## RECEIPT / SELLER TRUST / CONTENT — FINAL VERIFICATION (2026-09-10)
...
## R9C current-master integration candidate - completed (2026-09-09)

COMPLETED: R9C current-master semantic integration candidate on codex/r9c-master-integration-candidate, based on d4c7877; all 16 must-port items implemented from reviewed 3809b32. P0 and B2 fixed. Master product changes preserved. Reviewed 063/064 safely appended as 066/067; landed 065 unchanged.
TESTED: 188 distinct files PASS, including all 39 directly ported tests; payments 57/57, concurrency 8/8, API 44/44, integration 30/30, security 39/39, worker recovery/two-process fencing 3/3, completion-window 1/1, additional E2E 2/2, focused P0/B2 3/3, frontend smoke 3/3 plus unskipped QR 9/9; fresh 60-migration and master 58-to-60 upgrade/legacy/rerun/checksum proofs; mutants 4/4 killed and restored-source 4/4 PASS; fuzz 100/100 seed 26090866; TypeScript, lint/backend, payment, runtime-DDL, architecture and diff gates PASS. Exact evidence in docs/R9C_MASTER_INTEGRATION_EVIDENCE.json and docs/R9C_MASTER_INTEGRATION_REPORT.md.
OPEN: Grow sandbox/live proof (credentials, exact-operation status/finality, settlement horizon, settle/refund idempotency and callback/effect evidence); external financial review of the combined candidate. No remaining local integration blocker. No hosted migration/deployment performed.
PERCENTAGE: R9C code integration 100%; real-money readiness 0% approved (external financial proof is still absent, not an estimate of code completion).
NEXT STEP: Independent financial review of the pushed integration candidate and its migration/mutation evidence.
SAFE_TO_MERGE_CODE: YES. SAFE_FOR_REAL_MONEY: NO. REAL_MONEY_EXECUTED: 0. GROW_RUNTIME_ENABLED: NO. Master not merged.
