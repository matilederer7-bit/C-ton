...
- UPDATED: 2026-09-18
- BRANCH: `claude/redteam-money-input` → **PR #59**, `claude/redteam-runtime-role-gate` → **PR #60** (merge/deploy/hosted evidence is added here when the chain completes). Previous: `claude/nice-goldberg-a3qxq8` → PRs #57 and #58, both merged; master `d485550` deployed and verified in both languages.
- COMPLETED (phase 1, closed): **Siton is one system in two languages.** Hebrew stays the default and is never replaced by an operating-system or `Accept-Language` guess; English is an explicit choice kept in both `localStorage` and a first-party `siton_lang` cookie, so the React app and the server-rendered pages cannot disagree. 2454 keys, every one with Hebrew and English, addressed by hierarchical key — there is no `if (locale === "en")` in product code. Direction follows from `dir` alone through logical properties, with no second stylesheet. CMS blocks carry `fields_en`/`items_en` with a declared, reported fallback; the seven legal documents have an approved-English structure and deliberately no invented content. PR #57 and PR #58 merged, Render web deploy `dep-damjso97lnhs73cedfo0` live on `d485550`, 57 hosted surface-checks in both languages at 1440 and 390.
- COMPLETED (phase 2, red team): Attacked first, fixed only what was proven against a **running** system. **(a) Money inputs (PR #59).** Every money column is `numeric(12,2)`; the validators checked the raw JavaScript float and then handed it to PostgreSQL, so the value that passed validation and the value that got stored were not the same number. `price_per_unit: 0` is refused with 400 — and `0.001` was accepted, stored `0.00`, **published, and publicly live at a price of zero**, with the money rail computing gross 0, fee 0, seller net 0. A delivery cost of `"abc"` became `Math.max(0, NaN)` = NaN, which `numeric` accepts verbatim: the deal published, the buyer was shown `cost: null`, and `calculatePlatformFeeMoney` turned the poisoned total into a well-formed snapshot of **zero** through `Number(value) || 0`. A regular price of `50.001` against a group price of `50` stored 50.00 vs 50.00, advertising a saving of zero against its own comment. `1e15` and `min_units: 10.4` leaked raw `22003`/`22P02` as 500s while the draft-patch path already answered 400. Fixed with one canonical reader (`src/money_input.ts`) that reads each amount **as the column will hold it**, plus a fee engine that now fails closed on a non-finite amount. **(b) The database boundary (PR #60).** Production runs the Web process as the non-superuser `siton_web_runtime`, but all 112 DB-touching suites read one `DATABASE_URL` for both the app pool and their own fixtures, and CI supplies the owner. Running the whole suite twice on byte-identical databases — once as `postgres`, once as `siton_web_runtime` — 46 suites flip, 38 with SQLSTATE 42501, and **all 38 fail inside the test's own fixture code, none inside a route**. The boundary is sound; the harness simply had privileges the product never has. A new gate drives the real app on a production-shaped connection across the real route table.
- TESTED: All ten local groups PASS (unit 17, integration 47, db 8, api 49, workers 15, payments 45, security 44, concurrency 9, failure 9, e2e 15). New: `tests/platform_fee_money_input_boundary_validation.ts` (9 checks; **7 fail before the fix and pass after**, the other two are labelled guard-rails, not bug fixes) and `tests/runtime_role_route_privilege_validation.ts` (applies the canonical boundary, becomes `siton_web_runtime` the way production does, sweeps **142 registered GET routes** plus the full seller lifecycle, zero 42501). That gate was red-teamed against itself and the first version was weak: reading response bodies caught **1 of 142** routes because the others correctly sanitise into `internal_error`; capturing SQLSTATE 42501 in a Fastify `onError` hook instead caught **32 of 142** under the same negative control. `tests/concurrency_proof.ts` was attacked for the usual cheat and does not cheat — 100 parallel joins with distinct buyers across two web instances. The bilingual dictionaries were attacked for missing keys, orphans, placeholder mismatch, empty English and bidi marks: **zero** on every count. No test was weakened, disabled or skipped.
- OPEN: **Plural agreement is broken in both languages** — 12 keys interpolate a count into a hard-coded plural, so `{qty} units` renders **"1 units"** (and `1 יחידות`) whenever a buyer joins with a single unit. Proven and user-visible; not fixed, because the dictionaries are generated artefacts and the translator needs a plural-selection rule, which changes the i18n contract rather than patching a call site. `min_units: -5` still clamps silently to 1 — long-standing behaviour the create defaults rely on, so changing it is a product decision. Sections **not** attacked in this pass, and therefore claiming nothing: workers/outbox beyond existing coverage, migrations, Render, Supabase, general security, and cross-role authorization escalation beyond the database boundary. Carried over: no approved English legal text (owner action), `OTP_HASH_SALT` absent from both Render services, the unpublished `deal_page` CMS draft, Sentry holding zero projects, and the external link-viewer journey unproven end to end.
- PERCENTAGE: Bilingual layer 100% and deployed. Red team: money-input boundary 100% (found, fixed, regression-tested); database runtime-role boundary 100% (measured, no product defect, gate added); bilingual attack 100%; concurrency reviewed and sound; **roughly half of the §2.9–§2.14 attack surface was not reached in this pass and is listed openly above rather than assumed clean.** Repository chain for PRs #59/#60: code/tests/commit/push/PR 100%; CI, merge, deploy and hosted verification in progress.
- NEXT STEP: Take PR #59 and PR #60 through CI to green, merge both, let Render deploy, confirm the deployed SHA equals master, re-run the hosted checks against the deployed build — and only then record this milestone as closed. Then either fix plural agreement or hand it to the owner as a scoped item.
