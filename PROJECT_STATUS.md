...
- OPERATIONAL NOTE (owner action — pending migrations must be applied in ledger order): the app connects as a least-privilege role and does NOT auto-apply DDL, and this session's role cannot read the staging DB's applied migration high-water, so the exact lag is not verifiable here. The code high-water is 074; every migration through 074 that staging has not yet applied must be applied via the owner's Supabase process. Blast radius if the DB is behind:
    ...
    - migration `073_payment_authorization_create_idempotency.sql` (`payment_authorization_bindings.provider_payment_url`): the column is part of the shared `BINDING_COLUMNS` select list in `src/payment_binding.ts`, so if 073 is unapplied the WHOLE binding-backed payment flow errors (`undefined_column`), not only authorize: `POST /api/payments/authorize[-mock]` (`createBinding`), buyer JOIN (`consumeBindingForJoinTx`), payment-status confirmation (`confirmBindingAuthorized`), the Grow webhook correlation lookup (`getBindingByCorrelation` catches the SQL error and silently classifies a VALID callback as unmatched), and the capture/renewal workers (`resolveRenewalSourceForParticipant`, `applyAuthorizationRenewalInTx`) fail before charging. Treat the payment flow as NOT healthy on a 072 schema. (Flagged by Codex on PR #93; 073 predates this track.)
