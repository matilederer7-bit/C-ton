## LAUNCH INTEGRATION CANDIDATE — INDEPENDENT UX + COMMUNICATIONS REVIEW (2026-09-09)

**COMPLETED:** Fresh `codex/launch-integration-candidate` worktree from verified master `d4c7877f9b704734036d69f57f7d98f794f4d034`; verified and cherry-picked UX `682b93c` and communications through `996ba66`. All nine approved UX items and sixteen notification events integrated. Separate seller-facing rejection reason/internal note fields now work in both admin KYC entry points. Independent review fixed notification reads/inquiry enqueue outside savepoints, missing token RETURNING privileges and excess profile columns in grant 023, stale-quantity checkout, and a browser-proven late scroll shift. Rehearsal now refuses DB query overrides and blocks external sockets. Full review: [docs/LAUNCH_INTEGRATION_REVIEW.md](docs/LAUNCH_INTEGRATION_REVIEW.md).

**TESTED:** Combined repository coverage **213/213 files across all ten groups after documented reruns** (initial full run had four failures: two changed quantity source assertions and two provider-default test-environment mismatches; all reruns passed). Communications rehearsal **19/19 + 12/12**, 65 dry-run successes, deliberate blocked/exhausted rows, rollback/recipient/token/insert failure proofs, retry/crash reclaim, zero real-mode attempts; unsafe DB URL and external socket negative controls passed. Browser **82/82**, 73 page/form measurements at **390/430/1280**, zero horizontal overflow/console errors/failed essential requests, exact Back/Forward restoration, six KYC flows with audit and no internal-note leakage. Grant tested only with a disposable local role in a rolled-back transaction. TypeScript/build, lint, payment/runtime-DDL/architecture/integrity scans, static+behavioral route authorization, mobile gate and 58 existing migrations passed locally.

**OPEN:** No hosted deployment or hosted acceptance. Grant 023 is reviewed/corrected and safe for the established privileged staging procedure, **NOT applied**. Real email and SMS still need providers/adapters, sender/domain setup and provider sandbox proof. The pre-existing gap between deal finalization and subsequent participant/receipt/fulfillment work remains a crash-replay operational/real-money blocker; notification enqueue failures remain logged best-effort omissions, not guaranteed automatic repair. Future real delivery needs provider idempotency/stale-worker fencing. Node 22/Postgres 16 remote CI remains an environment check after local Node 24/Postgres 18 validation. Native map-app hand-off is outside browser proof.

**PERCENTAGE:** Scoped UX **9/9 (100%)**, required transactional event wiring **16/16 (100%)**, KYC UI gap **1/1 (100%)**. These are code-scope completion measures, not overall launch readiness. Real email activation **0%**, real SMS activation **0%**, real-money readiness **0%**. Closed-web-pilot code has local evidence; hosted readiness is pending and not re-scored. REAL MONEY=0; REAL EMAIL=0; REAL SMS=0; REAL EXTERNAL NOTIFICATION DELIVERY=0. Siton fee remains 8% on authoritative charge base including delivery, excluding buyer VAT; distributor commission/payout remain 0. No financial migration, Grow, hosted Supabase, Render plan or mobile feature change.

**NEXT STEP:** Review candidate and remote CI; separate merge/deploy authorization and staging grant application, then hosted dry-run owner acceptance with a finalization contingency. Do not activate payment/email/SMS providers. This milestone supersedes readiness claims in the preserved source-branch entries below.

## LAUNCH SPRINT 4 — OWNER UX CLEANUP (2026-09-09) — branch `claude/launch-ux-cleanup` from exact master `d4c7877`, NOT merged

**COMPLETED (9 owner findings, each closed separately — doc: docs/LAUNCH_UX_CLEANUP_SPRINT_4.md):**
1. **Pickup location / maps** — one navigation truth in `src/pickup_location.ts`: stored coordinates → Google Maps AND Waze navigate to the SAME coordinates ("נקודה מדויקת"); address text only → both apps get an address search, never a fabricated pin ("ניווט לפי חיפוש כתובת"); generic label → nothing. `describePickupLocation` projects `precision` + `navigation` on public/seller payloads, `tracking.pickup.pickup_navigation` on the buyer card; `PickupNavActions` (two labelled buttons with glyphs) replaces the lone "פתח במפה"; the seller sees "מיקום מדויק הוגדר" / "הוגדרה כתובת בלבד — מומלץ לאמת נקודה מדויקת"; inline map URL builders deleted. No paid maps API.
2. **Mobile overflow** — Sprint 3's fix (RTL honeypot at −9999px) was the previous confirmed source; NEW regression proof sweeps 17 routes × 3 widths (landing, deal, join sheet, address-only deal, tracking ×2, support, legal ×3, seller dashboard/deal/draft editor/pickup, admin overview/growth/buyers/deals) asserting documentElement AND body `scrollWidth <= innerWidth`: **0 overflow on 64 page checks** (390: 24, 430: 18, 1280: 22). No global `overflow-x: hidden` band-aid (pinned).
3. **Viral tree backstage only** — tracking page keeps the simple share loop (WhatsApp / share / copy) and loses every propagation counter ("מצטרפים שהבאת", "דורות בענף", "השרשרת שלך", descendants); buyer-token `/impact` returns the share identity only (no recursive tree query); public join reply drops `viral.generation`; orphan `web/src/vtree.tsx` deleted. Seller + admin propagation tree, viral endpoints and attribution collection untouched and still server-authorized.
4. **Legal native React** — `#/legal/terms|privacy|refunds` rendered inside the product shell from the ONE source (`src/legal_pages.ts` → `parseLegalBlocks` → `GET /api/legal/:slug`); footer + consent links stay in-product; direct `/legal/<slug>` → `302 /preview/#/legal/<slug>`; standalone legal HTML shell deleted; content unchanged.
5. **Back/forward scroll restoration** — `web/src/scrollRestoration.ts`: position per HISTORY ENTRY (key in `history.state`, sessionStorage), traversal restores / new entry tops, async-render aware (rAF retries, 3 s budget, no smooth glide), hold phase undoes the loader clamp when returning from a tall page, pending restores never overwritten by loader scroll events, reload adopts the stored key. Router's unconditional scroll-to-top removed.
6. **Buyer search** — intent-sensitive (`src/buyer_search_intent.ts`): letters → name only (tokens, NFKC, niqqud stripped, final letters folded, case-insensitive), digits → phone, `@` → e-mail, `CT-…` → order code, UUID → id; predicate on the DISPLAYED values (matching participation name shown); `%`/`_` literal; intent + match reason shown; debounced input. Root cause reproduced: `MAX(buyer_name)` displayed a different name than the one that matched.
7. **Copy** — "צמיחה וויראליות" → "ויראליות" (heading + nav item); routes unchanged.
9. **Virality time filter** — default 7 days; 7/30/90/custom Israel-local days (UTC instants server-side)/all time; the window drives every windowed number (`src/growth_window.ts`, `src/growth_metrics.ts`); lifetime rollup is a separate block labelled "מצטבר מאז ההשקה (כל הזמן)"; seller viral panel labelled lifetime; validation 400s with Hebrew reasons; no tiny product cap; no migration.
10. **Numeric inputs** — `QtyStepper` deleted; `QtyInput` + `web/src/quantityInput.ts`: text-like, `inputMode="numeric"`, digits only, zero/over-stock refused with a reason; seller min/max typed with integer check; number-field spinner arrows hidden; canonical constraints untouched. Steppers remaining: 0.

**TESTED:** unit `frontend_foundation_sprint4_ux_cleanup_validation` 13/13 · api `admin_buyer_search_intent_validation` 8/8 · api `admin_growth_window_validation` 6/6 · affected regressions 19/19 files (r6 viral graph 16/16, legal trust 7/7 + layer 6/6, countdown/pickup 12/12, geolocation, pickup, react-legacy, buyer polish, p05 admin viral, p07 ×2, seller pickup fulfillment, pilot readiness, cache policy, tracking command center, delivery handoff) · browser proof `scripts/sprint4_ux_cleanup_browser_proof.cjs` **73/73** (64 page checks @390/430/1280, 0 console errors, 0 failed essential requests, 0 overflow, 0 text <10 px; real admin cookie session) · TypeScript server + web, lint, payment scan, runtime-DDL scan, architecture gate PASS · route-authorization gate, mobile gate, migrations report, FULL SUITE: route-authorization gate PASS (101 protected routes, 592 probes, 0 unguarded), mobile gate PASS, migrations report PASS on a fresh DB (58/58; the long-lived local DB shows the known 014 CRLF checksum artifact), FULL SUITE 210/210 files — unit 16, integration 30, db 8, api 46, workers 13, payments 29, security 39, concurrency 7, failure 9, e2e 13 (all 10 groups PASS).

**OPEN:** seller virality panel windowing (separate engine); native app hand-off for Google Maps/Waze not provable by web automation (documented deep-link formats used); `deals.description` > 420 chars answers 500 instead of 400 (pre-existing, noted). Item 8 (long-horizon deals) lives on `claude/long-horizon-deals-architecture`.

**PERCENTAGE:** UX_OWNER_FINDINGS_CLOSED 9/9 (Track A) · closed-web-pilot readiness unchanged by this sprint except UX (no hosted deploy yet).

**NEXT STEP:** independent review → merge → deploy (no migration, nothing to apply on staging); then window the seller viral panel on the same `growth_window` contract.


## PILOT COMMUNICATIONS READINESS — TRANSACTIONAL NOTIFICATIONS, DRY-RUN PROOF, NO REAL DELIVERY (2026-09-09) — branch `claude/pilot-communications-readiness` from exact master `d4c7877`, NOT merged

**COMPLETED:** Every pilot business moment now produces exactly ONE intended notification row, committed in the SAME transaction as the canonical state it announces, with the recipient resolved from canonical rows and a deterministic idempotency key (`docs/PILOT_COMMUNICATIONS_READINESS.md`). **Wiring (`src/notification_events.ts`, savepoint-guarded `insideTx` of the lifecycle transitions):** `buyer_joined_authorized` (Join tx, reuses the Join's tracking token, mock-money truth), `buyer_deal_target_reached` + `seller_target_reached` (PendingTarget → TargetReached: inline Join path and `tryTargetReached`; N buyers + 1 seller; late joiners and pause/reopen never re-notify), `buyer_deal_completed` / `buyer_deal_failed` / `seller_deal_completed` / `seller_deal_failed` (Completed / Failed / deadline transitions — the message matches the outcome the same transaction commits), `seller_deal_published` (publish tx, canonical e-mail + link), `buyer_recovery_required` / `buyer_payment_recovered` (charge_failed / recovery_captured reconciliation truth, insideTx of the money transition), `buyer_voucher_issued` / `buyer_ticket_issued` (issuance tx; tracking link only, never the code), `seller_kyc_approved` / `seller_kyc_rejected` (KYC tx, only on an actual status change, audited as `seller_security_events` `seller.kyc.decision`, keyed on the decision ordinal; optional bounded `seller_reason`, the internal admin note is never forwarded), `admin_security_alert` (rejected webhook signature — one per provider/reason/hour bucket; system-opened PaymentMismatch case) to `ADMIN_ALERT_EMAIL` only. **Removed on purpose:** the `charge_captured → "payment recovered"` mis-alias (untrue), the refund-time second "deal failed" (duplicate intent), the completion-time `seller_excel_ready` (duplicate intent); legacy aliases `charge_succeeded`/`refund_issued` dropped from the compatibility adapter. **Recipients:** buyer = the participant's own OTP phone (`participants.buyer_phone`), seller = support → login → business-profile contact e-mail of the deal's OWNER (spoofed `seller_id` refused), admin = configured destination or internal. **Templates:** `money_mode` (mock money never claims a charge; unknown renders as mock), canonical `/preview/#/…` links from `PUBLIC_BASE_URL` → `RENDER_EXTERNAL_URL` (never localhost), brand `C-ton`, KYC reason optional/sanitized, payload hygiene at enqueue (secret-like values, forbidden keys, size, nesting refused). **Provider boundary:** `NOTIFICATION_PROVIDER_MODE=dry-run` (`DryRunNotificationProvider`: real claim → safety gate → render → attempt row, deterministic `dryrun_` ids, zero network); safety gate validates destination format in dry-run and real modes, real-mode shadow verdict exposed; `real` still fails closed (no adapter); adapter contract documented. **Operator read model:** `GET /api/admin/notifications/:id` (masked destination, redacted rendered subject/body, attempts, business correlation, safety verdicts) + `notifications-status` gains `blocked`/`cancelled`/`retry_scheduled`/per-channel `blocked`/`recipient_masked`. **Rehearsal:** `npm run rehearsal:communications` (`scripts/pilot_communications_rehearsal.cjs`: refuses non-local/hosted DB, disposable DB, both suites, masked "what would have been sent" report, asserts `real_mode_attempts=0`). **No migration** (066/067 stay reserved for the financial candidate); one staging grant file `supabase/staging/023_pilot_communications_worker_grants.sql` (Worker INSERT on participant_tracking_tokens + column-level SELECT on seller_business_profiles) — until applied Worker-side buyer links are queued with `link_mode=tokenless` (visible in the inspection endpoint), never lost. Payment provider / Grow / financial state machine / fulfillment architecture / mobile / web UI untouched.

**TESTED:** `tests/notification_pilot_events_validation.ts` **17/17** (real runtime + fresh DB + dry-run provider: publish, join + HTTP replay, failed-join rollback, target reached with 5 participants incl. late joiners + pause/reopen, finalize Completed with mixed capture outcomes + replay, deadline Failed + replay, voucher and ticket issuance + re-run, charge_failed/recovery_captured webhooks incl. duplicate event ids and a rolled-back money transition, inquiry with spoofed seller_id + retry, KYC approve/re-approve/reject/re-approve, admin alert burst, cross-seller/cross-buyer isolation sweep, the real Worker maintenance pass draining 57 rows with zero real-mode attempts, template/hygiene checks); `tests/notification_pilot_rehearsal_validation.ts` **12/12** (dry-run provider construction + fail-closed guards, sent/blocked paths, bounded retries, crash reclaim then success, permanent failure, duplicate ×10, real-mode simulation negative controls, masking/redaction, operator endpoints, payload hygiene, whole-run invariants); `scripts/pilot_communications_rehearsal.cjs` **PASS** on a disposable database (63 dry-run sends, 5 blocked, 2 deliberately failed, `real_mode_attempts=0`). Full repository suite `npm run test:all` on fresh isolated databases: unit 15/15, integration 30/30 (`mobile_readiness_validation` needed the mobile bundle built first — `npm run mobile:verify` — then passed in isolation; the other 29 passed in the run), db 8/8, api 44/44, workers 15/15, payments 29/29, security 39/39, concurrency 7/7, failure 9/9, e2e 13/13 — 209/209 files. TypeScript, lint/backend enforcement, payment compliance scan, runtime DDL scan, architecture gate, static route inventory (0 unguarded), mobile gate, `git diff --check`: all PASS. REAL_EMAIL_SENT=0 REAL_SMS_SENT=0 REAL_NETWORK_DELIVERY=0.

**OPEN:** real provider choice (e-mail: SPF/DKIM/DMARC + sender + adapter + sandbox proof; SMS: Israeli sender registration + adapter + segment budget), credentials, sandbox/provider proof, DNS/custom domain for `PUBLIC_BASE_URL`, production activation switches — exact steps in `docs/PILOT_COMMUNICATIONS_READINESS.md` §11–12; apply staging grant 023 (owner privileged procedure); the React admin console still posts only `admin_note` on KYC decisions (a seller-facing `seller_reason` field is accepted by the API but not yet offered in the UI); the finalize-replay window between the Completed transition and receipts/fulfillment issuance is a pre-existing financial-branch concern (notifications are no longer affected).

**PERCENTAGE:** TRANSACTIONAL_NOTIFICATION_CODE_READINESS **95 %** (remaining = review/merge + staging grant apply + hosted dry-run pass). REAL_EMAIL_READINESS **15 %** (rail, safety gate, adapter contract and activation steps exist; no provider, no domain, no adapter). REAL_SMS_READINESS **10 %** (same, plus sender registration and segment budget still open). CLOSED WEB PILOT readiness unchanged at **98 %**; REAL MONEY readiness **0 %** — unchanged and separate.

**NEXT STEP:** owner picks the e-mail provider (the seller inquiry pointer and the KYC decision are the two messages the pilot needs most) → adapter behind the existing boundary + sandbox proof with owner-only allowlist on staging.
