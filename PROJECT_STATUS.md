...
## INDEPENDENT ADVERSARIAL FINANCIAL REVIEW OF `5dcee1f` (2026-09-07) — branch `claude/r9c-financial-review-remediation` (from the exact candidate SHA), NOT merged

**COMPLETED**
- Git truth verified: `origin/master = 60ebf6d64fb0d5909ae98fa335a4a27ad45c9172`, `origin/claude/r9c-financial-torture-candidate = 5dcee1ff29fc2159d27ddc80a355d8d01b8a46dc`, master a direct ancestor (27 commits ahead, 0 behind). Master untouched; nothing merged; real money 0; real Grow calls 0; migrations 063/064 never applied to staging.
- The candidate was reviewed as another team's code (report, tests, oracle and classifications not trusted). Every finding was reproduced by a NEW counterexample committed RED against the unmodified candidate source (`644a133`, log `EVIDENCE_baseline_5dcee1f_unmodified.log`: adversarial 10/20 red, Grow 1/2 red, mock 0/1 red), then fixed narrowly (`3c9fc9c`, R-11 in `7c5d8af`) and pinned (`777b6b8`). Record: `docs/R9C_FINANCIAL_REVIEW.md`.
- **Findings on the unmodified candidate:** **R-1 CRITICAL (synthetic)** automatic recovery while the original capture could still settle — a provider answering a CONSISTENT `failed/final` or `authorized/final` during an asynchronous settlement defeats the F-9 double-read; 8 400 minor captured for a 4 200 obligation (RA-1a/1b/1c); **R-2 CRITICAL** the same at the completion-window end — terminal decision on money still moving (RA-9); **R-3 CRITICAL by policy** Grow: settle executed → 503 → lookup `failed` → `charge_failed` verdict (only `authorized/final` was gated by `negative_status_authoritative`) → recovery → a SECOND `settleSuspendedTransaction`; **R-4 HIGH (F-6)** `recovery_failed → AuthReleased` with zero provider releases, even when the provider would refuse the release (RA-2a/b/c); **R-5 MEDIUM** currency never part of the identity (USD answer applied as ILS `ChargedSuccess`); **R-6 MEDIUM** a wrong-reference status echo rewrote the durable binding (O-1); **R-7 MEDIUM** unverifiable status still let the recovery proceed (RA-6); **R-8 MEDIUM** the mock's fabricated `captured` status flipped declined capture identities to SUCCESS and blocked recovery on mock-backed deployments; **R-9 LOW-MEDIUM** id-only 2xx refund/release bodies treated as success; **R-10 LOW-MEDIUM** a stale reconcile flipped an already-declined capture identity to `success` from a sibling identity's status (FR-4); **R-11 MEDIUM** (found by the full regression on the remediated tree, latent on the candidate) finalize decided while a capture identity was SUCCESS but its canonical state not yet applied → a paid buyer stranded at `ChargedSuccess` on a Completed deal (no DealCompleted / receipt / fulfillment / case) — fixed in `7c5d8af` (guard covers executed-unpersisted rows; a finalize retry on a Completed deal sweeps every paid participant, idempotent), regressions FR-2b/FR-2c, mutants M30/M31.
- **Remediation (owner financial-truth decisions applied):** migration **064** — durable, provider-specific **settlement horizon** (`settlement_horizon_at`: opened at arm, monotonic, extended by `pending` reads, never reset by identity rotation) + **failure evidence** provenance (`dispatch_response` = the provider's answer to the exact request; `status_inference` / `provider_event` / `operator`); `siton.payment_capture_settlement_fence()`; DB INSERT trigger refuses recovery/release identities while fenced; UPDATE trigger keeps the horizon monotonic and never downgrades exact-request evidence. Recovery and release jobs defer to the horizon with an open case; finalize defers to the last open horizon and re-verifies every dispatched status-inferred failure at the provider before deciding (late settlement → visible truth, captured money never released). Pre-flight: amount AND currency must match, `pending` extends, two `unknown` reads hold unless the failure is exact-request evidence. Reconcile: `failed` is a verdict only for providers with authoritative negative status (**Grow fails closed on `failed`**), currency mismatch case, status echoes never rewrite the binding, terminal identity steps aside while a sibling is unresolved. **F-6 fixed**: `recovery_failed` → `Dropped` only; `AuthReleased` only through the provider-proofed release rail (refused release = honest held state + case). Mock status truthful. Id-only 2xx → UNKNOWN. Policy `settlement_horizon_ms`: mock/synthetic 0, provider-ready `PAYMENT_SETTLEMENT_HORIZON_MS` default 24 h (owner-configured per provider contract; never guessed), lab 1.5 s.
- **F-1 … F-9 reconstructed with different variations** (`payment_review_findings_reconstruction_validation.ts`): F-2 (unknown recovery identity), F-3 (HTTP `recovery_captured` for a Dropped participant), F-4 (dead-lease `processing` reconcile → found R-10), F-5b (refund event with capture correlation), F-7/F-7b, F-8, F-9 (`FLAP(failed, failed, captured)`), recovery vs recovery, refund vs reconcile, release vs capture (documented residual).

**TESTED** (fresh isolated local databases; provider = in-process simulator / in-process Grow transport; no hosted environment)
| Suite | Result |
|---|---|
| `payment_review_adversarial_validation.ts` (RA-1 … RA-9, 20) | red 10/20 on `5dcee1f` → **20/20** after remediation |
| `payment_review_grow_negative_status_validation.ts` (2) | red 1/2 → **2/2** |
| `payment_review_mock_provider_truth_validation.ts` (1) | red 0/1 → **1/1** |
| `payment_review_settlement_horizon_validation.ts` (H-1 … H-8 incl. NEGATIVE CONTROL, 12) | **12/12** |
| `payment_review_findings_reconstruction_validation.ts` (11) | **11/11** |
| Mutation pass (`scripts/financial_lab_mutations.cjs`, M18–M29 new + M01/M02/M07/M16/M17 re-run) | **19 tested / 19 caught** (M18–M31 14/14; author mutants M01/M02/M07/M16/M17 re-run 5/5; M27's first form was a dead-site mutant, retargeted and red) |
| Full repository regression on the exact final tip | **r2 on `b6e20da` (final code): static gates 7/7 PASS, migrations 59/59, test:all 10/10 groups green — 218 files (unit 12, integration 29, db 7, api 41, workers 13, payments 51, security 36, concurrency 7, failure 9, e2e 13), fuzz 300/300 (seed 2105845141), soak 30 s exact, route gate PASS, fault report OK** (r1 on `7265568` had 5 red files, all triaged: 3 stub/contract adjustments, R-11 fixed, 1 load-sensitive worker suite that passes in isolation on both trees) — `docs/R9C_FINANCIAL_REVIEW.md` §6 |

**OPEN (documented residuals)**: provider finality lies beyond the configured horizon (RA-1d, oracle-visible); legacy NULL-horizon rows (pre-064) are not fenced (pre-flight holds on unverifiable status; none carry real money); status-answer reference identity is verified by amount + currency only (R10 provider contract); a capture may still be dispatched while a release of the same participant is in flight (FR-C4 — provider state machine decides, negative settlement fenced, consistent end state); Grow settle/refund idempotency and exact-operation status UNPROVEN (sandbox blocked).

**PERCENTAGE**: review 100 % of the requested scope on the candidate. SAFE_FOR_CODEX_REVIEW = YES. SAFE_TO_MERGE_FINANCIAL_BRANCH = NO. SAFE_FOR_REAL_MONEY = NO. R10 BLOCKED.

**NEXT STEP**: Codex independent review of the exact final SHA of `claude/r9c-financial-review-remediation`; then the Grow sandbox proof (exact-operation status + settle/refund idempotency) before any merge of financial work.

## FINANCIAL TORTURE PROGRAM — PHASES 2–22 (2026-09-07) — branch `claude/r9c-financial-torture-candidate` at `40f94d1` (+ this status commit; src/ identical to `6cec97f`, the tree of regression #4 — later commits are tests/docs only) (base `82f9171` = master `60ebf6d` + Phase 1), NOT merged

**COMPLETED**
- Phase 2 — R9C refreshed onto the latest master: the 10 commits of `claude/r9c-integration-candidate` @ `3f00edb` cherry-picked (`-x`) onto the pre-financial tip; only `PROJECT_STATUS.md` conflicted (master's kept). Equivalence proven both ways with normalised patch text (`IDENTICAL`): old-candidate→new-candidate ≡ master+Phase 1 delta; R9C delta on the new base ≡ original R9C delta. Migrations 061 PRESERVED / 062 RESERVED / 063 PRESERVED; fresh install 58/58, re-run idempotent, checksum ledger consistent, drift 0. Record: `docs/R9C_FINANCIAL_TORTURE_CANDIDATE.md`.
- Phases 3–4 — independent instruments: `tests/lab/provider_simulator.ts` (programmable provider-ready HTTP simulator with a private economic ledger and a native-idempotency toggle), `tests/lab/oracle.ts` (integer-agorot re-implementation of the financial constitution; provider effects vs payment_attempts vs money/buyer/deal state vs fee ledger vs audit vs outbox/DLQ vs cases), `tests/lab/runtime.ts` (real application against the simulator, deterministic outbox driving, signed synthetic webhooks). Oracle anti-vacuity proven with planted corruptions.
- Phases 5–20 — torture suites on the candidate (all synthetic): C1/C2 counterexamples on every rail, lifecycle matrix (5 identity states × 4 rails) + DB guards, idempotency and reconciliation torture, refund/release/recovery on a NON-idempotent provider, deterministic crash-point matrix (incl. provider-effect-then-DB-failure and backend termination while in flight — process survives via Phase 1B), concurrency matrix with claim races 2–50, parallel deals 5–50, stale owners, outbox/lease war and TWO REAL WORKER PROCESSES, terminal-state attacks, integer-agorot economics under explicit 17 % VAT, seeded random schedules with minimisation, bounded soak with global reconciliation. Record: `docs/FINANCIAL_TORTURE_LAB.md`.
- Findings, each reproduced deterministically, fixed with the smallest change and pinned by a regression:
  - **F-1 MEDIUM** — recovery trusted a once-observed final negative capture status; a status that later changed produced a SECOND capture. Fix: `verifyOriginalCaptureBeforeRecovery` pre-flight (captured → late-effect case, no recovery; ambiguous → defer).
  - **F-2 HIGH** — `finalize_deal` decided Completed/Failed while capture identities were UNKNOWN → charged buyer on a Failed deal with no refund path (found by the two-real-worker run). Fix: finalize defers while any capture-side identity is unresolved, schedules its reconcile, keeps a case open. Regression `payment_lab_finalize_guard_validation.ts`.
  - **F-3 HIGH** — the HTTP webhook route ignored economically real late events without a case (worker path had the guard, HTTP path did not). Fix: `recordLateMoneyEffectException` dependency wired into `handleWebhookPayments`.
  - **F-4 MEDIUM** — a reconcile request behind another pending reconcile of the same participant was silently dropped by the one-pending-per-aggregate index (fuzz seed 20260907 #2, minimised). Fix: `schedulePaymentReconcile` reports `queued_behind` + case; worker-maintenance sweeper `reconcileOrphanedUnknownIdentities`.
  - **F-5 / F-5b MEDIUM** — late money events were attached to an identity inferred from participant state or from a foreign correlation (a recovery identity marked `success` for a capture event; a capture identity marked `success` for a refund callback — fuzz seed 20260907 #46 and #69, minimised). Fix: target identity resolved by the event's operation family; late-effect settlement stays within that family; the pre-flight passes the capture correlation.
  - **F-7 HIGH** — the provider-ready refund and release adapters declared SUCCESS on any 2xx body, including a `pending` / ambiguous one (a refund that never settled became a canonical refund). Fix: `classifyRefundOutcome` / `classifyReleaseOutcome` — success only for a declared success status, declared failure → permanent_fail, anything else → UNKNOWN (reconcile rail).
  - **F-8 LOW** — a recovery held by the pre-flight (ambiguous status) had already minted a NOT_DISPATCHED identity that nothing attended (fuzz 2000 #344). Fix: pre-flight before minting; worker-maintenance sweeper also reconciles quiet NOT_DISPATCHED identities; the pre-flight steps aside for participants that already carry a recovery identity (regression #2).
  - **F-9 CRITICAL (synthetic, fixed)** — found by the RANDOM-seed fuzz inside the full regression (seed 2061983203 #141, minimised to one participant): a capture answered `200 pending` with a delayed effect while the status seam flapped `failed ↔ captured`; the reconcile rail and the single-read recovery pre-flight both read `failed`, the recovery captured, then the delayed capture landed — 8 400 minor captured for a 4 200 participant. Fix: the pre-flight reads status twice (`RECOVERY_PREFLIGHT_CONFIRM_MS`, default 1 s) and takes the most conservative verdict (any `captured` → late-effect case, any `pending` or disagreement → hold + `payment-recovery-preflight-flapping` case). Three pinned scenarios + fuzz rerun on the exact seed.
  - **F-6 MEDIUM — NOT FIXED (owner decision)** — the canonical `recovery_failed` transition sets `AuthReleased` without any provider release request (hold left to the provider's expiry). No money lost or duplicated; contradicts the R9A "no AuthReleased without proof" rule. Recommendation: `Dropped` + `payment_release` rail. Counted by the oracle as `CANONICAL_RELEASE_WITHOUT_PROVIDER_PROOF`.
- Phase 21 — mutation testing (`scripts/financial_lab_mutations.cjs`, 16 deliberate defects, each applied to the working copy, run against the mapped lab suite on fresh databases, reverted with `git checkout`): **16 tested, 14 caught, 2 survived** — M05 (app-level arm CAS) and M10 (arm-time lease check) survive because the migration-063 trigger and the CAS refuse the same writes at the database (redundant defences, proven directly by the lifecycle suite's DB-guard scenario). All economics mutations (7 %, 9 %, VAT in the base, delivery excluded, 5 % distributor share) and all money-safety mutations with an observable effect (UNKNOWN fencing, recovery blocking, refund ambiguity, identity rotation, ledger atomicity, reconcile deferral, late-event protection, duplicate webhooks, F-1 pre-flight) are red. Two first-pass survivors were test defects and were fixed honestly (a vacuous `duplicate` assertion, a provider guard masking the F-1 mutant).
- Phase 22 — full repository regression on the candidate. First full run (`5e521f3`): 213 files, 8/10 groups green (unit 12, integration 29, api 41, workers 13, security 36, concurrency 7, failure 9, e2e 13); db 6/7 and payments 44/46 red — all three failures attributable to this program's own F-1/F-3 changes (a recovery pre-flight that treated a status answer of `unknown` as ambiguous stalled two recovery proofs; the R9A webhook-truth proof asserted the pre-F-3 "late capture leaves the identity permanent_fail" behaviour). Fixed in `d4c2046` (pre-flight holds on `captured`/`pending` only) and by aligning that proof with the R9C late-effect rule. Second full run on the final tree: 9/10 groups (unit 12, integration 29, api 41, workers 13, payments 46 — incl. the fresh-seed fuzz 300/300 —, security 36, concurrency 7, failure 9, e2e 13) with db 6/7, the Phase 1B pool-hygiene proof flaking on a foreign idle backend / a 300 ms observation window; the proof was hardened twice (own pool backends + bounded poll still raced the 100 ms idle reaper of the test pool; the vacuity check now terminates a held client's backend) and the db group reruns 3/3 green (7/7 each, `40f94d1`). In between, regression #2 (`f5ab275`, 9/10: three payments proofs red because the F-8 pre-mint pre-flight misread an EXISTING recovery identity — fixed `565270f`) and regression #3 (`3b67821`, 8/10: the fuzz drew seed 2061983203 and found **F-9**, fixed `6cec97f`). Every red file across the four runs was attributable to this program's own changes or to a snapshot-trusting test; none to master or the ported R9C lifecycle.

**TESTED** (fresh isolated local databases; provider = in-process simulator; no hosted environment touched)
| Suite (fresh isolated DB each, final tree) | Result |
|---|---|
| `payment_lab_foundation_validation.ts` (simulator contract × 15 behaviours, finalize path, private counters, oracle anti-vacuity × 5, signed webhooks) | **23/23** |
| `payment_lab_c1_c2_validation.ts` (C1, C1b stale owner, C1c; C2 × 8 capture, × 5 recovery, × 5 refund, × 5 release; pre-dispatch) | **27/27** |
| `payment_lab_lifecycle_reconcile_validation.ts` (5 identity states × 4 rails, DB guards, reconcile torture, idempotency, F-4) | **37/37** |
| `payment_lab_refund_release_recovery_validation.ts` (non-idempotent provider; refund/release/recovery matrices; F-7; F-9 pins) | **29/29** (incl. 3 F-9 pins) |
| `payment_lab_crash_matrix_validation.ts` (crash points, lease expiry in flight, DB faults after the provider effect, backend termination, pool exhaustion) | **13/13** |
| `payment_lab_finalize_guard_validation.ts` (F-2 / F-2b) | **3/3** |
| `payment_lab_terminal_economics_validation.ts` (terminal-state attacks; integer-agorot economics under explicit 17 % VAT × 9 + 12-participant deal) | **17/17** |
| `payment_lab_concurrency_matrix_validation.ts` (claim races 2/5/10/25/50 × 4 rails, parallel 5/10/25/50, capture vs reconcile ×5, stale owner ×2/5/10, outbox war, backlog 60, TWO real worker processes 80/80) | **32/32**, deadlocks 0 |
| `payment_lab_random_schedule_fuzz_validation.ts` (seed 20260907) | **300/300** (451 participants, 525 provider effects); long run: **2 000/2 000** (3 107 participants, 3 576 provider effects, 789 s); F-9 seed 2061983203: **300/300** (456 participants, 512 effects); fresh seeds drawn by the full regressions: 2061983203 (found F-9), 2064003608 300/300 |
| `payment_lab_soak_validation.ts` + global reconciliation | 30 s: 231 deals / 550 participants / 2 079 jobs — provider 452 captures + 97 recoveries = canonical, 5 000 950 minor both sides, ledger fees 472 019 = oracle, DLQ 0, deadlocks 0; 180 s: **PASS** — 1 181 deals / 2 894 participants / 6 846 jobs / 82 lease expiries / 22 interval audits; provider 2 395 captures + 100 recoveries + 394 releases = canonical (399 released = 394 + 5 F-6), 22 489 550 minor both sides, ledger fees 2 122 649 = oracle, net 20 366 901 = oracle, DLQ 9 (visible), deadlocks 0, uncaught/unhandled 0 |
| Phase 1 proofs on the same tree | `cancel_outbox_concurrency_validation.ts` 22/22, `db_client_error_process_survival_validation.ts` 10/10 |
- Static gates: tsc, lint, backend enforcement scan, payment compliance scan, runtime DDL scan (62 files), architecture gate, route authorization gate (static 1 / behavioural 4) — all PASS on the final candidate tree.
- Economic effects across every lab run: DUPLICATE_CAPTURE 0 · DUPLICATE_REFUND 0 · DUPLICATE_RECOVERY 0 · DUPLICATE_RELEASE 0 · FALSE_CANONICAL_SUCCESS 0 · LEDGER_MISMATCH 0 · UNKNOWN_AUTOMATIC_REPEAT 0 · deadlocks 0.

**OPEN**
- Grow native settle/refund idempotency and exact-operation status remain UNPROVEN (sandbox blocked); Grow fails closed by policy.
- A provider that declares a *final* negative status for an operation it executed can only be caught after the fact (F-1 pre-flight / late-effect cases). `payment-late-money-effect` and `deal-finalize-waiting-unresolved` cases are operator action items — no automatic refund exists for money captured on a deal that later failed.
- Status-echo reference trust (O-1), duplicate `charge_deal` after the window → DLQ (O-2), reconcile-before-dispatch pre-emption (O-3) — recorded, benign, for the reviewer.
- Statement timeouts, PostgreSQL restarts and true deadlock injection not simulated.
- Master CI intermittent test-design defects (request-id substring, worker credential substring) are fixed on `claude/ci-request-id-flake-repair` (NOT merged); the original e2e failure on master `60ebf6d` was never reproduced (13/13 in two isolated runs).

**PERCENTAGE**: program 100 % — Phases 0–21 complete, Phase 22 complete — four full regressions (`5e521f3`, `f5ab275`, `3b67821`, `6cec97f`); on the final src every group is green: payments 46/46 incl. a fresh-seed fuzz, db 7/7 ×3 after the proof rewrite, the other eight groups green in regression #4; gates 6/6; long fuzz 2 000/2 000; F-9 seed 300/300; mutations 17 tested / 15 caught. SAFE_FOR_REAL_MONEY = NO. SAFE_TO_MERGE_FINANCIAL_BRANCH = NO. R10 BLOCKED.

**NEXT STEP**: fresh independent adversarial review of the exact candidate SHA `40f94d1` (+ this status commit; src/ identical to `6cec97f`, the tree of regression #4 — later commits are tests/docs only) in a separate conversation and worktree; then Codex re-review (F-1..F-5 unseen by Codex) and the Grow sandbox proof. Owner decisions: merge `claude/ci-request-id-flake-repair` (CI stability) and `claude/pre-financial-resilience` first; keep the financial candidate unmerged.

## FINANCIAL TORTURE PROGRAM — PHASE 0/1 BASELINE + PRE-FINANCIAL RESILIENCE (2026-09-07) — branch `claude/pre-financial-resilience` at `b2d67ff` (base master `60ebf6d`), NOT merged

**COMPLETED**
- Phase 0 — master verified: `origin/master = 60ebf6d64fb0d5909ae98fa335a4a27ad45c9172` (system hardening + route gates + logging/input hardening + concurrent publish/outbox race fix), worktree clean. Hosted staging smoke (`https://siton-staging-web.onrender.com/preview/`) PASS: `/health` 200, preview / seller-login / seller-drafts / inquiries pages 200, `/api/mall/deals` 200, public deal `7f0a0c07` payload + `/d/` OG share + activity 200, seller/draft/inquiry API guards answer 401/404/400 (never 5xx), inquiry POST validates (400 `inquiry_name_required`). No real money, no writes.
- Phase 0 — GitHub CI on master `60ebf6d`: `Web runtime depth gates` GREEN; `Backend and deployment quality gates` **RED at step 29 "End-to-end tests"** (steps 1-28 GREEN, including the Security tests step that was red on `75baa21`; e2e step 98 s; only public annotation is "exit code 1", logs need login). An isolated diagnostic run of `npm run test:e2e` alone on the identical tree (ubuntu-24.04, node 22, postgres 16) = **13/13 PASS in 79 s** → the failure is NOT reproducible in isolation; a full-sequence mirror diagnostic (every group wrapped with failure annotations + artifact upload) is running on throwaway branch `claude/ci-security-diagnostic` (`f8c3d53`, never to merge, delete afterwards). Baseline is therefore NOT declared clean for e2e; classification (deterministic-in-sequence vs infrastructure/flake) is pending that run.
- Phase 1A — deal.cancel outbox race REPRODUCED deterministically (latch at `atomic.after_durable_writes_before_commit`, competitors proven blocked by `pg_stat_activity`): unfixed master answered **500** (23505 on `ux_outbox_one_pending_per_aggregate_event`) in 16/22 scenarios — same key ×2, distinct keys ×2, parallel 2/5/10/25 (deterministic + free-running), sequential re-cancel under a NEW key after a lost response, and re-cancel while the worker holds the cancel_refund in `processing`. Financial truth was never duplicated in any interleaving: exactly one `deal.cancel` audit row, exactly one logical `cancel_refund` (same `event_uuid` across follow-ups), 0 `payment_attempts`, 0 platform-fee money events, 0 money/buyer audit rows, 0 participants (Draft → Cancelled is the ONLY cancel transition; a published deal's cancel is a 409 with nothing written). **CANCEL_DUPLICATE_FINANCIAL_INTENT = 0.** FIX (narrow): `deal.cancel` opts into `serializeOnEntity` (deal row locked FIRST, then idempotency lookup, then state precheck, before any durable write — identical lock order to publish / draft edit / delete). After the fix: 22/22, cross-deal cancels do not serialize on each other.
- Phase 1B — checked-out PostgreSQL client error REPRODUCED: pg-pool removes its client `'error'` listener on checkout; `pg_terminate_backend` on a checked-out client emitted three unhandled client errors (57P01, ECONNRESET, "Connection terminated unexpectedly") at EVERY boundary (before BEGIN, after BEGIN, between statements, before COMMIT, mid-statement, while holding a row lock) — `src/` has no process-level handler, so a real forked child with no handler **died with exit code 1**. FIX (smallest): the runtime pool's `connect` hook attaches a persistent per-client error guard (`src/db.ts`: records + logs `[db.client.error]`, bounded observations exported as `dbClientErrorObservations()`); the owning transaction still FAILS (pg marks the client not queryable, `withTx` rolls back and rethrows, pg-pool discards the dead client on release), nothing is retried and nothing is reported committed that was not (a write before the termination never persists; a held row lock is released immediately). **PG_CLIENT_PROCESS_SURVIVAL = PASS.**

**TESTED** (fresh isolated local databases, no hosted environment touched)
- `tests/cancel_outbox_concurrency_validation.ts` (concurrency group): unfixed 6/22 → fixed **22/22**.
- `tests/db_client_error_process_survival_validation.ts` (db group, with `tests/support/pg_client_error_child_harness.ts`): unfixed 0/10 (20 uncaught exceptions, child exit 1) → fixed **10/10** (0 uncaught, 22 guarded client errors observed, child exit 0, pool serves after every termination).
- Regression on the fixed tree: concurrency group **6/6** (incl. `publish_outbox_concurrency_validation` 21/21), db group **7/7**; `tsc --noEmit`, lint, backend enforcement scan, payment compliance scan, runtime DDL scan (62 files), architecture gate — all PASS.

**OPEN**
- Master CI e2e failure root cause (full-sequence mirror diagnostic pending; if deterministic and attributable to master, a SEPARATE repair branch from exact master will be created; master is never patched silently).
- Residual (documented, not hidden): a termination while COMMIT is already in flight is reported as a failed transaction even if the server committed — honest ambiguity, surfaced as an error, never as success.
- Phases 2-22 of the financial torture program (R9C refresh onto this tip, independent financial oracle, programmable synthetic provider, C1/C2, lifecycle / crash-point / concurrency / idempotency / reconciliation / refund / release / recovery / outbox / DB-fault matrices, economics fuzz, seeded random schedules, soak, global reconciliation, mutation testing, full regression).

**PERCENTAGE**: program 8% (Phase 0 90% — e2e root cause open; Phase 1 100%; Phases 2-22 0%). SAFE_FOR_REAL_MONEY = NO. R10 BLOCKED.

**NEXT STEP**: Phase 2 — fresh branch `claude/r9c-financial-torture-candidate` from this tip; semantic port of the complete R9C payment-lifecycle delta (`claude/r9c-integration-candidate` @ `3f00edb`, 10 commits, dry-run cherry-pick onto `60ebf6d` is conflict-free except `PROJECT_STATUS.md`) preserving 061 (P0.7) / 062 (RESERVED Codex Amazon) / 063 (R9C); then prove master hardening + publish fix + R9C lifecycle + fresh/rerun migrations + no P0.7 regression.
