...
- Tested: the staging SQL passed two full `BEGIN ... ROLLBACK` preflights. GitHub CI on code candidate `f1fa83449bb98b7ba6584a38f55e66aa85e80910` passed both workflows: TypeScript, architecture/enforcement/payment/runtime-DDL gates, clean 45/45 migration replay, unit, integration, database, API, Worker, payment, security, concurrency, failure, E2E, complete repository suite, extended Docker/MinIO/multi-Web/Worker/outbox smoke, and Web runtime core/resilience. The new isolated runtime-role, health/readiness, rollback, audit, idempotency, capacity and fee/VAT tests passed within that run.
