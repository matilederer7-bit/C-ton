## STAGING MIGRATIONS 067/068 — NOT APPLIED, BLOCKED AT THE STAGING WRITE (2026-09-14) — branch `claude/staging-apply-067-068` from exact master `4aaaa73`, NOT merged

- **COMPLETED:** Pre-flight for applying `067_payment_operation_lifecycle.sql` (position 60) and `068_payment_settlement_horizon.sql` (position 61) to Supabase staging `hnptacfzuqebfgeshadq` from exact master `4aaaa73ab287c536e1175460f8b58b24158f8f52` (`git ls-remote` verified; work in a disposable worktree, main checkout untouched). Staging target confirmed through the Supabase MCP project URL (`https://hnptacfzuqebfgeshadq.supabase.co`, PostgreSQL 17.6, session role `postgres` = owner of schema `siton`). **LEDGER_BEFORE = 59 rows, head `066` @ position 59, 0 running/failed rows; all 59 rows match the repository files row-by-row under the runner's own `checksum()` (LF, BOM-stripped) — 59/59, 0 mismatches.** Canonical runner proof: `npm run db:migrate` (`scripts/run_migrations.cjs`) on a fresh disposable local database applied **61/61** (`MIGRATION_OK 067`, `MIGRATION_OK 068`), second pass idempotent (0 new, `MIGRATIONS_COMPLETE count=61`), recorded checksums `067 = d22dd7b611254c79d45c2f00602d795e47078c3c9cb33b4e4977eef0df4a7a8c`, `068 = b426aaa2ba3dcfd8a41aac7a0e42d1d5785fec327d8b204e12cfc130a32be24c` (equal to the git-blob hashes); resulting schema has the 11 new `payment_attempts` columns, 8 functions and 4 triggers expected. Gotcha re-confirmed: this Windows checkout is `core.autocrlf=true`, so the as-checked-out files hash differently (`067 = a0b3a8fc…`, `068 = 394a892c…`) — the runner must run from an LF checkout or it refuses at 066 (`migration checksum mismatch`). Real-money baseline (read-only SQL): every provider reference is `mockpay` (webhook_events 8, platform_fee_money_events 8) or `internal-ledger` (payout batches 6); 0 Grow references; 0 `payment_authorization_bindings`; `render.yaml` on master pins `PAYMENT_PROVIDER=mockpay`, `PAYMENT_PROVIDER_MODE=mock-backed`, `PAYMENT_ENVIRONMENT=demo`. Business baseline: deals 31, participants 81, sellers 7, fulfillment units 9, payment_attempts 42 (40 success / 2 legacy temporary_fail / 0 unknown), outbox 125, DLQ 0, operational cases 33; schema 75 tables / 17 functions / 15 triggers / 253 indexes. Added the paired hosted grant script `supabase/staging/024_r9c_payment_lifecycle_grants.sql` (staging 008/009 model: PUBLIC/anon/authenticated revoked on the seven new SECURITY DEFINER functions; EXECUTE on the two application-invoked helpers `payment_operation_in_flight(uuid,integer)` and `payment_capture_settlement_fence(uuid,uuid)` for `siton_web_runtime`/`siton_worker_runtime`; self-check DO block) — NOT applied.
- **TESTED:** Read-only hosted smoke against `https://siton-staging-web.onrender.com` (pre-migration baseline): `/health` 200 (24.4 s cold start — free-plan idle), `/readiness` 200 `database=connected, runtime_role=siton_web_runtime`, `/api/mall/deals` 200, `/api/deals/1be945e5…/public` 200, `/d/1be945e5…` 200 (OG share page), `/api/deals/…/receipt-info` 200, `/preview/` 200; seller boundary `/api/seller/deals` 401 `SELLER_AUTH_REQUIRED`; admin boundary `/api/admin/outbox-status` 401 `admin_auth_required`. No payment executed; Grow not called; real money 0.
- **OPEN (BLOCKER):** **067 and 068 are NOT applied on staging; ledger unchanged (verified after the denials: 59 rows, head 59, `payment_attempts` still 7 columns, no new functions/triggers).** The canonical runner cannot reach staging from this session: no staging `DATABASE_URL` exists locally (kept out-of-band by design; the classifier refused credential look-ups), the Render MCP that holds the hosted secret is unauthenticated, and the Supabase MCP exposes no connection string. The repository's documented fallback (docs/STAGING_ACCEPTANCE_2026-09-10.md, PILOT_LAUNCH_RUNBOOK.md — DDL + ledger row committed together through Supabase MCP `apply_migration`, as used for 065/066) was **denied by the auto-mode classifier** in this session, as was the runner-shaped alternative (`execute_sql` ledger row `running` → `apply_migration` unmodified file → `succeeded`). No variant was attempted beyond those two; nothing was written to staging. Staging is therefore CODE-AHEAD-OF-SCHEMA: Render runs `4aaaa73` (R9C code expecting the 067/068 columns) against a 066 schema — public/read paths are healthy (smoke above), the payment-attempt write paths will fail until the migrations land. `RENDER_SHA` could not be independently read (no public build marker; Render MCP unauthenticated) — owner-asserted `4aaaa73`.
- **PERCENTAGE:** STAGING MIGRATION 067/068 — 0 % applied (pre-flight 100 %: target, ledger, checksums, runner proof, grant script, baseline, smoke). Real money 0; Grow not called; payment-provider configuration, Render and Supabase auth untouched.
- **NEXT STEP:** Owner applies the two migrations with the canonical runner from an LF checkout of `4aaaa73` using the owner-held `postgres` staging connection string (`DATABASE_URL=<staging postgres URL> npm run db:migrate` — expected output exactly `MIGRATION_OK 067`, `MIGRATION_OK 068`, `MIGRATIONS_COMPLETE count=61`; the runtime login roles cannot ALTER the postgres-owned tables, so the Render container is not a valid place to run it), then applies `supabase/staging/024_r9c_payment_lifecycle_grants.sql` in the Supabase SQL editor; or re-runs this task with the Supabase MCP write (`apply_migration`) allowed for the two files + the grant script. Post-apply verification to run (read-only): ledger `067 @ 60` / `068 @ 61` succeeded with the checksums above, 0 dirty rows, 11 new columns + 7 new functions + 4 new triggers present, business counts unchanged (31/81/7/9/42), providers still mockpay/internal-ledger only, then the hosted smoke above plus one mock-backed join → capture on a synthetic deal.
