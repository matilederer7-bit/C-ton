## CLOSED PILOT WAR GAME (2026-09-09) — branch `codex/closed-pilot-war-game`, NOT merged

**COMPLETED:** Deterministic local closed-pilot proof: 5 synthetic sellers, 12 deals, 130 verified buyer identities / 120 participations, 40 concurrent attempts at capacity 30, one threshold crossing, zero oversell/duplicate participation/orphans. Physical pickup and shipping, voucher/ticket, seller isolation, pause/reopen/cancel, successful/failed/recovery states, support, exports and admin truth exercised. A genuinely new app process preserved canonical rows, all 120 tracking links, idempotency, fulfillment, 2 operational cases, 320 viral events, 120 attributions and 16 metric scopes. Real worker stop/restart drained 200 safe jobs exactly once while web stayed available; 11 future jobs stayed scheduled; separate two-worker fault proof passed lease expiry/reclaim/fencing. One blocker fixed: admin overview included released orders in settlement totals (1400 instead of 1260); a seven-line backend read-model diff now counts only ChargedSuccess/RecoveredCharge. No payment code or UX change.

**TESTED:** `node scripts/closed_pilot_war_game.cjs`: **39/39 phases PASS**, **28/28 selected suite files PASS**, including TypeScript, lint/backend, runtime-DDL, architecture/payment static gates, fresh migration install + rerun/ledger, unit/API/integration/security/concurrency/workers/deal-type fulfillment. 240 checks of the unchanged 8% fee including delivery/excluding authoritative buyer VAT; distributor commission 0. Four harness refusal controls PASS. Hosted GET-only preview + JS/CSS + health/readiness all 200. Exact inventory, timing, pre-fix reproduction and limitations: [report](docs/CLOSED_PILOT_WAR_GAME_REPORT.md). No full R9C torture/fuzz or mobile campaign. One existing intermittent hash-regex test false positive documented; assertions unchanged; final campaign green.

**OPEN:** No remaining pilot blocker in the exercised local scope. The existing owner console readiness actions and real-money blockers remain unchanged; hosted proof did not verify an exact deployed SHA or deploy this fix.

**PERCENTAGE:** CLOSED WEB PILOT readiness **98%** (retained from master; operational assessment, not a reliability probability). REAL MONEY readiness **0%**, separate and unchanged. Real money / email / SMS **0**; hosted database changes **0**.

**NEXT STEP:** review and merge the isolated admin read-model fix with its reusable pilot proof.
