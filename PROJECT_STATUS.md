...
Current merged baseline: `72651b2da2152368b43eb39eabc805257b668f90`
...
- UPDATED: 2026-09-17
- BRANCH: `claude/long-horizon-closeout-20260917` (closeout on top of merged master `72651b2`); implementation branch `claude/long-horizon-current-master-20260917` merged as PR #42
- COMPLETED: Issue #40 executed end to end and CLOSED OUT. LONG_HORIZON_DEALS was rebuilt on current master as migration `071_long_horizon_authorization_renewal.sql` (CMS keeps 069, Seller Distribution Hub keeps 070) and merged to `master` at `72651b2da2152368b43eb39eabc805257b668f90`. Old PR #24 was provenance only — 3-way reconciled, never merged or blind cherry-picked — and is now CLOSED as superseded after verifying its files are byte-identical on master apart from the 069->071 renumbering. The 7-day maximum deal duration is gone from every active surface (seller picker, hint, validator, Hebrew errors, backend validator, legacy `frontend/app.js`); the 2-hour minimum stands; a 20-year technical sanity ceiling and an advisory-only >1-year notice remain. The full renewable-authorization architecture is preserved, not a UI fix: shared pinned deadline policy, renewable authorization metadata on `payment_authorization_bindings`, durable `reauthorize` identity, capture/recovery/release blocked while a renewal is unresolved, expiry as a payment-maintenance condition that never fails a deal or drops a committed participant, atomic renewal application, reconcile path, and lease/idempotency protection. Migration 071 was applied to Supabase staging and the staging ledger was brought into full agreement with the manifest (069/070/071 at positions 62/63/64; they had previously been applied as raw SQL without ledger rows). Canonical docs that still described the cap as pending were reconciled, and the historical spec quote in the closed drift map was marked DEPRECATED rather than rewritten. A permanent regression suite, `tests/long_horizon_staging_smoke_validation.ts`, now pins the 30/60/90-day journey, the short-deal regression, both rejection boundaries, the built-bundle copy scan and the money/Grow/messaging zero assertions.
- TESTED: PR #42 CI green on all six jobs on head `57d6637`. Canonical verifier: release-static PASS, migrations-isolated PASS, route-authorization PASS, grouped repository suite 243/243 test files PASS. Staging after 071: high-water `071`, 64 ledger rows all `succeeded`, positions contiguous 1..64, manifest comparison DRIFT=0, 79 tables, all five binding columns, `reauthorize` constraint, renewal-shape constraint, renewal index and eligibility trigger present. Render staging LIVE on the merge SHA (`dep-dalum9vqj5pc73ebfl20`) answering `/readiness` 200 after the migration. 30/60/90-day smoke through the real HTTP API on the merged build: 10/10 PASS (Draft create, exact deadline persistence, publish, public re-read exact, all three discoverable in the Mall, ordinary 24h deal unaffected, <2h rejected, >ceiling rejected, 87 built assets carry no 7-day copy). The same shapes were accepted by the STAGING database in a rolled-back transaction with nothing persisted. REAL MONEY 0; Grow untouched and still exposing no `reauthorize`; the smoke created no payment attempt, reauthorization, binding, fee event or notification attempt.
- OPEN: An authenticated seller BROWSER smoke against hosted staging was not run from here — this container's egress policy answers 403 to CONNECT for `siton-staging-web.onrender.com:443`, so hosted HTTP is unreachable; the 30/60/90 proof was obtained against the merged build and directly against the staging database instead. Grow real-provider stored-instrument re-authorization remains an unproven external sandbox gap by design. Issue #39 is untouched and is the next lane. PR #38 Product Catalog still needs migration 072 on reintegration.
- PERCENTAGE: Issue #40 100% (merged, staged, smoked). Long-horizon track 100% for repository and staging scope; production remains intentionally out of scope.
