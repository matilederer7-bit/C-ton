...
- Verdict: `R6_DEPTH_HOSTED_PROVEN`. **SITON PREVIEW URL: https://siton-staging-web.onrender.com/preview**
...
- **Real hosted money loop closed**: the strawberry showcase deal ran TargetReached→Closed→Ready→Charging on the live Worker; the FIRST live charge exposed a genuine R2 gap (worker lacked `webhook_events` — 42501 → retried into the 3-per-30m cap SN429 → DLQ) — fixed in `supabase/staging/014`, event requeued, charges captured (3 participants, 4 units, ₪220 gross, ₪20.76 platform fee rows in the canonical ledger), completion window aged (time-fixture only, mirroring the repo's own test technique), Worker finalized to **Completed**, viral recompute now reports charged truth: 4 charged units / ₪220, of which 3 units / ₪165 attributed to share chains at max generation 2. Admin overview cross-checks: fee_actual ₪20.76 vs projection ₪1,114.48 under separate labels, notifications 75 sent, DLQ 0, worker heartbeat fresh. Temporary proof-admin identity removed; `admin_users` is empty until the owner claims it.
...
- Open (hosted phase): deploy to staging → seed synthetic showcase catalog through the REAL hosted flows (`scripts/r6_staging_showcase_seed.cjs`: synthetic seller, ~8 deals with generated imagery, viral join chains, one deal walked through real mockpay charging) → browser + API proofs → cold/warm startup measurement (dominant suspicion: Render free-plan sleep; warm TTFB measured 0.2–0.4s) → CI.
- Estimated R6-depth completion: backend ~95%, public UX ~85% of spec depth (chat/live/share/join/track done; OTP+payment iFrame remain parked by policy), seller UX ~80% (voucher/ticket wizard steps and bulk delivery-status updates pending), admin ~75% (all 11 screens live; KYC queue/payout retry/action-execution UIs still lean on legacy `/app/admin`), virality ~90% (personal impact, metrics, tree, recompute live; distributor console UI still legacy).
