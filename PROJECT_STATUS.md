## PR #19 CLOSEOUT — INTEGRATION FLAKE ROOT-CAUSED AND FIXED (2026-09-16) — branch `claude/affectionate-wright-k1vdhv` from master `3e55aa5` (the PR #19 merge commit)

- **COMPLETED:** PR #19 was merged by the owner at 06:24 UTC as master `3e55aa519eef235ccbdf8537424dff97045f01aa`; it is the single merge line, PR #18 was not merged separately. The one CI failure recorded against the PR head (`backend-quality-gates` run 230 attempt 1, integration step, `stage32c_product_surface_closure_validation.ts`: affiliate clicks expected 2, actual 1; attempt 2 on the same SHA passed) was root-caused as a **product defect**, not test pollution and not a stale assertion. `POST /api/affiliate/links/visit` in `src/frontend_runtime.ts` called `reply.code(202).send(...)` *inside* `deps.withTx`, so the client received `recorded: true` before `COMMIT`. Fastify completes `app.inject()` on `send`, so the test's very next read (`GET /api/affiliate/overview`, its own transaction, READ COMMITTED) could observe the previous click count while the second `INSERT` was still uncommitted; the same window exists for a real browser navigating on immediately after the visit beacon. The CI log shows the second visit answering in 10 ms versus 2.5 ms for the first, and the overview request starting in the same millisecond the second visit responded. **Fix (minimal, one handler):** the transaction now returns `{ recorded }` and the 202 is sent after `withTx` resolves, mirroring the already-correct `POST /api/viral/events` shape. No SQL, schema, auth, or contract change; the response body is byte-identical.
- **TESTED:** Regression test **`tests/affiliate_visit_response_after_commit_validation.ts`** written first and **observed failing on the unmodified handler** with the exact CI symptom (`visit answered 202 {"ok":true,"recorded":true} before its transaction committed; a reader that trusted the acknowledgement saw clicks=1`), passing after the fix. It parks the second visit at the `db.before_commit` block fault (no sleeps), asserts the HTTP response has not been produced while the transaction is open, asserts no uncommitted click leaks into the overview, releases the barrier, and asserts the committed click is visible to the very next read. Then: focused `stage32c_product_surface_closure_validation.ts` PASS · `tsc --noEmit` clean · `npm run lint` + `scan:backend` PASS · `scan:payment` PASS (318 files) · `scan:runtime-ddl` PASS (66) · `gate:base44-canonical-integrity` findings=0 · `build:demo` OK · `ARCHITECTURE_GATE_PASS` · `MOBILE_GATE_PASS` · `ROUTE_AUTHORIZATION_GATE_PASS static=1 behavioural=4` · `git diff --check` clean · complete repository suite `npm run test:all` against real Postgres 16: **233 files, 9/10 groups green on the first pass — unit 16/16 · integration 34/34 (includes the new test and stage32c) · db 8/8 · api 45/45 · workers 14/14 · payments 43/43 · security 41/41 · concurrency 9/9 · failure 9/9 · e2e 12/14.** The two e2e failures were `frontend_browser_smoke_validation.ts` and `frontend_browser_v11_validation.ts`, both failing in under 4 s with `browser executable not found at /usr/bin/google-chrome` before any browser assertion ran. Classified **environmental and pre-existing, not caused by this change**, by evidence rather than assumption: (1) the identical two failures reproduce on a pristine `3e55aa5` master worktree in this same container; (2) with a container-local Chromium wrapper (root sandbox flag, outside the repository, not committed) both proofs **pass on the fixed head** — smoke 14/14 assertions including *distributor creates a named attribution link* and *distributor sees the browser join attributed to the named link*, v11 8/8 — so the e2e group is **14/14** on this head and the whole suite is **233/233**. Docker smoke (`ci:docker-smoke`) is environment-blocked here (no daemon) and is covered by GitHub CI.
- **OPEN:** **PUSH BLOCKED — O-7 unchanged.** `git push` to `claude/affectionate-wright-k1vdhv` returned HTTP 403 twice ("Claude doesn't have GitHub access to matilederer7-bit/C-ton for your organization") and the GitHub API `create_branch` returned 403 "Resource not accessible by integration": the Claude GitHub App still has read-only access, exactly as recorded on 2026-09-15. Not retried further on owner instruction. The commits are delivered as a single `git am` patch series based on master `3e55aa5` (see NEXT STEP); GitHub CI has therefore not run on this head. **Money and Grow untouched, confirmed by diff:** the runtime change is 7 insertions / 3 deletions in one measurement-only handler; `src/grow_payment_adapter.ts`, `src/payment_provider.ts`, `src/payout_rail.ts`, `src/platform_fee_money.ts`, `src/vat_authority.ts`, `src/webhook_ingestion.ts`, `src/app.ts` and `config/real-money-release-policy.json` have a zero-byte diff; no notification path, no migration, no auth/ownership check, no outbox `event_uuid` and no idempotency logic was changed. REAL MONEY remains 0. The same send-inside-transaction shape remains in two other handlers, deliberately **not** touched by this fix: `POST /api/deals/:dealId/chat` (`withTx` opened at `src/frontend_runtime.ts:3542`, 201 sent inside) and `handleWebhookInvoices` (`withTx` at `:6166`, 200/202 `duplicate`/`ignored`/`queued` answers sent inside — a provider could be told "queued" before the durable claim commits, which is a webhook-durability concern on a money-adjacent path and deserves its own concurrency test rather than a drive-by change). Master `backend-quality-gates` run 237 on the merge commit was still `in_progress` when this branch was prepared; the fix here does not depend on its outcome. Everything else already recorded stays open: O-1 Render Blueprint/live worker command sync, O-2 hosted `OTP_HASH_SALT`, F13 provider semantics, F-12 image pruning, F-07 architecture decision, LONG_HORIZON_DEALS, legacy server-emitted `/app/...` URLs and the canonical React recovery screen. REAL MONEY remains 0. Grow remains untouched and blocked.
- **PERCENTAGE:** Technical readiness for the closed-pilot product path **91%** (the integration gate now has a proven, deterministic guard instead of a re-run; +1 from 90). Closed-pilot readiness **92%** unchanged. Real-money readiness **35%** unchanged — no money-path code was touched.
- **NEXT STEP:** Owner applies the patch series with `git am` onto master `3e55aa5` (or grants O-7 write access and the branch is pushed as-is), opens a small PR, lets GitHub CI qualify it (the new test runs in the integration group), and merges after green. Then apply the same after-commit acknowledgement to the invoice webhook route with a barrier test of its own. Then the documented owner/runtime actions (O-1, O-2) before any hosted pilot or real-money activation.
...
## GITHUB PATCH TRANSFER + PR #18 — O-7 DELIVERY BLOCKER CLOSED (2026-09-15)

- **COMPLETED:** The adversarial-review handoff was transferred from the owner-supplied `git am` patch to GitHub without reconstructing repository code by hand. The source patch was applied to exact base `f2121f60cc37d742e2bf32d0ca48fe2ec83a807d`; GitHub now contains the two review commits as `0953b8bee45a493dbe419bd525776da5928f0aaa` and `56120ab8440a1d037dd8aa3c8143215b63879403`. PR **#18** is open against `master`. O-7 is therefore closed as a delivery blocker for this branch. This does not claim that Claude's own GitHub App permission changed; the transfer used the owner's write-enabled ChatGPT/GitHub Actions path.
- **TESTED:** Original patch size **145,178 bytes** and SHA256 `eb6ff3c3ce34e203f147f8c5ef3421e148daa5841738fb375251adcc9e618153` were verified before application. GitHub Actions reconstructed the transferred patch and re-verified that SHA before `git am`. The first applied commit produced tree `3de0db48b3405f2524bd68fd1c6685ffbabb117c`; the second produced final tree **`25476f2c4ae8d5c798e5889180aca2c2fc787e95`**, exactly matching the independently supplied expected tree. At PR creation, `master` was still exact base `f2121f6`; the branch was 2 commits ahead and 0 behind. PR #18 contained **25 changed files, 860 additions, 109 deletions**.
- **OPEN:** GitHub CI must still be evaluated on the final PR head before any merge decision. **O-1 remains open:** the live Render worker was independently observed still configured as `npm run start:worker:prod`, so the hosted graceful-drain fix is not yet applied. **O-2, O-3, O-4, O-5 and O-6 remain open. F13 remains external and unresolved. LONG_HORIZON_DEALS remains unsolved and blocked on proven provider semantics. REAL MONEY remains 0 and Grow must remain unactivated.**
- **PERCENTAGE:** Technical production readiness **88%**, closed-pilot readiness **92%**, real-money readiness **35%** — unchanged by a Git transport operation. GitHub delivery of the adversarial patch itself is **100% complete**.
- **NEXT STEP:** Inspect every required GitHub workflow on the final PR head. `SAFE_TO_MERGE` may become YES only if the required CI is green. Do not merge from an agent session. After owner merge, perform O-1 Render Blueprint/live-command sync and O-2 `OTP_HASH_SALT` before the next pilot deploy; then verify hosted behaviour. F13 and LONG_HORIZON_DEALS remain blocked until provider semantics are proven.
...
- **COMPLETED:** A deliberately hostile production-grade review of master `f2121f60cc37d742e2bf32d0ca48fe2ec83a807d` (master had moved on from the briefed `903175c4`; the later SHA was used and recorded). Full report: **`docs/SENIOR_ADVERSARIAL_REVIEW.md`**. **18 findings: 0 P0, 4 P1, 8 P2, 4 P3, 2 INFO.** All 4 P1s that are repository-owned are FIXED; the 4th (F13) is external by definition. Fixes, each minimal and evidence-led:
  ...
  - **F-03/F-04/F-16 (P1) — six documented startup-config gaps were defended only by the PRE-DEPLOY release gate.** An env var can change in the Render console long after that gate last ran; the boot guard is the only control that re-evaluates every start. Worst case: **every production control hangs off `productionMode()`, which reads one variable, and the Dockerfile defaults `APP_DEPLOYMENT_MODE=demo-preview`** — a hosted service that never declared it boots as a demo (demo seller context, mock payment routes) on the real hostname with every guard silently off. Also closed: placeholder `ADMIN_API_KEY` accepted; `TRACKING_LEGACY_COMPAT=1`; `DEBUG_SURFACES_ENABLED=1`; `OTP_TEST_BYPASS_CODE`; `SUPABASE_SERVICE_ROLE_KEY`; and **`OTP_HASH_SALT` falling back to the literal `siton-otp-salt-default` published in this repository** (a 6-digit OTP hashed with a public salt is recoverable in ~10^6 HMACs). A matching placeholder/length check was added for `SELLER_SESSION_SECRET` (same defect class: a placeholder value means forgeable seller sessions). Method: the six matrix cases were flipped to `runtime_expect: "reject"` **first**, producing `STARTUP_CONFIG_MATRIX_FAIL fail=6` against the live guard, then `src/production_guards.ts` was hardened. **`STARTUP_CONFIG_MATRIX_PASS cases=23 runtime_gaps=0`** (was 6). Non-regression proven explicitly, not assumed: real hosted staging web **ACCEPT**, hosted staging worker **ACCEPT**, `docker-compose.yml` demo stack **ACCEPT**, release-lab worker **ACCEPT**, hosted host that forgot `APP_DEPLOYMENT_MODE` **REJECT**. The hosted-marker design (`RENDER`/`RENDER_EXTERNAL_URL`/`RENDER_SERVICE_ID`) was chosen precisely so the local labs, which legitimately run `NODE_ENV=production` + `demo-preview`, keep booting.
  - **F-05/F-06 (P2) — LOG-1: buyer PII in hosted logs, behind a vacuous check.** `recipient_ref` (buyer phone / seller support email) was logged raw and staging runs `NOTIFICATION_PROVIDER=log-only`, so it reached Render's log store. The logging gate *had* a runtime probe for this whose PASS message claimed *"notification log provider emitted recipient_ref"* — running it showed it emitted **nothing**: the probe used `template_key: "buyer_joined_authorized"` where the real key is `buyer_joined_authorized_he`, so `renderNotification()` returned `null`, the provider answered `"skipped"` without logging, and the check passed on an empty string whether or not the value was masked. Masking added (on the **shape** of the value, since a phone number is routinely stored against `whatsapp_link`); the probe now sends sms + email cases with distinct sentinels and emits `PROBE_INERT` if a send produces no line; the gate fails on a raw sentinel, on `PROBE_INERT`, **and** on absence of the masked forms. **Mutation tested**: reverting the mask makes the gate FAIL. `LOGGING_HYGIENE_GATE_PASS pass=6 warning=0` (was `warning=1`).
  - **F-08 (P2) — the operator dashboard contradicted the guard.** Mission control recomputed `legacy_links_allowed` from a *second, different* expression than `trackingMode()`. Measured across five configurations, **three diverge**, including the dangerous direction: with `TRACKING_LEGACY_COMPAT=1` the runtime **allows** anonymous participant tracking links while the security panel reported them **blocked**. It now calls `trackingMode()` — one source of truth — and raises a blocker.
  - **F-10 (P2) — GAP-HTTP-1 closed.** `/readiness` (the Render health-check path) had no `cache-control: no-store`; a cached readiness verdict is a stale verdict. The smoke test had *excused* `/readiness` from `expectNoStore` and downgraded it to a non-failing note; it is now a hard assertion. **Mutation tested.** `HTTP_SECURITY_SMOKE_PASS 11/11`.
...
- **OPEN:** (see also the standing item **LONG_HORIZON_DEALS** recorded immediately below — the runtime 7-day limitation is NOT solved and is blocked on F13/provider semantics)
  ...
  - **F-14 (P3)** — `verifyParticipantTrackingAccess`'s "constant-time" compare compares a value **to itself**; the branch is unreachable. Not a vulnerability (the real lookup is a SHA-256 index equality on a 256-bit token) but it reads as a control and is inert.
  - **F-18 (P3)** — webhook `claimEvent()` is check-then-insert with no row lock, so a **concurrent** duplicate delivery raises a unique violation (5xx) instead of answering `duplicate`. The safety property holds for the right reason — `PRIMARY KEY (provider,event_id)` makes a second row impossible, so the event is never processed twice — and the provider's retry then gets the correct answer. Noisy, not wrong. Exact fix recorded (`ON CONFLICT … DO NOTHING` + re-read); not applied because it changes money-path behaviour and deserves its own concurrency test.
  - **F-15 (P3)** — `src/frontend_runtime.ts` 11,970 lines, `src/app.ts` 7,435 (45% of the TypeScript). The clearest structural "built by AI" signal. Real maintainability risk, not a correctness defect; splitting it is explicitly out of scope.
