...
- Completed: upgraded the existing admin frontend with an **Admin Mission Control / מרכז שליטה תפעולי** section: green/yellow/red system status, manual refresh, stale-data badge, cards for exceptions, internal admin-only search, exceptional deal cards, Seller Onboarding / KYC table, Audit & Forensics table, and clear boundaries that admin cannot perform state override, capture, refund, void, or direct payout from the UI.
...
- Checked regression: frontend syntax, TypeScript build, product surfaces, RTL/accessibility, frontend flow, seller profile readiness, seller auth, seller analytics, deal duplicate, seller Excel export, spec drift, platform-fee 8% model, seller payout rail, invoice rail, Morning adapter, and payment authorization env guard.
- Open: real browser visual QA for the admin console, staged deploy smoke, deeper RBAC/MFA, CSV export for audit, real alerting, and external KYC/provider integrations.
