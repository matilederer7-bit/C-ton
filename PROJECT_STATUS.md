## INDEPENDENT REVIEW — CODEX CLOSED PILOT WAR GAME (2026-09-09) — branch `claude/review-codex-war-game` from exact master `d4c7877`, reviewing `codex/closed-pilot-war-game` @ `6fa0e62`, NOT merged

**COMPLETED:** Source verified against the remote (Codex tip `6fa0e62`, 3 commits ahead of master, 0 behind, merge-base `d4c7877`, one production file `src/frontend_runtime.ts` +5/−2, no migration/Supabase/Render/payment/Grow/mobile/web change). **Settlement blocker P-1 reproduced independently on unmodified master** with a wider matrix than Codex used (ChargedSuccess + RecoveredCharge counted; AuthReleased/Dropped, Refunded, ChargeFailedRecovery and a captured order on a Failed deal excluded; qty 1/2/3; delivery 0/20): the overview reported joined money **600** where success money is **280**; with the Codex fix it reports 280 and `joined_units` keep their inventory meaning. The fix uses the canonical success predicate (`money_state IN ('ChargedSuccess','RecoveredCharge')`) already used by fulfillment, receipts, exports, seller analytics, mission control and the fee ledger — **SETTLEMENT_FIX_CORRECT = YES**, cherry-picked verbatim onto this branch for test execution. **Two findings the war game could not see, both fixed minimally here:** D-1 — `summarizeMoney` (admin overview `platform_fee_amount`, seller deal `receipts_surface.summary`) accepted but ignored the buyer-VAT amount, so under `SITON_VAT_MODE=explicit` (required for real money) both surfaces overstated Siton's fee (26.43 shown vs 22.40 canonical for 280 gross at 18% VAT; invisible under the pilot's synthetic-zero VAT — the 240 Codex fee checks exercised `calculatePlatformFeeMoney`, which is correct, not this helper) → fee base now = gross − VAT, seller detail passes its VAT through the VAT authority; H-1 — `scripts/deny_external_network.cjs` intercepted only `net.connect`/`fetch`, so `https`, raw sockets and `pg` bypassed the "external-network denial" the report leans on → hardened at `net.Socket.prototype.connect` (https/raw/pg to a non-local host now refuse; local Postgres still connects). **Proof-class audit (docs/CODEX_WAR_GAME_REVIEW.md §4.2):** real runtime = sellers/deals/publish, 130 OTP identities, 120 joins, the 40-request burst (exactly 30 succeed, one target-reached audit), replay idempotency, cross-seller refusals, pickup resolve/handoff, exports vs DB, viral attribution, a genuinely separate app process for the restart phase, a real standalone worker killed and restarted (200 jobs exactly once); synthetic fixture = the two "Completed" deals (trigger-legal SQL state walks, no charging) — labelled as such by Codex, correct for read-model/fulfillment-projection assertions, not a payment proof; lifecycle/voucher/ticket/lease-reclaim proofs come from the listed existing suites (`deal_types_e2e`, `full_e2e_gate`, `charging_completion_window`, `worker_two_process_fencing`), not from the scenario (the worker phase kills between jobs, so no in-flight lease is reclaimed there). **Gap closed on this branch:** the burst ran the LEGACY inventory path (`CANONICAL_POSTGRES_RUNTIME` unset in the harness and in the local runner) while hosted staging runs the canonical Postgres inventory RPC; no local suite drove concurrent joins on that path → new `tests/canonical_inventory_concurrency_validation.ts` applies `supabase/staging/001` locally and proves 40 concurrent joins on the canonical RPC path: exactly 30 succeed, ledger == business rows, one threshold crossing, replay/no-orphan. **Hosted:** GET-only; `/health`, `/readiness` 200; `/api/preview/meta` exposes `runtime_commit_sha = d4c7877` — the exact hosted SHA IS verifiable (Codex said it was not): staging runs master, not the Codex branch, as expected for an unmerged fix.

**TESTED:** Codex war game reproduced at `6fa0e62`: **39/39 phases PASS** (my run 211.6 s), refusal controls 6/6, and again **38/38 under the hardened hook** with zero egress attempts. `npm run test:all` at `6fa0e62`: 206/207 files — only `worker_two_process_fencing_validation` timed out under full-suite load and then passed 2/2 in isolation (documented runner-flake history; unrelated to the overview change). Review branch `test:all`: 209 files, all groups green after one fixture correction in the new canonical suite (api 45/45 incl. `admin_settlement_success_money_validation` 8/8 — FAILS on master 600≠280 and on Codex for the VAT steps A5/A6 by design; concurrency 8/8 incl. `canonical_inventory_concurrency_validation` 5/5 — 40 concurrent joins on the canonical RPC path, exactly 30 succeed, ledger == rows, one threshold crossing). Static gates: lint, payment scan, runtime DDL, architecture, diff --check PASS. Hosted GET-only: health/readiness 200, `runtime_commit_sha = d4c7877`. REAL MONEY 0, REAL E-MAIL 0, REAL SMS 0, HOSTED DB CHANGED 0.

**OPEN:** merge decision for Codex is YES (see report); D-1 and H-1 live only on this review branch and need their own review/merge (D-1 is required before real money / explicit VAT, not before the dry pilot); `worker_two_process_fencing_validation` remains timing-sensitive on a loaded workstation (documented runner flake history, PROJECT_STATUS 2026-09-06); the overview's `LIMIT 100` deal window (Codex F-1) is unchanged; the canonical-path proof applies the staging inventory SQL from `supabase/staging/001` inside the test — it is a local proof of the same RPC, not a hosted concurrency test.

**PERCENTAGE:** CODEX WAR-GAME REPORT ACCURACY **90 %** (settlement fix, phase inventory, refusal controls, restart/worker/durability claims and the real-vs-synthetic labelling all verified; overstated: "external-network denial" transport coverage, "inventory protection" silent about the legacy path, "hosted SHA cannot be verified"). CLOSED WEB PILOT readiness unchanged at **98 %**. REAL MONEY readiness **0 %** — unchanged and separate (D-1 is one of the items on that path).

**NEXT STEP:** merge `codex/closed-pilot-war-game` (P-1 fix + harness), then review/merge this branch's D-1 + H-1 + the two new suites; re-run the war game on the merged master before the pilot's first real seller.
