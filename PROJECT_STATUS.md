## FINAL FINANCIAL INTEGRATION — PHASE 2 RESIDUAL CLOSURE (2026-09-08) — branch `claude/r9c-final-financial-integration` (from exact master `8ead7c8`), NOT merged

**COMPLETED**
- Phase 0/1 (commit `91b5821`): baseline `origin/master = 8ead7c828e6d6233bf7d17bf7f67d1a67ad35767` verified (Codex independently repaired baseline; GitHub backend-gates RED only at step 30 "Complete repository suite" — CI-only, financial-independent, NOT folded in); the 30 reviewed financial commits (`e829e79 … 921b85b`) semantically ported with `cherry-pick -x`; Codex's cancel serialization / PG client survival / PG error-logging safety / request-id / credential-leak baseline preserved (equivalence: src differs from `921b85b` only by Codex's `safeCodes` hunk). Migrations 061 · 062 RESERVED (Amazon) · 063 · 064, no renumbering.
- Residual A (provider finality) CLOSED with DB authority: migration 064 (extended IN PLACE — never applied outside disposable databases) records `payment_attempts.negative_finality_authoritative` at dispatch from the provider policy (NULL = unproven/legacy); `siton.payment_capture_settlement_fence()` returns `'infinity'` when the horizon is NULL or the authority is not `true`; the INSERT trigger refuses recovery/release identities while fenced; recovery/release rails and the terminal finalize decision open `payment-recovery-/payment-release-/deal-finalize-negative-finality-unproven` cases and stop (no deferral: waiting creates no proof). Horizon expiry alone never authorises recovery; the pre-flight requires exact evidence (`dispatch_response` | `operator`) or an authoritative contract AND ties every read to the exact operation (adapter-judged `reference_matches_query`, currency). `PAYMENT_NEGATIVE_STATUS_AUTHORITATIVE=false` forces the provider-ready rail fail-closed (lab model of Grow; Grow itself unchanged, fail-closed by construction). Operator way out: `failure_evidence='operator'` → inside the completion window exactly one recovery once the recovery job is re-armed; past the window the terminal decision releases WITH proof, never captures.
- Residual B (legacy NULL horizon) CLOSED: NULL is never "elapsed" (permanent fence); 064 backfills `settlement_horizon_at = dispatched_at + 24 h` where the dispatch instant is durable (authority stays NULL → still fenced); negative control (NULL-as-elapsed) is a red mutant (M33).
- Residual C (capture vs release) CLOSED: `siton.payment_release_conflict()` + INSERT trigger refuse `charge_start`/`recovery` while a release identity is unknown or executed; the charge rail defers (`charge_held_behind_unresolved_release`) / applies `AuthReleased` with a case; new legal transition `ChargeAttempt → AuthReleased` (app + `siton.is_valid_money_transition`) only when no capture-side identity is unknown/executed.
- Integration-found defect I-1 fixed: the finalize horizon deferral truncated its retry instant to whole seconds (`String(Date)`), so the outbox could refuse it as already past (`outbox_lease_lost`, decision delayed by a lease, intermittent H-6/H-6b). Exact instant kept now; horizon suite deterministic (3/3 consecutive isolated runs green; 2/3 red before).
- Lab: seed defaults model dispatched rows (exact evidence, authority from env, horizon, `dispatched_at`; explicit `null` = legacy), simulator status scripts accept `persist`, mutants M32–M35 added (anchors of M18/M25 refreshed; all 35 anchors verified against the tip).

**TESTED** (fresh isolated databases, Windows Node 24): `tsc` clean; Phase 2 financial set **23/23 files** — residual A-unproven AU-1..9 + AU-9b (10), A-authoritative AA-1..7 (7; AA-2/AA-7 = documented contract boundary), B LB-1..7 (7), C RC-1..9 (9), lab c1_c2 27, crash matrix 13, finalize guard 3, foundation 23, lifecycle/reconcile 37, refund/release/recovery 29, terminal economics 17, review adversarial 20, findings reconstruction 13 (FR-C4 rewritten to the closed residual), Grow negative status 2, mock provider truth 1, settlement horizon 12, R9C ambiguous / reconciliation race / stale-owner settle, Grow ambiguity policy, provider operation-identity crash, recovery real rail, release lifecycle — all green. Codex baseline proofs (Phase 1 tip): cancel 22/22, PG survival 11/11 + log safety, request-id 9/9, ci_log_security 181, two-process fencing PASS.

**OPEN**: Phase 6 anti-vacuity mutations (all 35 mutants incl. the owner list: horizon off, horizon-expiry-alone, NULL-as-elapsed, release fence, reference check, currency, AuthReleased proof, R-11 guard, UNKNOWN→retryable, fee 7 %/9 %, VAT, delivery, distributor); Phase 7 fresh fuzz ≥ 1 000 scenarios (+ known F-9 seed 2061983203); Phase 8 soak 5–10 min two workers + global reconciliation; Phase 9 ONE exact-tip full sequential regression (10 groups + all static/build/route/mobile-verify gates + migration proofs); Phase 10 final docs (`docs/R9C_FINAL_FINANCIAL_INTEGRATION.md` §6/§7) + report. Real Grow automatic recovery stays disabled / fail-closed (negative finality unproven; sandbox userId/pageCode still blocked externally).

**PERCENTAGE**: 55 %. SAFE_TO_MERGE = NO. SAFE_FOR_REAL_MONEY = NO. BEHIND_MASTER = 0.

**NEXT STEP**: run the 35 mutants on the committed tip, fuzz + soak, then the single exact-tip regression; finish docs; hand `FINAL_SHA` to Codex Astra for the final financial review (then the Grow sandbox proof — external access still blocked).
