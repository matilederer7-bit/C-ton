...
## R9C — CONTROLLED INTEGRATION CANDIDATE (2026-09-04) — BRANCH claude/r9c-integration-candidate (NOT MERGED, NOT DEPLOYED) — STATUS: CANDIDATE BUILT, TWO INDEPENDENT REVIEWS REQUIRED

**COMPLETED: the full R9C payment-safety changeset (7 commits, `f025d3f`) ported onto current master `123bbf9` in a clean worktree — durable provider-operation identity, lease fencing, DB-enforced money-operation lifecycle (migration 063), participant-wide in-flight reconciliation deferral, post-dispatch ambiguity = UNKNOWN in every adapter, per-provider ambiguity policy (Grow fails closed), recovery/refund/release eligibility, state+fee-ledger atomicity, 9 proof files — plus a fresh adversarial self-review of the INTEGRATED code that found and fixed ONE NEW CRITICAL (SR-1) and ONE MEDIUM (SR-2). Master is untouched (still `123bbf9`, clean). TESTED: whole repository suite GREEN on the patched candidate - `npm run test:all` 181/181 files, 10/10 groups (unit 12, integration 29, db 6, api 41, workers 12, payments 37, security 18, concurrency 4, failure 9, e2e 13), every group in fresh isolated local databases; the SR-1 A-B proof measures 2 provider money effects before the fix and 1 after (recovery calls 1 -> 0, stale write admitted -> SN409 refused, ChargeFailedRecovery -> ChargedSuccess); Codex C1 effects 1 / capture 1 / recovery 0; C2 503, 429, connection drop, client timeout and malformed 2xx all 1 effect on one identity; recovery/refund/release ambiguity 1 effect each; Grow ambiguous settle and refund both FAIL CLOSED; F2 ledger atomicity not regressed; isolated migration proof 58/58 fresh install + re-run + checksum, drift 0; tsc, backend enforcement, direct money-state mutation, Payment SDK boundary, secret, payment compliance, runtime DDL, architecture and mobile gates all PASS. Real money 0, real Grow calls 0, no deployment, canonical staging untouched. OPEN: independent Codex re-review has NOT happened (capacity) and has never seen the SR-1 fix; a fresh Claude adversarial audit of this branch in a separate conversation; Grow native settle/refund idempotency + exact-operation status UNPROVEN (fail-closed in code); JOIN RATE HARDENING open (alias/hook ordering, deliberately not touched); real external e-mail provider open; P0.7 owner acceptance still pending. PERCENTAGE: 100 % of the integration + self-review scope as an ENGINEERING CANDIDATE; SAFE_TO_MERGE_TO_MASTER = NO; R10 = BLOCKED. NEXT STEP: fresh adversarial Claude audit of this branch, then independent Codex audit when capacity returns. Do not merge, do not deploy, do not apply 063 to canonical staging.**

Integration record (details in docs/R9C_INTEGRATION_CANDIDATE.md):
- **Conflicts: 2.** `PROJECT_STATUS.md` — master's P0.7 + P0.6A-ACCEPTED sections kept verbatim, R9C section placed above them, R9C's stale duplicate P0.6A "ENGINEERING COMPLETE" header dropped (master is authoritative for product status). `scripts/migration_manifest.cjs` — both entries kept in order with 062 explicitly reserved for the Codex Amazon Product branch. `src/app.ts` auto-merged; equivalence proven both ways (the R9C delta and the master delta are each reproduced line-for-line in the candidate), so no product behaviour was rolled back and no R9C behaviour was lost.
- **Migrations: 061 PRESERVED (P0.7), 062 RESERVED (Codex, never consumed), 063 INTEGRATED (R9C, never renumbered).** Isolated proof on a disposable local database: 58/58 fresh install, re-run idempotent, checksum ledger consistent, drift 0.
- **SR-1 — NEW CRITICAL, FOUND AND FIXED HERE (exists on the R9C source branch too).** `settleProviderDispatch` had no owner fence for non-success outcomes and the 063 UPDATE guard refused only negative settles, re-arms and disarms — not a foreign `unknown`+`responded` write. A worker that stalled after dispatch, lost its lease, was reclaimed and whose identity was re-armed by a live successor could flip the successor's IN_FLIGHT row to `responded`, blinding the C1 in-flight guard: reconcile then read "authorized/final" while the successor's capture was still at the provider, declared `charge_failed`, settled the identity `permanent_fail` (not a blocking class) and recovery captured the buyer a SECOND time. Deterministic A-B proof `tests/payment_r9c_stale_owner_settle_validation.ts`: **before the fix provider money effects = 2** (capture + recovery, participant ends Recovered/RecoveredCharge, one fee-ledger entry for two captures); **after the fix effects = 1**, recovery calls 0, reconcile deferred after 0 status reads, participant ChargedSuccess, one identity, one ledger entry. Fix in three layers: owner-fenced settlement returning `foreign_owner`, rails converting that into `OutboxLeaseLostError` (stale job stops, no ACK, no reconcile scheduling), and a DB backstop (`SN409 payment_attempt_in_flight_foreign_write`) that refuses any foreign write on an in-flight operation except provider SUCCESS.
- **SR-2 — MEDIUM, FIXED.** R9C retries a proven pre-dispatch failure with the SAME identity, but the mock provider drew its outcome as a pure function of the correlation id, so an identity that once drew "transient failure" drew it forever and the deal-types e2e proof deadlocked (`B2 … expected Completed, got Charging`). The mock now keeps the seeded draw for the first call of a key and re-draws on retries; only the simulation path changed. Production was always bounded by `OUTBOX_MAX_ATTEMPTS` and the 050 rolling cap.
- **Product behaviour from master re-proven, not assumed**: P0.6A geolocation, P0.7 seller inquiries, no seller e-mail/phone/WhatsApp on the public deal (payload carries only business name/description + `contact_channel:"siton_inquiry"`), pickup location + publish readiness, four-cell countdown, seller-authorized Draft buyer preview, P0.7C read-budget polling and the two-tab no-429 budget.
- **Financial constitution unchanged**: Siton commission exactly 8 % (literal `SITON_PLATFORM_FEE_RATE = 0.08`), fee base includes delivery, authoritative buyer VAT excluded, distributor commission 0, distributor payout 0.
- **Honest e-mail state unchanged**: internal inquiry COMPLETE, notification event COMPLETE, real external delivery OPEN / PROVIDER REQUIRED.

Earlier R9C history (branch `claude/r9c-system-red-team`, still not merged): first pass `33a2cb2` (F1 identity-on-reclaim, F2 ledger atomicity) → independent Codex review `550a976` graded it UNSAFE_TO_MERGE with C1 (capture/reconcile race), C2 (503/429 after dispatch) and H1 (Grow idempotency unproven) → remediation `5f5af24`/`3c81f50`/`f025d3f` closed C1 and C2 and made Grow fail closed. That remediation has never been independently re-reviewed, and SR-1 shows why that review is still required.
