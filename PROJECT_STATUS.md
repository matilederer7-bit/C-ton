...
## R9C — CLAUDE SYSTEM RED TEAM + REMEDIATION (2026-09-03) — BRANCH claude/r9c-system-red-team (NOT MERGED) — STATUS: REMEDIATED, INDEPENDENT RE-REVIEW REQUIRED

**COMPLETED: independent Codex review of R9C SHA 33a2cb2 (docs/R9C_CODEX_INDEPENDENT_REVIEW.md, verdict UNSAFE_TO_MERGE: F1 PARTIAL, 2 new CRITICAL C1/C2, 1 HIGH H1) reproduced verbatim BEFORE any change (C1 effects=2 capture+recovery; C2 503/429 effects=2 each) and remediated with ONE durable DB-enforced operation lifecycle (migration 063: recorded/dispatching/responded + owner lease + guards), an arm-before-I/O CAS on every money rail, participant-wide in-flight deferral in reconciliation, post-dispatch ambiguity = UNKNOWN in every adapter (5xx/429/408/gateway/transport/timeout/malformed), a per-provider ambiguity policy (Grow fails closed: no automatic repeat settle/refund, no automatic negative verdict → FINANCIAL_OUTCOME_UNRESOLVED case), recovery/refund/release eligibility blocked behind unresolved or executed captures, and late provider money effects surfaced as cases instead of being discarded. TESTED: adapted Codex counterexamples PASS (C1 effects=1, recovery calls=0; 503/429/drop/timeout/malformed effects=1, no fresh identity; recovery/refund/release ambiguity 1 effect each; Grow CASE 16/17 fail closed), exact Codex originals now FAIL, original F1 S1–S9 10/10, F2 2/2; payments 36/36, workers 12/12, concurrency 4/4, failure 9/9, db 6/6, integration 28/28, e2e 12/12, isolated migration proof 57/57, tsc/lint/secret/payment-compliance/DDL/architecture gates PASS — fresh isolated databases only, real money 0, real Grow calls 0. OPEN: independent Codex re-review of the new SHA; Grow settle/refund idempotency + exact-operation status = UNPROVEN (fail-closed, not proven). PERCENTAGE: 100% of the remediation scope; R9C SAFE_TO_MERGE = pending independent re-review; R10 = NOT STARTED / BLOCKED. NEXT STEP: hand the new R9C SHA to Codex for a second adversarial review; no merge, no rebase onto master (123bbf9) until it passes.**

Remediation record (details in docs/R9C_CLAUDE_SYSTEM_RED_TEAM.md § "R9C remediation"):
- **C1 CRITICAL — FIXED**: reconciliation converted a provider "authorized/final" into charge_failed while the SAME capture was in flight under a live lease, then recovery moved money under a second identity. Now: the row is `dispatching` with the owner lease before I/O; any reconcile defers while any money operation of the participant is in flight (no status read, no verdict); negative settlements are CAS-guarded inside the state transaction and refused by the DB for in-flight operations; recovery is refused (app + DB) while a capture is unresolved or executed.
- **C2 CRITICAL — FIXED**: 503/429 after dispatch were temporary_fail → outbox retry minted identity n2 → second capture. Now: post-dispatch non-success without a provider-declared outcome is UNKNOWN on the SAME identity (adapter `dispatched` honesty distinguishes proven pre-dispatch failures, which keep normal retry with the same identity and no rolling-cap consumption); the DB refuses a new identity while a prior one is unresolved.
- **H1 HIGH — MITIGATED / UNPROVEN**: Grow settle/refund transmit no Siton operation key; repeat idempotency and exact-operation status are unproven → Grow policy fails closed (manual case, zero automatic repeats); to be proven in the Grow sandbox before R10.
- **F2 — PASS (not regressed)**; financial constitution untouched (8% exact, delivery included, buyer VAT excluded, distributor 0).
