...
- PERCENTAGE (this round): seller-login brute-force defect 100% (found, reproduced both directions, fixed, local suite + static gates green — PR/merge/deploy/hosted-verify are the remaining lifecycle steps). Distributor/admin login lockout and the `trustProxy` root fix: 0% (blocked on Render topology + egress, recorded above). Surfaces re-attacked and found sound this round listed above.
- NEXT STEP: open the PR for `claude/peaceful-johnson-7rqtz1`, land it through CI, deploy, and hosted-verify the 429 once a staging seller/egress path exists. Then (owner topology input) pin `trustProxy` to Render's hop count and extend the per-account lockout to distributor/admin login — the latter may need a small events rail, which must wait for the payments branch to release the migration numbering.
