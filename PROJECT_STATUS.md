...
- UPDATED: 2026-09-18
- BRANCH: `claude/nice-goldberg-a3qxq8` → **PR #57** (open; merge/deploy/hosted-smoke evidence is added to this block when the chain completes). Previous: `claude/eloquent-dijkstra-xlmlo2` (PRs #50/#51/#52/#53/#54).
- COMPLETED: **Siton is one system in two languages.** Hebrew stays the product default and is never replaced by an operating-system or `Accept-Language` guess; English is an explicit choice, kept in both `localStorage` and a first-party `siton_lang` cookie so the React app and the server-rendered pages cannot disagree. Same routes, same components, same backend, same permissions, same data — only the text layer changes. (a) **`web/src/i18n`**: a locale store, a hierarchical-key translator with interpolation, a `<Tx>` component for sentences that carry markup (so a translator can move a bolded unit count instead of inheriting Hebrew word order), and a fallback that is **recorded** rather than passed off as a translation. **2454 keys, every one with Hebrew and English**; there is no `if (locale === "en")` anywhere in the product code. (b) Every module-level catalogue — error codes, deal/money/participation state labels, CMS editor labels, buyer copy, geolocation and scanner guidance — holds KEYS, because a constant holding resolved text would freeze whichever language was active at import and survive every later switch. The app tree is keyed on the locale, so a switch remounts every screen and a modal or an error already in state cannot survive it in the old language. (c) **Direction**: the stylesheet was already written in logical properties, so LTR follows from `dir` alone — no second stylesheet and no per-language fork. Two real anchoring bugs were fixed: the admin hotspot was pinned to the physical left (the inline END of an RTL document, the wrong side in LTR), and the legal shell's English notices sat inside the Hebrew document's RTL article. (d) **CMS**: blocks gain `fields_en`/`items_en`, validated by the same safety rules but never required — a blank English value is a declared fallback reported by `missingEnglishContent`, and the editor gains a content-language toggle that names the Hebrew text each empty English field will show. (e) **Legal documents are contracts**: there is a structure for an approved English legal document and, until the owner supplies one, nothing in it. An English visitor sees the Hebrew document with a notice that the Hebrew text is binding; `legalTranslationsRequired()` reports the gap and `OWNER_TRANSLATION_REQUIRED` is a value in the code, not a comment.
- TESTED: All ten local groups PASS on a disposable PostgreSQL 16 (unit 17, integration 46, db 8, api 49, workers 15, payments 44, security 44, concurrency 9, failure 9, e2e 15 — **256 test files**), plus every static gate: lint, backend enforcement, payment compliance, runtime DDL, seven-day cap, architecture, no-real-money (16/16), distributor attribution-only, base44 canonical integrity, route authorization, `ci:migrations` at high-water 072 over 65 migrations, and `mobile:verify`. New: `npm run gate:i18n` in CI (every `t()` key resolves; no Hebrew sentence hard-coded in the shipped UI; the generated dictionaries match their checked-in sources), `tests/i18n_contract_validation.ts` (21 checks — dictionary and placeholder parity, the recorded fallback, the glossary, and the rule that English copy for an authorization-only flow may never say the buyer paid), `tests/i18n_server_surface_validation.ts` (12 checks through real requests) and `tests/frontend_browser_i18n_validation.ts` (a real Chromium over CDP across **15 surfaces × 2 languages × 2 viewports**, checking direction, one `h1`, no horizontal overflow at 390px, labelled controls, console and network errors, and persistence across reload and navigation; it writes screenshots and refuses to be skipped in CI). **Three real defects the browser suite found, all fixed**: the landing page fell back to Hebrew CMS defaults on the English screen; the sign-in, admin step-up and support forms had labels that were siblings of their inputs with no `htmlFor`, so those controls were unlabelled for a screen reader **in Hebrew too**; and the legal shell's English notices rendered right-aligned with their full stops on the wrong side. Existing suites that pinned copy by matching component source now read the dictionary and assert **both** languages — stricter than what they replace. No test was weakened, disabled or skipped. Two generated keys read `card_number` and tripped the payment-compliance scanner; the KEYS were renamed rather than the gate relaxed. Both pre-existing browser suites now resolve the browser through one shared helper instead of hard-coding `/usr/bin/google-chrome`, so they actually run outside a GitHub runner — and both pass.
- OPEN: **No approved English legal text exists.** All seven legal documents fall back to Hebrew with a visible notice; this is owner action, not a coding task — `legalTranslationsRequired()` names them. Seller-authored content (product names, descriptions, business names) is deliberately NOT machine-translated: the structure exists (`fields_en`), the runtime does not invent a translation. Locale-aware URLs were not introduced: the language is app state on the same routes, as scoped. Everything carried over from the previous milestone remains open: `OTP_HASH_SALT` absent from both Render services, the unpublished `deal_page` CMS draft that is the owner's copy, Sentry holding zero projects, and the external link-viewer journey unproven end to end.
- PERCENTAGE: i18n layer 100%. React app system copy 100% (2454 keys, gate green). Server-rendered surfaces 100%. CMS bilingual structure 100%. English dictionary 100%. Legal documents: **structure 100%, approved English content 0% and blocked on the owner.** Repository chain: code/tests/commit/push/PR 100%; CI, merge, deploy and hosted browser smoke in progress on PR #57.
- NEXT STEP: Take PR #57 through CI to green, merge, let Render deploy, confirm the deployed SHA equals master, then run the hosted browser verification on the deployed build in both languages at 1440 and 390 — and only then record this milestone as closed.
