## LONG-HORIZON DEALS — ARCHITECTURE + SAFE FOUNDATIONS (2026-09-09) — branch `claude/long-horizon-deals-architecture` from exact master `d4c7877`, NOT merged (owner item 8, Track B — kept apart from the UX branch)

**COMPLETED:** `docs/LONG_HORIZON_DEALS_ARCHITECTURE.md` (17 sections: exact 7-day constraint map — 20 sites, 2 financial (AuthHeld semantics, Grow J5 window), 6 already horizon-agnostic and proven; why a temporary authorization cannot be stretched; Grow proven vs UNPROVEN table — tokenization / merchant-initiated charge / recurring / account updater / failed-card recovery all UNPROVEN, long authorization DISPROVEN by the documented 7-day J5 window; target model = COMMITMENT + STORED-INSTRUMENT MANDATE (`PaymentMethodStored → FutureChargeAuthorized → MandateValid → ReauthorizationRequired → MandateRetired`) entering the existing money machine at `ChargeAttempt` so 063/064 lifecycle + settlement horizon + recovery + refund are reused and `AuthHeld` keeps its exact meaning — no canonical enum mutated; buyer / money implications; worker scheduling PROVEN to 20 years; card expiry / replacement / failure matrix; re-auth flow (tracking token + OTP → new mandate, old retired, deadline never pauses, two-phase window); completion-window interaction preserving "no successful deal without successful charges"; PCI/token storage; migrations 066+ required (mandates, transition allow-lists, outbox event types, legal acceptance) with the exact 063/064/065 collision note; >1-year warning copy; staged plan S0–S7; test plan; exact real-money blockers) + `docs/LONG_HORIZON_DEALS_IMPLEMENTATION_PLAN.md` (checklist). **Safe foundations (no runtime behaviour change):** `src/deadline_policy.ts` — ONE policy (2 h min; max = PROVEN authorization hold = 7 d, reason `proven_authorization_hold`; 365-day warning; 20-year technical ceiling; provider-capability input; NO environment flag can raise the ceiling); `src/app.ts` create/update bounds derived from it (same codes/messages); `web/src/pages/seller.tsx` picker + `validateDeadline` derived from the same module, `LongHorizonWarning` (> 365 d) wired and dormant; duplicated 7-day constants removed from the picker.

**TESTED:** `tests/long_horizon_deadline_policy_validation.ts` 5/5 (30/180/366 d + 2/5 y refused under the runtime policy, warning only > 365 d, exact 365 d threshold, hypothetical stored-authority policy admits years up to the 20-year ceiling, POST /deals still 400 `deadline_above_maximum` "within 7 days" at 8/30/366 d and 200 at 6 d, no ceiling flag, source pins) · `tests/worker_long_horizon_scheduling_validation.ts` 5/5 (+30 d/+180 d/+366 d/+5 y/+20 y rows never claimed while a due row is, admin queue counts them scheduled_future not due_now with next_scheduled_in_s ≈ 30 d, timestamptz millisecond precision at 20 y, deadline_check on a 366-day deal DEFERS to available_at = deadline exactly, deal stays PendingTarget) · TypeScript server + web, lint, payment scan, runtime-DDL scan, architecture gate PASS · `tests/backend_sanity_suite.ts` deadline pins unchanged.

**OPEN (exact blockers before any long-horizon money):** Grow sandbox credentials (R9B blocker); Grow tokenization + merchant-initiated future charge UNPROVEN; 063/064 not on master (financial integration first; long-horizon migrations start at 066); notifications log-only (re-auth cannot reach buyers); legal mandate consent text/version; owner decisions (withdrawal before close, window length, first-pilot cap 60–90 d). RUNTIME_7_DAY_LIMIT_REMOVED = NO — joining a 1-year deal today would create a J5 hold that Grow documents as valid ≤ 7 days / auto-released ~10 days: every hold would expire before the deal decides.

**PERCENTAGE:** LONG_HORIZON_ARCHITECTURE 100% · LONG_HORIZON_RUNTIME_IMPLEMENTATION 10% (S0 foundations only) · REAL_MONEY_READINESS 0% (unchanged; REAL_MONEY_EXECUTED = 0).

**NEXT STEP:** obtain Grow sandbox credentials and record the exact token / merchant-initiated charge contract (or select a provider with a documented card-on-file MIT contract) — the single fact the whole model waits on; everything else in S1–S3 is sequenced behind the 063/064 integration.
