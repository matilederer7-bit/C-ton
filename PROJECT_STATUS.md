## RELEASE ENGINEERING + OPERATIONAL READINESS LAYER (2026-09-14) — branch `claude/release-readiness-night` from exact master `82c91d62fd092350748405c8aec15a23d0e2af5e`, NOT merged

**COMPLETED:** A repeatable answer to "can this exact commit go to staging/production?" and "if something goes wrong, how do we detect, stop, understand and recover?" built entirely in release-engineering space: `.github/workflows`, `scripts/`, `config/`, `docs/`, Docker/compose, package scripts, `tests/release_tools`. **Not touched:** `src/**` (incl. the financial runtime files and `production_guards.ts`), `web/**`, `frontend/**`, `tests/lab/**`, `supabase/**`, `render.yaml`, migration contents, Grow, any hosted environment. Map: `docs/RELEASE_READINESS_ARCHITECTURE.md`. **One command** `npm run release:preflight` (`scripts/release_preflight.cjs`, `config/release-preflight-gates.json`, profiles static/standard/full) orchestrates 30 existing gates without duplicating logic and prints PASS / FAIL / WARNING / SKIPPED_ENVIRONMENT per gate plus `REAL_MONEY: BLOCKED`; `npm run release:manifest` (SHA, dirty state, Node/npm, migration high-water + LF checksums, demo/mobile/web build tree hashes, preflight result with STALE detection, real-money/Grow status), `npm run release:checklist` (CODE/TESTS/MIGRATIONS/SECURITY/DATA/PAYMENTS/INFRA/BROWSER/MOBILE/OPERATIONS with PROVEN/WARNING/FAIL/SKIPPED/OPEN), `npm run release:owner-check` (`READY FOR CODE DEPLOY` / `READY FOR REAL MONEY: NO`). **Scanner path policy:** `scripts/lib/repo_scan_policy.cjs` is the one file-selection policy (excludes `.git`, `.worktrees`, `node_modules`, build output, `.tmp*`, review artefacts; keeps every real source dir incl. `tests/lab`); every static scanner refactored onto it; `.worktrees/` and `.release-artifacts/` added to `.gitignore` and `.dockerignore`. **Brittle gates hardened:** `money_tax_invoice_gate` now executes the real fee arithmetic through a tsx probe (100/0, 118/18, refund sign, VAT clamp) and checks declarations by AST instead of exact expression regexes (it was FAILING on clean master after a harmless refactor); `legal_compliance_gate` uses the new semantic raw-card scanner (`scripts/lib/raw_card_terms.cjs`: identifiers/keys/columns/form fields, prose never a finding, explicit per-file allow-list `config/raw-card-term-allowlist.json`), fixes a `[^\\n]` regex bug, and reports the production-only seller KYC approval step as an OWNER_DECISION warning (an unconditional KYC refusal fails). **Configuration matrix:** `config/runtime-environment-policy.json` for development / demo-preview / test / staging / production with `scripts/runtime_environment_gate.cjs` (evaluates process.env, env files, `render.yaml` services, compose files; secret values never echoed) and `config/startup-config-matrix.json` + `scripts/startup_config_matrix.cjs` (23 unsafe combinations run through the REAL boot guard via `scripts/probes/production_guards_probe.ts` and the release policy; 6 runtime gaps documented as WARNING, never PASS). **Real-money governance:** `config/real-money-release-policy.json` (`REAL_MONEY_ALLOWED=false`, four reasons: F-13 unresolved, Grow live verification not performed, production activation not approved, financial branch not integrated) + `npm run proof:no-real-money` (16 checks incl. executed runtime seam; `docs/REAL_MONEY_RELEASE_GOVERNANCE.md`). **Migration safety:** `scripts/run_migrations.cjs` hashes the LF-normalised body (git index is LF, Windows checkouts CRLF — the split that corrupted staging ledger metadata before) and accepts CRLF-era checksums as EOL variants; `.gitattributes` pins `src/migrations/*.sql`; `npm run migrations:preflight` (fresh, upgrade from `origin/master`, partial ledger, CRLF ledger, tampered checksum, dirty ledger, failing migration atomic with implicit and explicit BEGIN/COMMIT, schema drift), `npm run migrations:doctor` (read-only verdicts, hosted refused without `--allow-hosted`), `npm run migrations:repair` (dry-run by default, `--yes`, `--clear-failed` needs `--i-verified-no-partial-effects`), `npm run db:backup-restore-rehearsal` (migrate → synthetic fixtures → pg_dump custom+plain with secret-shape scan → new DB → pg_restore → counts/FK/index/trigger/function parity/ledger/runner no-op/records/invariants). **Hygiene gates:** `npm run scan:secrets` (keys, tokens, service-role JWT by payload, Grow credentials, DB URLs with real credentials, Luhn PANs outside documented test cards, committed .env, PII in runtime; allow-list with reasons), `npm run gate:logging-hygiene` + `config/logging-data-classification.json` + `docs/LOGGING_DATA_CLASSIFICATION.md` (NEVER_LOG / SENSITIVE / SAFE; 40 log calls, 0 NEVER_LOG, 1 SENSITIVE unmasked = notification log provider `recipient_ref`, documented), `npm run report:routes` + `config/route-classification.json` (213/213 routes intentionally classified: admin 66, seller 49, distributor 2, buyer-token 16, anonymous-by-design 10, webhook 3, public-read 13, public-write 8, app-shell 37, legacy-alias 3, demo-only 3, health 3; an unclassified sensitive route fails), `npm run check:health-contract` (liveness vs readiness proven with a dropped-database negative control; worker heartbeat; gaps HC-1..4), `npm run smoke:http-security` (11 checks; GAP-HTTP-1: `/readiness` lacks no-store), `npm run check:supply-chain` (4 production high advisories = owner upgrade decision, nothing upgraded), `npm run check:reproducible-build` (demo, mobile, Vite builds DETERMINISTIC), `npm run check:repo-hygiene`, `npm run check:docker-static`. **Local QA:** `scripts/lib/test_db_isolation.cjs` (agent-tagged unique local-only DBs, exit cleanup, preserve-on-failure, stale lister found 14 leaked databases from earlier runs), `scripts/lib/process_cleanup_guard.cjs` + `npm run qa:diagnose` / `qa:cleanup-stale-dbs -- --yes` (kills only owned children; lists strays, leaked connections, occupied ports), `scripts/lib/flake_classifier.cjs` + `npm run qa:classified` (REAL_FAILURE / ENVIRONMENT_FAILURE / explicit CORRECTIVE_RERUN; never silent). **Docker/CI:** `docker-compose.release-lab.yml` + `npm run release:local-lab` (build, start, migrate+seed, smoke, worker heartbeat, graceful exit codes, down -v; SKIPPED_ENVIRONMENT here — no Docker engine on the build machine); new `.github/workflows/release-readiness.yml` (`preflight-static`, `preflight-database` with a Postgres service + pg tools, `docker-release-lab` on same-repo PRs + master); `backend-quality-gates.yml` runs the duplicate `test:all` only post-merge (`docs/CI_TEST_STRATEGY.md`). **Runbooks/docs:** DEPLOYMENT (code deploy, database migration, real-money activation are three separate actions), ROLLBACK, PAYMENT_INCIDENT, SECURITY_INCIDENT, DATABASE_INCIDENT, PRODUCTION_DATA_ACCESS_BOUNDARIES, PARALLEL_AGENT_DEVELOPMENT, HEALTH_CHECK_CONTRACT, HTTP_SECURITY_SURFACE, RUNTIME_ENVIRONMENT_POLICY, MIGRATION_SAFETY_SYSTEM, DB_BACKUP_RESTORE_REHEARSAL, FLAKE_CLASSIFICATION, SUPPLY_CHAIN_STATUS, REPRODUCIBLE_BUILD, RELEASE_READINESS_SCORECARD.

**TESTED:** `tests/release_tools` **59/59** (scan policy 5, raw-card 6, money gate 10 incl. reformat survival + 9 mutants, legal gate 6 incl. 5 mutants, runtime policy 9 incl. no-real-money mutants, migration tools 6, secret scan 6, process/flake 4, runtime surface 3 incl. an unclassified-route mutant, orchestration 4). `npm run release:preflight` standard profile on this branch: standard profile PASS on 2026-09-14 (18 PASS, 0 FAIL, 8 WARNING all documented, 1 SKIPPED_ENVIRONMENT = Docker lab, no engine on the build machine); static profile PASS (13 PASS, 0 FAIL, 7 WARNING). Existing gates re-run green: TypeScript, lint/backend enforcement, architecture, payment compliance (302 files), runtime DDL, Base44 integrity, isolated migration proof, route authorization (static + 4 behavioural suites), demo build, mobile contract. Environment: Windows 11, Node 24 (CI/Docker 22), PostgreSQL 18 local, no Docker engine.

**OPEN:** Docker lab and image build proven only by CI (not executed here). Hosted: `OTP_HASH_SALT` not declared on either Render service (runtime falls back to a public default salt); Render Starter + Supabase Site URL decisions unchanged. Owner decisions: production KYC approval step; dependency upgrades (fastify tree, exceljs, capacitor toolchain, unused vitest). Runtime gaps documented, not implemented (isolation rule): OTP salt default, `TRACKING_LEGACY_COMPAT` accepted in production-like mode, debug surfaces with key, demo admin key accepted by the boot guard, demo-mode bypass on a production host, service-role key presence undetectable, `/readiness` without no-store and without worker/ledger state, notification log provider logs `recipient_ref` raw. Found while grounding the runbooks (src untouched, engineering follow-up): `admin_mission_control.ts` outbox/correlation trace queries reference non-existent columns and return empty via `safeQuery`; `service_role` retains EXECUTE on `siton_inventory_rpc`; tracking-token revoke helper has no route caller; no bulk session revocation; `ADMIN_API_KEY` rotation leaves admin sessions valid; `pause_charging_emergency` enforced only at `charging/start`; no DLQ redrive for money events; no repair for a `running` ledger row. UX not tested (Codex branch in flight). Real money **BLOCKED**: F-13 unresolved, financial branch not integrated, Grow live verification not performed, activation not approved.

**PERCENTAGE:** RELEASE ENGINEERING LAYER **100 %** built and locally tested; RELEASE READINESS OF MASTER `82c91d6` per `docs/RELEASE_READINESS_SCORECARD.md`: repository 95, build 95 (image unproven here), tests 85, security 85, database 90, financial safety 60 **BLOCKED FOR REAL MONEY**, UX 0 (out of scope), infrastructure 70, observability 70, operations 90, real-money activation 0. REAL MONEY = 0; no deployment; no merge.

**NEXT STEP:** After canonical financial + UX integration, rebase/integrate this release-engineering branch onto the resulting master, run `npm run release:preflight` (full profile) + the `release-readiness.yml` Docker CI job, then merge the release tooling. This branch is NOT to be merged before the financial branch integration and the UX branch integration/reconciliation.
