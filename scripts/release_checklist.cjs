...
const OPEN_ITEMS = [
  { category: "PAYMENTS", owner: "owner + reviewer", text: "F-13 REAL_MONEY_BLOCKER: Grow provider contract for real money resolved (the R9C rails are on master since PR #9; the provider-side facts are still unproven outside the sandbox)", ref: "config/real-money-release-policy.json F13_PROVIDER_CONTRACT_UNRESOLVED" },
  ...
  { category: "PAYMENTS", owner: "senior skeptical engineer", text: "Adversarial review of the integrated master (R9C financial rails + hardened UX + release tooling) performed and its findings closed", ref: "config/real-money-release-policy.json ADVERSARIAL_REVIEW_NOT_PERFORMED" },
  ...
  { category: "SECURITY", owner: "engineering", text: "CLOSED by the adversarial review: the boot guard (not only this pre-deploy gate) now refuses OTP_HASH_SALT default, placeholder ADMIN_API_KEY/SELLER_SESSION_SECRET, TRACKING_LEGACY_COMPAT=1, DEBUG_SURFACES_ENABLED=1, OTP_TEST_BYPASS_CODE and SUPABASE_SERVICE_ROLE_KEY in production, and a demo deployment mode on a hosted platform; recipient_ref is masked; /readiness is no-store. startup matrix runtime_gaps=0", ref: "docs/SENIOR_ADVERSARIAL_REVIEW.md F-03/F-04/F-05/F-10" },
  { category: "INFRA", owner: "engineering", text: "npm audit: production-tree HIGH advisories cleared 4 -> 0 (fastify 5.12.4; in-major pins for fast-uri, brace-expansion, tmp). Remaining criticals (tar, vitest) are dev/build-only and reach production ONLY because the Dockerfile does not prune dev deps - see the image-prune item", ref: "docs/SENIOR_ADVERSARIAL_REVIEW.md F-11/F-12" },
  ...
  { category: "OPERATIONS", owner: "engineering", text: "Release-readiness reintegration PR merged to master with the backend, web-runtime and release-readiness workflows green on the merge commit", ref: "PROJECT_STATUS.md, .github/workflows/release-readiness.yml" }
];
...
const GATE_TO_CATEGORY = {
  ...
  "real-money-activation": "PAYMENTS"
};
...
function main() {
  ...
  const mark = (status) => status === "PASS" ? "[x] PROVEN " : status === "WARNING" ? "[~] WARNING" : status === "FAIL" ? "[!] FAIL   " : status === "BLOCKED" ? "[ ] BLOCKED" : "[ ] SKIPPED";
  ...
  const md = ["# Release checklist", "", "Generated " + new Date().toISOString() + " for `" + git.sha + "` (" + git.branch + ")." + (stale ? " Preflight report is STALE for this SHA." : ""), "", "Legend: [x] PROVEN by an automated gate for this SHA; [~] proven with documented warnings; [!] gate failed; [ ] BLOCKED held by governance (real money); [ ] SKIPPED could not be proven here; [ ] OPEN owner/provider/hosted decision (never pretended done).", ""];
  const totals = { proven: 0, warning: 0, fail: 0, blocked: 0, skipped: 0, open: 0 };
  for (const category of categories) {
    md.push("## " + category, "");
    for (const line of lines[category]) {
      md.push("- " + line);
      if (line.startsWith("[x]")) totals.proven += 1; else if (line.startsWith("[~]")) totals.warning += 1; else if (line.startsWith("[!]")) totals.fail += 1; else if (line.startsWith("[ ] BLOCKED")) totals.blocked += 1; else if (line.includes("OPEN")) totals.open += 1; else totals.skipped += 1;
    }
    md.push("");
  }
  md.push("## Totals", "", "proven=" + totals.proven + " warning=" + totals.warning + " fail=" + totals.fail + " blocked=" + totals.blocked + " skipped=" + totals.skipped + " open=" + totals.open, "");
  ...
  console.log(md.join("\n"));
  console.log("RELEASE_CHECKLIST proven=" + totals.proven + " warning=" + totals.warning + " fail=" + totals.fail + " blocked=" + totals.blocked + " skipped=" + totals.skipped + " open=" + totals.open + " written=.release-artifacts/release-checklist.md");
