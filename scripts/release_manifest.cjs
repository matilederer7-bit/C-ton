...
function main() {
  ...
  const manifest = {
    ...
    preflight: preflight ? { overall: preflight.overall, counts: preflight.counts, profile: preflight.meta && preflight.meta.profile, for_sha: preflight.meta && preflight.meta.sha, finished_at: preflight.finished_at, stale: Boolean(preflightStale) } : { overall: "NOT_RUN" },
    real_money: { allowed: realMoney.real_money_allowed === true, status: realMoney.status, blocking_reasons: (realMoney.blocking_reasons || []).filter((r) => !r.cleared).map((r) => r.id) },
    grow: { status: "SANDBOX_TRANSPORT_PROVEN_NOT_AUTHORISED", live_verification: "not performed", detail: "docs/R9B_GROW_SANDBOX_PROOF_RUNBOOK.md; sandbox userId/pageCode blocker with Grow support; staging pinned to mockpay by config/runtime-environment-policy.json" },
  ...
  const md = [
    ...
    "| Preflight | " + manifest.preflight.overall + (manifest.preflight.stale ? " (STALE: produced for " + String(manifest.preflight.for_sha).slice(0, 12) + ")" : "") + (manifest.preflight.counts ? " pass=" + manifest.preflight.counts.PASS + " fail=" + manifest.preflight.counts.FAIL + " warning=" + manifest.preflight.counts.WARNING + " skipped=" + manifest.preflight.counts.SKIPPED_ENVIRONMENT : "") + " |",
