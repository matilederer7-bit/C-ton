...
// source (this worktree; every mutation is restored from its exact pre-mutation bytes),
...
const MUTATIONS = [
  ...
  { id: "M22_status_echo_rewrites_binding", invariant: "exact-operation identity: a status echo never rewrites the durable provider reference (O-1) — with the residual-A reconcile reference-mismatch guard disabled as well (it masks the single-edit mutant)",
    files: [
      { file: "src/app.ts",
        ...
        to: `  if (status.reference_matches_query === false && Number(1) === 2) {\n    await openPaymentOperationalCase({\n      autoKey: \`payment-reconcile-reference-mismatch:\${participantId}:\${attemptType}\`,` },
      { file: "src/app.ts",
...
for (const m of toRun) {
  ...
  try {
    ...
    for (const e of edits) {
      ...
      restored.add(e.file);
      fs.writeFileSync(e.file, e.original);
      // Exact-byte restore preserves migration checksums even with core.autocrlf.
