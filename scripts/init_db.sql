...
CREATE TABLE IF NOT EXISTS platform_fee_money_events (
  ...
  platform_fee_vat_rate NUMERIC(6,4) NOT NULL,
...
CREATE TABLE IF NOT EXISTS operational_cases (
  ...
  case_type TEXT NOT NULL CHECK (case_type IN (
    'RefundRequest','DeliveryIssue','SellerRisk','BuyerComplaint','PaymentMismatch',
    'InvoiceIssue','ContentReport','SystemException','Other'
  )),
  status TEXT NOT NULL DEFAULT 'Open' CHECK (status IN (
    'Open','NeedsSeller','NeedsAdmin','WaitingExternal','Resolved','Closed'
  )),
...
CREATE TABLE IF NOT EXISTS operational_case_events (
  event_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  case_id UUID NOT NULL REFERENCES operational_cases(case_id) ON DELETE RESTRICT,
  event_type TEXT NOT NULL CHECK (event_type IN (
    'case.create','case.update_status','case.assign','case.close','case.escalate','case.refund_request_marked'
  )),
...
CREATE INDEX IF NOT EXISTS idx_operational_cases_status_priority_created ON operational_cases(status, priority, created_at);
CREATE INDEX IF NOT EXISTS idx_operational_cases_type_status ON operational_cases(case_type, status);
CREATE INDEX IF NOT EXISTS idx_operational_cases_deal ON operational_cases(deal_id) WHERE deal_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_operational_cases_seller ON operational_cases(seller_id) WHERE seller_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_operational_cases_participant ON operational_cases(participant_id) WHERE participant_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS ux_operational_cases_open_auto_key
  ON operational_cases(auto_key)
  WHERE auto_key IS NOT NULL AND status IN ('Open','NeedsSeller','NeedsAdmin','WaitingExternal');
CREATE INDEX IF NOT EXISTS idx_operational_case_events_case_created ON operational_case_events(case_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_operational_case_events_type_created ON operational_case_events(event_type, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_legal_acceptances_deal
  ON legal_acceptances (deal_id, accepted_at);
CREATE INDEX IF NOT EXISTS idx_legal_acceptances_actor
  ON legal_acceptances (actor_type, actor_ref, accepted_at);

-- OTP rail (migration 031). Provider-ready, log/dev only by default.
...
CREATE INDEX IF NOT EXISTS idx_otp_challenges_deal
  ON otp_challenges (deal_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_otp_delivery_attempts_challenge
  ON otp_delivery_attempts (challenge_id, created_at);
