...
CREATE TABLE IF NOT EXISTS seller_payout_attempts (
  ...
  attempt_type TEXT NOT NULL CHECK (attempt_type IN ('prepare','dispatch','reconcile')),
  result_class TEXT NOT NULL CHECK (result_class IN ('success','permanent_fail','temporary_fail','unknown')),
  correlation_id TEXT NOT NULL,
  provider_reference TEXT NULL,
  payload JSONB NOT NULL DEFAULT '{}'::jsonb,
...
CREATE TABLE IF NOT EXISTS seller_payout_reconciliation (
  payout_reconciliation_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  payout_batch_id UUID NOT NULL REFERENCES seller_payout_batches(payout_batch_id) ON DELETE CASCADE,
  provider_code TEXT NOT NULL,
  reconciliation_status TEXT NOT NULL CHECK (reconciliation_status IN ('matched','manual_review')),
  correlation_id TEXT NOT NULL,
  provider_batch_reference TEXT NULL,
