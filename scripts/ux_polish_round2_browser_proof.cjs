  ...
  try {
    ...
    for (const width of WIDTHS) {
      ...
      await check(`UX-4 @${width}: choosing another option moves the orange fill (and only one stays chosen)`, async () => {
        ...
        // The exact orange and single-selection assertions below remain required.
        await waitFor(`(() => {
          const card = document.querySelectorAll('[data-testid="receipt-method-option"]')[3];
          return card?.dataset.selected === '1' && getComputedStyle(card.querySelector('.choice-ind')).backgroundColor.replace(/ /g, '') === 'rgb(236,102,8)';
        })()`);
        const snap = await ev(`(() => {
          const cards = [...document.querySelectorAll('[data-testid="receipt-method-option"]')];
          return {
