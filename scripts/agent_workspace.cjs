...
function startTask(agent, rawSlug) {
  ...
  if (!worktree) throw new Error(`agent worktree missing for ${agent}; run npm run agents:setup first`);
  ...
  if (branchExists(repoRoot, branch)) throw new Error(`task branch already exists: ${branch}`);
...
function printPlan() {
  ...
  console.log("AGENT_WORKTREE_BOUNDARY overwrite_existing_path=false discard_uncommitted=false force_push=false");
}
