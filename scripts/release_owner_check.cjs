...
function main() {
  ...
  const lines = [
    ...
    "Real money:          " + (realMoney.real_money_allowed ? "ALLOWED" : "BLOCKED") + " (" + (realMoney.blocking_reasons || []).filter((r) => !r.cleared).length + " blocking reasons)",
  ...
  console.log("RELEASE_OWNER_CHECK code_deploy=" + (readyForCodeDeploy ? "YES" : "NO") + " real_money=NO preflight=" + (preflight ? preflight.overall : "NONE") + " checklist_exit=" + checklist.status);
