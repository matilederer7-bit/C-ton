...
function runGh(args, allowFailure = false) {
  ...
  if (result.error) throw new Error(`GitHub CLI unavailable: ${result.error.message}`);
  if (result.signal) throw new Error(`gh terminated by signal ${result.signal}`);
  if (typeof result.status !== "number") throw new Error("gh did not return an exit status");
  if (result.status !== 0 && !allowFailure) {
    throw new Error(String(result.stderr || result.stdout || `gh exit ${result.status}`).trim());
  }
  return result;
}
...
function summarizeLog(log) {
  ...
  return {
    firstError: first.replace(/^##\[error\]/i, "").trim(),
    files: extractFileHints(lines.join("\n")),
    reproduce: reproductionHint(lines.join("\n"))
  };
}

function latestFailedRunId() {
  const result = runGh(["run", "list", "--status", "failure", "--limit", "1", "--json", "databaseId", "--jq", ".[0].databaseId"]);
  const id = String(result.stdout || "").trim();
  if (!/^\d+$/.test(id)) throw new Error("no failed GitHub Actions run found");
  return id;
}
...
function runMetadata(runId) {
  ...
  return {
    workflow: data.name || data.displayTitle || "unknown",
    url: data.url || "",
...
function main() {
  ...
  const runId = arg === "latest" ? latestFailedRunId() : arg;
  ...
  console.log(`CI_FAILURE_SUMMARY run=${runId}`);
