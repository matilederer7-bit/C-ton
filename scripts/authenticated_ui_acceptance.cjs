...
const PLAN = {
  ...
  admin: [
    ...
    { id: "A3", name: "CMS (site content) loads", needs: ["admin"], allow: [] },
    { id: "A4", name: "edit content (footer text)", needs: ["admin"], allow: [] },
    { id: "A5", name: "save content", needs: ["admin"], allow: [] },
    { id: "A6", name: "reload and verify persistence (CMS form + public site-content)", needs: ["admin"], allow: [] },
    { id: "A7", name: "revision conflict: stale revision is refused (409) and the concurrent write wins", needs: ["admin"], allow: [409] },
    { id: "A8", name: "restore/revert previous content", needs: ["admin"], notSupported: "master has no restore/revert route or UI (the server only stores previous_value_jsonb); nothing to drive" },
...
const BROWSER = [process.env.SITON_ACCEPTANCE_BROWSER,
...
async function openBrowser(tag) {
  ...
  const proc = spawn(BROWSER, ["--headless=new", "--disable-gpu", "--no-first-run", "--no-default-browser-check", "--lang=he", `--remote-debugging-port=${port}`, `--user-data-dir=${profileDir}`, "about:blank"], { stdio: "ignore", windowsHide: true });
  for (let i = 0; i < 80; i++) {
    try {
...
const STEP_IMPL = {
  ...
  async A3({ cdp }) {
    ...
    await waitFor(cdp, `(() => ${bodyTextHas("ניהול תוכן האתר")} && document.querySelector('.admin-main select') && document.querySelector('.admin-main form') ? true : null)()`, 30_000, "CMS form");
    ...
    return `sections=${Object.keys(r.body.sections).length}`;
  },
  async A4({ cdp, ctx }) {
    await cdp.evaluate("(() => { const s = document.querySelector('.admin-main select'); const set = Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value').set; set.call(s,'footer'); s.dispatchEvent(new Event('change',{bubbles:true})); return true; })()");
    await waitFor(cdp, "document.querySelector('.admin-main select').value === 'footer' && Boolean(document.querySelector('.admin-main form textarea'))", 10_000, "footer section");
    ctx.footerOriginal = await cdp.evaluate("document.querySelector('.admin-main form textarea').value");
    const mark = MARK("admin");
    ctx.footerEdited = (ctx.footerOriginal + " " + mark).slice(-500);
    await cdp.type("document.querySelector('.admin-main form textarea')", ctx.footerEdited);
    const now = await cdp.evaluate("document.querySelector('.admin-main form textarea').value");
    if (now !== ctx.footerEdited) throw new Error("textarea did not accept the edit");
    return `footer.text: ${ctx.footerOriginal.length} chars → appended ${mark}`;
  },
  async A5({ cdp }) {
    await cdp.click("document.querySelector('.admin-main form button')");
    const status = await waitFor(cdp, "(() => { const p = document.querySelector('.admin-main p[role=status]'); return p && p.innerText.trim() ? p.innerText.trim() : null; })()", 20_000, "CMS save status");
    if (status !== "התוכן נשמר ויוצג באתר") throw new Error(`save status: ${status}`);
    return `status="${status}"`;
  },
  async A6({ cdp, ctx }) {
    await cdp.reload();
    await waitFor(cdp, "Boolean(document.querySelector('.admin-main select') && document.querySelector('.admin-main form textarea'))", 30_000, "CMS after reload");
    await cdp.evaluate("(() => { const s = document.querySelector('.admin-main select'); const set = Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value').set; set.call(s,'footer'); s.dispatchEvent(new Event('change',{bubbles:true})); return true; })()");
    const form = await waitFor(cdp, "(() => { const s = document.querySelector('.admin-main select'); const t = document.querySelector('.admin-main form textarea'); return s && s.value === 'footer' && t && t.value.includes(" + JSON.stringify(MARK("admin")) + ") ? { v: t.value } : null; })()", 15_000, "persisted footer in CMS form");
    const pub = await cdp.evaluate(anonFetch("/api/site-content"));
    if (!String(pub.body?.content?.footer?.text || "").includes(MARK("admin"))) throw new Error("public site-content does not show the saved footer");
    return "CMS form and public /api/site-content both carry the edit after reload";
  },
  async A7({ cdp, ctx }) {
    const cur = await cdp.evaluate(sessionFetch("/api/admin/site-content"));
    const revision = Number(cur.body?.sections?.footer?.revision);
    if (!Number.isInteger(revision)) throw new Error("no footer revision");
    // a "second admin" restores the original text with the same revision the
    // form still holds — this is also the cleanup of the harness's own edit
    const concurrent = await cdp.evaluate(sessionFetch("/api/admin/site-content/footer", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ value: { text: ctx.footerOriginal }, revision }) }));
    if (concurrent.status !== 200) throw new Error(`concurrent write status=${concurrent.status}`);
    await cdp.click("document.querySelector('.admin-main form button')");
    const status = await waitFor(cdp, "(() => { const p = document.querySelector('.admin-main p[role=status]'); const t = p ? p.innerText.trim() : ''; return t && t !== 'התוכן נשמר ויוצג באתר' ? t : (t === 'התוכן נשמר ויוצג באתר' ? 'SAVED' : null); })()", 20_000, "conflict status");
    if (status === "SAVED") throw new Error("stale revision was ACCEPTED — the concurrent edit was silently overwritten");
    ...
    return `stale revision ${revision} refused with "${status.slice(0, 50)}"; original footer restored and public-verified`;
  },
  async A9({ cdp, other }) {
    await cdp.viewport(other);
    await cdp.navigate(`${BASE}/preview/#/admin/content`);
    await cdp.reload();
    const cms = await waitFor(cdp, `(() => ${bodyTextHas("ניהול תוכן האתר")} && document.querySelector('.admin-main form') ? { over: ${noOverflowExpr} } : null)()`, 30_000, `CMS at ${other.width}`);
    if (!cms.over) throw new Error(`horizontal overflow on CMS at ${other.width}`);
    await cdp.screenshot(`admin-A9-cms-${other.width}.png`);
    return `CMS renders at ${other.width}px without horizontal overflow`;
  }
};
