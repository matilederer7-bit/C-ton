...
function main() {
  ...
  // A node_modules that is a symlink/junction into ANOTHER checkout (the
  // parallel-worktree recipe in docs/PARALLEL_AGENT_DEVELOPMENT.md) makes
  // `npm ls` resolve the real path and report every package as extraneous.
  // That is not a dependency problem and must not be reported as one; it is
  // also not a PASS. Only a real install (`npm ci`) can answer this check.
  const nodeModules = path.join(root, "node_modules");
  let linkedNodeModules = false;
  try { linkedNodeModules = fs.lstatSync(nodeModules).isSymbolicLink(); } catch { linkedNodeModules = false; }
  const ls = linkedNodeModules ? null : npm(["ls", "--json", "--depth=0"]);
  let lsJson = null;
  try { lsJson = ls ? JSON.parse(ls.stdout || "{}") : null; } catch { lsJson = null; }
  if (linkedNodeModules) report.skip("installed tree", "node_modules is a symlink/junction into another checkout; npm ls cannot attribute the tree - run npm ci in this checkout for a canonical answer (CI always does)");
  else if (!lsJson) report.skip("installed tree", "npm ls produced no JSON (node_modules missing?)");
  ...
  const all = linkedNodeModules ? null : npm(["ls", "--all", "--json", "--omit=dev"]);
  let allJson = null;
  try { allJson = all ? JSON.parse(all.stdout || "{}") : null; } catch { allJson = null; }
  if (linkedNodeModules) report.skip("duplicate majors (production)", "node_modules is linked from another checkout; run npm ci for a canonical tree");
  else if (allJson) {
