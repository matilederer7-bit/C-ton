...
try {
  ...
  await run("CMS rejects unauthenticated writes; named admin edits persist, revision protects against lost changes and HTML is rejected", async () => {
    ...
    // SITE CMS — pages are template block lists; the hero is the locked first block.
    const withTitle = (page: any, title: string) => ({ blocks: page.blocks.map((b: any, i: number) => i === 0 ? { ...b, fields: { ...b.fields, title } } : b) });
    const value = withTitle(home.published, "כותרת חדשה לבדיקה");
    ...
    assert.equal((await request("GET", "/api/site-content")).json().content.home.title, "כותרת חדשה לבדיקה");
    assert.equal((await request("GET", "/api/site-content")).json().content.home.blocks[0].fields.title, "כותרת חדשה לבדיקה");
    assert.equal((await request("PUT", "/api/admin/site-content/home", headers, { value, revision: 0 })).status, 409);
    assert.equal((await request("PUT", "/api/admin/site-content/home", headers, { value: withTitle(value, "<script>alert(1)</script>"), revision: 1 })).status, 400);
    assert.throws(() => validateContent("home", withTitle(value, "x".repeat(121))));
    assert.throws(() => validateContent("__proto__", {}));
    const legal = read.json().sections.legal_terms;
    const body = "תנאים מעודכנים לבדיקה";
    const legalDoc = legal.published.blocks[0];
    assert.equal((await request("PUT", "/api/admin/site-content/legal_terms", headers, { value: { blocks: [{ ...legalDoc, fields: { ...legalDoc.fields, body } }] }, revision: 0 })).status, 200);
    ...
    assert.equal(revision.previous_value_jsonb.blocks[0].fields.body, legalDoc.fields.body); assert.equal(revision.updated_by, admin.admin_user_id);
