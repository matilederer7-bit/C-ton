...
async function terminate(pid: number) { await killer.query(`SELECT pg_terminate_backend($1)`, [pid]); }
...
await run("REAL PROCESS: a child with no uncaughtException handler survives idle-in-transaction and mid-statement terminations and exits 0", async () => {
  ...
  assert.equal(done.results.length, 3, JSON.stringify(done.results));
