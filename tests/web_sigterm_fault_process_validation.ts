...
function waitExit(child: ReturnType<typeof fork>) { return new Promise<void>((resolve, reject) => { child.once("exit", () => resolve()); child.once("error", reject); }); }
async function run(point: "web.request.before_commit" | "web.request.after_commit", run: number) {
  ...
  if (first !== "fault") { child.kill("SIGTERM"); await waitExit(child); throw new Error(`request completed before fault boundary: ${first}; ${stderr}`); }
  child.kill("SIGTERM");
  await Promise.all([request, waitExit(child)]);
