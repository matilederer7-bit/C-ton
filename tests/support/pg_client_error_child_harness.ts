...
// 4. Terminate after the body fails, while its rollback is being attempted.
// No process-level exception handler or temporary client error listener masks
// the missing-guard mutation here.
{
  const original = new Error("synthetic-rollback-cause");
  let error: unknown = null;
  try {
    await withTransaction(async (c: any) => {
  ...
  if (error !== original) throw new Error("rollback swallowed or replaced the original failure");
  results.push(summarize("withTransaction.during_rollback", error, await probePool()));
}
