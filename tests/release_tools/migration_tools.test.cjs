...
test("doctor verdicts and repair dry-run/apply on a disposable database", { skip: !dbAvailable && "no local DATABASE_URL (SKIPPED_ENVIRONMENT)" }, async () => {
  ...
  try {
    ...
    await tools.withClient(db.url, (client) => client.query("UPDATE siton.migration_ledger SET status='failed', error_message='simulated' WHERE migration_id=$1", [analysis.files[analysis.files.length - 1].id]));
    assert.match(doctor(db.url).stdout, /verdict=BLOCKED[\s\S]*dirty rows: 066:failed/);
    const refused = repair(db.url, ["--clear-failed", "066"]);
    assert.equal(refused.status, 2);
    assert.match(refused.stderr, /i-verified-no-partial-effects/);
    const cleared = repair(db.url, ["--clear-failed", "066", "--i-verified-no-partial-effects", "--yes"]);
