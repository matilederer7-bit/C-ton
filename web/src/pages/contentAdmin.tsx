...
export function ContentAdmin() {
  ...
  const choosePage = (k: string) => { if (busy || !sections[k]) return; if (dirty && !window.confirm(t("pages.content_admin.c88583d2"))) return; setKey(k); setWorking(sections[k]!.draft || sections[k]!.published); setDirty(false); setMessage(null); setAddType(""); };
  ...
  const saveDraft = async () => { const page = validateLocally(); if (!page) return null; return mutate(`/api/admin/site-content/${key}/draft`, "PUT", { value: page }, t("pages.content_admin.e8f968b8")); };
  const preview = async () => {
    if (IN_EDITOR_PREVIEW.has(key)) {
      setInlinePreview(v => !v);
      setMessage({ tone: "info", text: inlinePreview ? "" : t("pages.content_admin.c8619929") });
      return;
    }
    ...
    setMessage({ tone: "info", text: t("pages.content_admin.283f1a64") });
  };
  const publish = async () => {
    if (!section) return;
    ...
    await mutate(`/api/admin/site-content/${key}/publish`, "POST", {}, t("pages.content_admin.5b8e2ff7"));
  };
  const discard = async () => { if (!window.confirm(t("pages.content_admin.a98d3d05"))) return; await mutate(`/api/admin/site-content/${key}/discard`, "POST", {}, t("pages.content_admin.b78bc372")); };
  ...
  return <div className="cms" data-testid="cms-admin">
    <h1>{t("pages.content_admin.77722472")}</h1>
    <p className="muted">{t("pages.content_admin.7ccc1a63")}</p>
    <div className="cms-pages" role="tablist" aria-label={t("pages.content_admin.d8ca9f4c")}>
      {Object.entries(sections).map(([k, s]) => <button key={k} type="button" role="tab" aria-selected={k === key} className={`chip${k === key ? " active" : ""}`} data-testid={`cms-page-${k}`} disabled={busy} onClick={() => choosePage(k)}>{s.label}{s.draft ? t("pages.content_admin.053a4d19") : ""}</button>)}
    </div>
    {section && working && contract ? <>
      <section className="panel cms-status" data-testid="cms-status" data-dirty={dirty ? "1" : "0"} data-has-draft={section.draft ? "1" : "0"}>
        <div className="cms-status-text">
          ...
          <div className="small muted">{section.published_at || section.updated_at ? t("pages.content_admin.b9522bbb", { updated_at: when(section.published_at || section.updated_at) }) : t("pages.content_admin.c22c5fd0")}{section.draft ? t("pages.content_admin.c04ce31c", { draft_updated_at: when(section.draft_updated_at) }) : ""}{dirty ? t("pages.content_admin.5fcab34e") : ""}</div>
        </div>
        <div className="cms-actions">
          <button type="button" className="btn btn-ghost btn-sm" data-testid="cms-save-draft" disabled={busy || !dirty} onClick={() => void saveDraft()}>{t("pages.content_admin.ef09cea8")}</button>
          <button type="button" className="btn btn-ghost btn-sm" data-testid="cms-preview" disabled={busy} onClick={() => void preview()}>{t("pages.content_admin.e95c9328")}</button>
          <button type="button" className="btn btn-primary btn-sm" data-testid="cms-publish" disabled={busy || (!dirty && !section.draft)} onClick={() => void publish()}>{t("pages.content_admin.fe330ad3")}</button>
          {section.draft || dirty ? <button type="button" className="btn btn-danger-ghost btn-sm" data-testid="cms-discard" disabled={busy} onClick={() => dirty && !section.draft ? (setWorking(section.published), setDirty(false)) : void discard()}>{t("pages.content_admin.010c65be")}</button> : null}
        </div>
      </section>
      ...
      <div className="stack cms-blocks">
        {working.blocks.map((block, index) => {
          ...
          return <BlockCard key={block.id} block={block} index={index} locked={locked} busy={busy}
            ...
            onRemove={() => { if (window.confirm(t("pages.content_admin.45f7d980", { name: TEMPLATES[block.type].name }))) update(p => ({ blocks: p.blocks.filter(b => b.id !== block.id) })); }}
            onMessage={text => setMessage({ tone: "err", text })} />;
        })}
      </div>
      {addable.length ? <section className="panel cms-add" data-testid="cms-add-block">
        <div className="panel-title">{t("pages.content_admin.5fd6d5ac")}</div>
        <div className="row">
          <select aria-label={t("pages.content_admin.a76898d5")} value={addType} disabled={busy || !canAdd} onChange={e => setAddType(e.target.value as TemplateId)}>
            <option value="">{t("pages.content_admin.f4c6723c")}</option>
            {addable.map(t => <option key={t} value={t}>{TEMPLATES[t].name}</option>)}
          </select>
          <button type="button" className="btn btn-ghost btn-sm" data-testid="cms-add-block-confirm" disabled={busy || !canAdd || !addType} onClick={addBlock}>{t("pages.content_admin.c3425511")}</button>
        </div>
        {addType ? <p className="muted small">{TEMPLATES[addType].description}</p> : null}
        {!canAdd && working.blocks.length >= contract.maxBlocks ? <p className="muted small">{t("pages.content_admin.3dae0af0", { maxBlocks: contract.maxBlocks })}</p> : null}
      </section> : null}
    </> : null}
...
function CopyPreview({ page }: { page: PageContent }) {
  ...
  return <section className="panel cms-copy-preview" data-testid="cms-copy-preview">
    <div className="panel-title">{t("pages.content_admin.e95c9328")}</div>
    {seller ? <div className="stack">
      <div className="notice info"><b>{f("seller", "pending_title")}</b> {f("seller", "pending_body")}</div>
      <div className="notice err"><b>{f("seller", "rejected_title")}</b> {f("seller", "rejected_body")} <a href="#/support">{t("pages.content_admin.3bc1abed")}</a></div>
      ...
      <p className="muted small">{t("pages.content_admin.629228d0", { v0: f("seller", "journey_title") })}</p>
...
function BlockCard({ block, index, locked, busy, canUp, canDown, onChange, onMove, onRemove, onMessage }: {
  ...
  return <section className={`panel cms-block${block.enabled ? "" : " cms-block-off"}`} data-testid={`cms-block-${block.id}`} data-block-type={block.type} data-enabled={block.enabled ? "1" : "0"} data-position={index}>
    <div className="cms-block-head">
      ...
      <div className="cms-block-title"><b>{tpl.name}</b>{locked ? <span className="chip cms-chip">{t("pages.content_admin.41920abc")}</span> : null}{!block.enabled ? <span className="chip cms-chip">{t("pages.content_admin.8544debd")}</span> : null}</div>
      <div className="cms-block-tools">
        {!locked ? <label className="cms-switch"><input type="checkbox" data-testid={`cms-block-enabled-${block.id}`} checked={block.enabled} disabled={busy} onChange={e => onChange({ ...block, enabled: e.target.checked })} />  {t("pages.content_admin.ca50af13")}</label> : null}
        <button type="button" className="btn btn-ghost btn-sm" aria-label={t("pages.content_admin.af6be407")} data-testid={`cms-block-up-${block.id}`} disabled={busy || !canUp} onClick={() => onMove(-1)}>▲</button>
        <button type="button" className="btn btn-ghost btn-sm" aria-label={t("pages.content_admin.250e598f")} data-testid={`cms-block-down-${block.id}`} disabled={busy || !canDown} onClick={() => onMove(1)}>▼</button>
        {!locked ? <button type="button" className="btn btn-danger-ghost btn-sm" data-testid={`cms-block-remove-${block.id}`} disabled={busy} onClick={onRemove}>{t("pages.content_admin.a99e68e9")}</button> : null}
      </div>
    </div>
      ...
      {tpl.items ? <div className="cms-items" data-testid={`cms-items-${block.id}`}>
        ...
        {items.map((item, i) => <div className="cms-item" key={i} data-testid={`cms-item-${block.id}-${i}`}>
          <div className="cms-item-tools">
            ...
            <button type="button" className="btn btn-ghost btn-sm" aria-label={t("pages.content_admin.af6be407")} data-testid={`cms-item-up-${block.id}-${i}`} disabled={busy || i === 0} onClick={() => setItems(move(items, i, i - 1))}>▲</button>
            <button type="button" className="btn btn-ghost btn-sm" aria-label={t("pages.content_admin.250e598f")} data-testid={`cms-item-down-${block.id}-${i}`} disabled={busy || i === items.length - 1} onClick={() => setItems(move(items, i, i + 1))}>▼</button>
            <button type="button" className="btn btn-danger-ghost btn-sm" data-testid={`cms-item-remove-${block.id}-${i}`} disabled={busy || items.length <= tpl.items!.min} onClick={() => setItems(items.filter((_, j) => j !== i))}>{t("pages.content_admin.7c8173fa")}</button>
          </div>
...
function Field({ id, def, value, busy, onChange, onMessage }: { id: string; def: FieldDef; value: string; busy: boolean; onChange: (v: string) => void; onMessage: (text: string) => void }) {
  ...
  if (def.kind === "image" || def.kind === "video") {
    ...
    const pick = async (file: File | undefined) => {
      ...
      catch (err: any) { onMessage(err?.message || t("pages.content_admin.d3a16f6e")); }
      finally { setUploading(false); }
    };
    return <div className="field cms-media" data-testid={id}>
      <label htmlFor={`${id}-file`}>{label}</label>
      {isVideo ? (value ? <video className="cms-media-preview" style={{ maxWidth: "100%", maxHeight: 220 }} src={value} muted playsInline controls preload="metadata" data-testid={`${id}-preview`} /> : <p className="muted small">{t("pages.content_admin.d9f0978c")}</p>)
        : <img className="cms-media-preview" style={{ maxWidth: "100%", maxHeight: 220 }} src={value || BRAND_LOGO_URL} alt={value ? t("pages.content_admin.f9535dc8") : t("pages.content_admin.78a8bdc2")} data-testid={`${id}-preview`} data-from-cms={value ? "1" : "0"} />}
      <div className="row">
        ...
        {value ? <button type="button" className="btn btn-ghost btn-sm" data-testid={`${id}-clear`} disabled={busy || uploading} onClick={() => onChange("")}>{t("pages.content_admin.a99e68e9")}</button> : null}
        {uploading ? <span className="muted small">{t("pages.content_admin.03914699")}</span> : null}
      </div>
    </div>;
  }
  if (def.kind === "select") {
  ...
  return <div className="field"><label htmlFor={id}>{label}</label>
    <input id={id} data-testid={id} type="text" dir={def.kind === "link" ? "ltr" : undefined} placeholder={def.kind === "link" ? t("pages.content_admin.f504ecee") : undefined} maxLength={def.max} value={value} disabled={busy} onChange={e => onChange(e.target.value)} /></div>;
}
