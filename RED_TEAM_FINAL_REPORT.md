...
- **Status:** FIXED for the trivial single-challenge brute (the exploitable path); the residual issuance-throttle is documented under A2/A3. Commits `9bbe82b` + (this commit).
...
- **Test:** `tests/security_production_guards_validation.ts` — guard rejects `COMPLETION_WINDOW_MINUTES=1`/`10080` in production and accepts unset/`1440`; the resolver returns 1440 under `NODE_ENV=production`/`RENDER`/`RENDER_EXTERNAL_URL` regardless of the override, and honors it under `NODE_ENV=test`.
- **Status:** FIXED (`9bbe82b`).
