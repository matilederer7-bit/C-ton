...
export async function ensureInvoiceRailTables(withTx: WithTx) {
  await withTx(async c=>assertRequiredTables(c,["invoice_documents","invoice_document_attempts","invoice_reconciliation_cases"]));
}

async function ensureInvoiceRailTablesForDb(db: pg.Pool | pg.PoolClient) {
  await assertRequiredTables(db,["invoice_documents","invoice_document_attempts","invoice_reconciliation_cases"]);
}
