...
export type AttemptType =
  ...
  | "cancel_refund"
  | "release"
  /** LONG_HORIZON_DEALS (069) — re-establishment of the authorization instrument from the stored payment method; never a charge attempt */
  | "reauthorize";
  ...
  async function beginProviderAttempt(args: {
    ...
    return deps.withTx(async (c) => {
      ...
      const releaseConflict = (args.attempt_type === "charge_start" || args.attempt_type === "recovery")
        ...
        : undefined;
      // LONG_HORIZON_DEALS (069) — while a re-authorization is unresolved the
