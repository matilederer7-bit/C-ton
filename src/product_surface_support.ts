...
export async function ensureRemainingProductSurfaceTables(withTx: WithTx) {
  if (!ensurePromise) {
    ...
    ensurePromise = (async () => await withTx(async (c) => {
      ...
      await c.query(`ALTER TABLE siton.seller_accounts ADD COLUMN IF NOT EXISTS seller_status TEXT NOT NULL DEFAULT 'Active'`);
      await c.query(`ALTER TABLE siton.seller_accounts ADD COLUMN IF NOT EXISTS seller_status_reason TEXT NOT NULL DEFAULT ''`);
      await c.query(`ALTER TABLE siton.seller_accounts ADD COLUMN IF NOT EXISTS seller_status_updated_at TIMESTAMPTZ NULL`);
      await c.query(`ALTER TABLE siton.seller_accounts ADD COLUMN IF NOT EXISTS seller_status_updated_by TEXT NULL`);
      await c.query(`UPDATE siton.seller_accounts SET seller_status='Active' WHERE seller_status IS NULL OR btrim(seller_status)=''`);
      await c.query(`
        DO $$
        BEGIN
          IF NOT EXISTS (
