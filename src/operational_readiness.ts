  ...
  return {
    ...
    payment_provider: {
      ...
      what_is_real: payment.mode === "provider-ready" && payment.configured
        ? "authorization contract selection and provider env wiring"
      ...
      what_is_mock: payment.mode === "mock-backed"
        ...
        : "capture, recovery, and refund still return non-live placeholder results even when provider-ready is selected",
      what_is_partial: payment.mode === "provider-ready"
        ? "authorization path can present as provider-ready, but downstream capture/recovery/refund rails are not live"
        : "none",
      depends_on_env: [
        "PAYMENT_PROVIDER",
    ...
    authorization_charge_recovery: {
      state: payment.mode === "mock-backed" ? "partially-simulated" : payment.configured ? "authorization-only-partial" : "not-ready",
      what_is_real: "state machine transitions, outbox scheduling, idempotency, webhook reconciliation, and persistence of payment attempts",
      what_is_mock: payment.mode === "mock-backed"
        ? "authorization, charge capture, recovery, and refund outcomes are generated by mock logic"
        : "downstream execution rails still do not hit a real provider transport",
    ...
    production_assumptions: {
      ...
      what_blocks_true_production: [
        ...
        "fully live payment capture/recovery/refund transport",
