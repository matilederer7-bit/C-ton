...
export function buildOperationalReadinessSummary(args: {
  ...
  const paymentActivation =
    payment.mode === "mock-backed"
      ...
      : payment.configured
        ? "authorization-only-contract-ready"
  ...
  return {
    ...
    payment_provider: {
      ...
      what_is_real: payment.mode === "provider-ready" && payment.configured
        ? "live authorization transport, live capture transport, provider env wiring, and canonical authorization/capture path selection"
      ...
      what_is_mock: payment.mode === "mock-backed"
        ...
        : "recovery and refund still return non-live placeholder results even when provider-ready is selected",
      what_is_partial: payment.mode === "provider-ready"
        ? "webhook truth, duplicate-safe ingestion, late-webhook safety, and capture reconciliation are live app rails, but recovery/refund are still not live"
        : "none",
      depends_on_env: [
        "PAYMENT_PROVIDER",
    ...
    authorization_charge_recovery: {
      state: payment.mode === "mock-backed" ? "partially-simulated" : payment.configured ? "authorization-capture-partial" : "not-ready",
      what_is_real: payment.mode === "provider-ready" && payment.configured
        ? "state machine transitions, outbox scheduling, idempotency, webhook ingestion, webhook reconciliation, payment-attempt persistence, live authorization transport, and live capture transport"
      ...
      what_is_mock: payment.mode === "mock-backed"
        ...
        : "recovery and refund outcomes still do not hit a real provider transport",
      what_is_partial: payment.mode === "provider-ready" && payment.configured
        ? "capture success/failure now resolves through webhook truth, but recovery/refund and full provider lifecycle coverage are still pending"
    ...
    production_assumptions: {
      ...
      what_blocks_true_production: [
        ...
        "fully live payment recovery/refund transport and mature reconcile coverage",
