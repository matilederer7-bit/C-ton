...
function attachClientErrorGuard(client: any, kind: string) {
  ...
  client.on("error", (err: any) => {
    // Connection-event diagnostics must never include arbitrary error text or
    // untrusted code strings. Query/transaction callers still receive the error.
    const code = typeof err?.code === "string" ? err.code : "";
    const safeCodes = ["08000", "08001", "08003", "08004", "08006", "08007", "08P01",
      "57P01", "57P02", "57P03", "57014", "53300", "53400", "ECONNRESET",
      "ECONNREFUSED", "ECONNABORTED", "EPIPE", "ETIMEDOUT", "ENOTFOUND", "EAI_AGAIN"];
    const observation = {
      at: new Date().toISOString(),
      kind,
      code: safeCodes.includes(code) ? code : "unknown",
